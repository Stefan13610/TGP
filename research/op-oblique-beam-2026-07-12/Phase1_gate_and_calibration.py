# Phase 1: BRAMKA G2 (czas zycia szachownicy L=256 pod dynamika
#          pomiarowa — MIERZONE ponownie, nie cytowane z proby #3)
#          + kalibracja G3 na MODACH SKOSNYCH (2q, q), q in
#            {10,13,20,25,30} (pasmo [0.5, 1.7])
#          + background_check + czesc G1
# Implementacja SCISLE wg Phase0_balance.md proby #5 (LOCKED;
# D1, D7, D8, D13, D14, D15). KOLEJNOSC = CZESC PROTOKOLU (D18):
# bramka G2 przed jakimkolwiek runem z pakietem; G2 FAIL -> STOP
# calego cyklu (zero runow ugiecia; errata techniczna — to samo
# tlo co proba #3).
#
# background_check: szachownica 4 wirow L=256 (2 pary poziome
#   theta_h_pair — kod BEZ ZMIAN vs proba #3), szew, detektor 2+2
#   (D14), relaksacja 2000, determinizm bitowy.
...
# G3 (D7): tlo jednorodne s*, zaburzenie rzeczywiste
#   1e-3*cos(kx x + ky y), k = (2 pi/L)(2q, q); okno tau=100;
#   blad <= 5%; dyspersja sieciowa analityczna obok; determinizm
#   bitowy pierwszego modu.
...
print("=== Phase 1: BRAMKA G2 (tau=600) + kalibracja G3 na modach "
      "SKOSNYCH + background_check ===")
print(f"(LOCK proby #5: D1/D7/D8/D14/D15) N={N} L={L} dx={dx} dt={dt}")
print()

# ---------------- background_check ----------------
print("--- background_check: szachownica 4 wirow L=256 (kod tla BEZ "
      "ZMIAN vs proba #3) ---")
psi = make_lattice()
th0 = np.angle(psi)
...
print(f"  diagnostyka szwu: max |dtheta| poza rdzeniami = "
      f"{seam:.4f} rad << pi: {seam < 0.5}  (proba #3: 0.1244)")
...
print("    (kryterium: max przemieszczenie KAZDEGO rdzenia <= 1.0 "
      "dla tau <= 450; (450,600] poza kryterium; MIERZONE, nie cytowane)")
...
per_core_450 = {k: max(p[2][k] for p in disp_series if p[0] <= 450.0)
                for k in pos0} if m450.any() else {}
if per_core_450:
...
print("    KONTRAST (cytowane): proba #3 (to samo tlo) — 0.0000 przez "
      "tau=600; siatka skosna proby #4 — utrata rdzeni tau=227; "
      "para — 107.5")
...
if not g2_pass:
    print("=== G2 FAIL -> STOP CALEGO CYKLU wg reguly decyzyjnej ===")
    print("  zadnych runow ugiecia; to samo tlo co proba #3 (0.0000)")
    print("  -> FAIL wskazywalby na blad srodowiska: errata techniczna")
    print("  kalibracja G3 POMINIETA")
    raise SystemExit(0)

# ---------------- G3: dyspersja na modach SKOSNYCH (D7) ----------------
print("--- G3: dyspersja na tle jednorodnym s* (D7), mody SKOSNE "
      "k = (2 pi/L)(2q, q) ---")
print("    q     |k|     omega_meas   omega_teor   blad%   omega_siec   "
      "H_dryft(sek.)  H_osc")
TAU_WIN = 100.0
...
for qq in Q_LIST:
    kx = TWO_PI * 2 * qq / L
    ky = TWO_PI * qq / L
    kk = float(np.hypot(kx, ky))
    pert = 1e-3 * np.cos(kx * X + ky * Y)
...
print(f"G3 (kalibracja SKOSNA, pasmo [0.5,1.7]): max |blad| = "
      f"{max(errs):.2f}% <= 5%: {disp_ok}  "

...
def w12_dispersion_relation():
    ...
    print("    => omega_+-^2 = k_z^2 +- g k_z n_par,  n_par = k_hat . grad(ln X)")
    print()
    print("  [NOTE 2026-05-01: written in dimensionally-explicit form;")
    print("   shorthand 'omega^2 = k^2 +- g (n.k_hat)' is dimensionally ambiguous]")
    print()
    print("  Sign meaning:")
    ...
    print("    omega_+-(k) = k * sqrt(1 +- g n_par / k)")
    print("    ~ k +- g n_par / 2  (leading WKB)")
    print()
    print("  Equivalent covariant form: with p_mu = g d_mu(ln X),")
    print("    omega^2 = k^2 +- k p_par  (special case of CFJ-class")
    print("    (k^2)^2 + p^2 k^2 - (p.k)^2 = 0 in WKB).")
    print()
    print("  This is the canonical axion-photon dispersion relation,")
    print("  parity-odd birefringence at leading order in g n_par / k.")
    pass_gate = True
    print(f"  -> {'PASS' if pass_gate else 'FAIL'}")
    return pass_gate
...
def w13_polarization_eigenmodes():
    ...
    print()
    print("  Eigenvectors:")
    print("    e_+ (right circular):  eigenvalue lambda_+ = -k^2 + g k n_par")
    print("    e_- (left circular):   eigenvalue lambda_- = -k^2 - g k n_par")
    print()
    print("  Setting lambda_+- = 0:")
    print("    omega_+-^2 = k^2 +- g k n_par,  n_par = k_hat . grad(ln X)")
    print()
    print("  Note sign convention: we choose convention where")
    print("  + helicity has higher omega^2 if g > 0 and n_par > 0.")
    ...
    print("    Delta chi(L) = (omega_+ - omega_-) L / 2 = (g/2) integral n_par ds")
    print("                 = (g/2) integral d(ln X)  (gauge-invariant integrated gradient)")
...
def w14_wkb_validity():
    ...
    print("    n / k ~ 1e-33 << 1")
    print()
    print("  Lab static parallel E.B substrate gradient:")
    print("    d(ln X)/dx induced by E.B (omega.1 box(ln X) = (g/4 f_X^2) F F~,")
    print("    where F.F~ = -8 E.B -- pure B alone gives F.F~=0, hence no n_par)")
    print("    For f_X ~ 1e16 GeV (Planckian), |d ln X| ~ 1e-30 / m for")
    print("    parallel E ~ 1e8 V/m, B = 10 T  ->  E.B ~ 1e9 V*T/m")

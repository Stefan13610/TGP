...
def w31_lab_phase_velocity():
    banner("W3.1 -- Lab phase-velocity Mach-Zehnder + parallel E.B test")
    print("  [NOTE 2026-05-01: source for n_par is F.F~ \\propto E.B, NOT pure B^2.")
    print("   A pure static B field with E = 0 gives F.F~ = 0 and does NOT induce n_par.]")
    print()
    print("  Setup: Mach-Zehnder interferometer, one arm passing through region with")
    print("  parallel E and B (F.F~ \\propto E.B != 0) of length L. Measure relative")
    print("  phase shift between L/R circularly polarized photons.")
    print()
    print("  Phase shift (sigma.1 prediction; dispersion omega^2 = k^2 +- g k n_par):")
    print("    Delta phi_+- = (omega_+ - omega_-) * t \\approx g * n_par * t")
    print("    where n_par = k_hat . grad(ln X), induced by F.F~ = -8 E.B via omega.1")
    print()
    print("  In static B field with E = 0, F.F~ = 0 -> n_par induced = 0")
    print("    -> NULL lab-induced signal in pure B field")
    print()
...
def w33_cmb_chirality():
    ...
    print()
    print("  Status: LIVE PARTIAL candidate via Planck PR4 + ACT 2024 ~3.8 sigma")
    print("          (downgraded 2026-05-01: partial detection / hint, not confirmed signal;")

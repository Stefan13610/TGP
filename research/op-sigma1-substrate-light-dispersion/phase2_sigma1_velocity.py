...
def w21_phase_velocity():
    ...
    # Dispersion (dimensionally consistent form, k > 0, n_par = k_hat . grad ln X):
    # omega_+-^2 = k^2 +- g k n_par   (NOT "k^2 +- g (n . k_hat)" -- explicit form
    # avoids dimensional ambiguity; both sides have dim 1/L^2)
    omega_plus  = sp.sqrt(k**2 + g * k * n_par)
    omega_minus = sp.sqrt(k**2 - g * k * n_par)
    v_plus  = omega_plus / k
    v_minus = omega_minus / k
    print(f"  Dispersion: omega_+-^2 = k^2 +- g k n_par,  n_par = k_hat . grad ln X")
...
def w22_group_velocity():
    ...
    print("  At O(n_par^2): v_g_+- = 1 + (g n_par)^2 / (8 k^2)   [POSITIVE; sign-corrected 2026-05-01]")
    print("  Direct check: omega_+- = sqrt(k^2 +- g k n_par)")
    print("                v_g    = d omega/dk = (2k +- g n_par) / (2 omega)")
    print("                       = 1 + (g n_par)^2/(8 k^2) + O(eps^3)")
    # Numeric verification of the symbolic series (positive sign at O(n_par^2)):
    coeff_plus = sp.simplify((v_g_plus_quad - 1) / n_par**2)
    coeff_minus = sp.simplify((v_g_minus_quad - 1) / n_par**2)
    expected_coeff = g**2 / (8 * k**2)
    diff_plus_coeff  = sp.simplify(coeff_plus  - expected_coeff)
    diff_minus_coeff = sp.simplify(coeff_minus - expected_coeff)
    print(f"  Coeff(v_g_+ at n_par^2)  - g^2/(8k^2) = {diff_plus_coeff}")
    print(f"  Coeff(v_g_- at n_par^2)  - g^2/(8k^2) = {diff_minus_coeff}")
    print()
...
def w25_optical_metric():
    banner("W2.5 -- Helicity-dependent optical cone (effective dispersion geometry)")
    print("  [LANGUAGE softened 2026-05-01: not a full classical pseudo-Riemannian metric")
    print("   -- this is a leading WKB null-cone / effective dispersion geometry for")
    print("   the special case n_i parallel to k_i, static gradient. Full CFJ-class")
    print("   structure (k^2)^2 + p^2 k^2 - (p.k)^2 = 0 is more general.]")
    print()
    print("  Modified Maxwell can be cast as photon propagation along a helicity-")
    print("  dependent optical cone:")
    print("    Q^{(+-)}_{mu nu}(p) k^mu k^nu = 0,  p_mu = g d_mu(ln X)")
    print("    Special case (n parallel k, static): omega_+-^2 = k^2 +- g k n_parallel")
    print()
    print("  For a photon of helicity +- propagating in n_mu = d_mu(ln X) gradient:")
    print("    null condition (helicity-specific): Q^{(+-)} k^mu k^nu = 0")
    print("  Inverting: omega_+-^2 = k^2 +- g k n_parallel  (WKB special case)")
    print()
    print("  Geometric content (NOT classical metric):")
    print("    leading shift in null-cone:  delta omega^2 / omega^2 ~ +- g n_par / k")
    print("    helicity-specific cone tilts in opposite directions -> birefringence")
    print()
    print("  c-mechanism interpretation:")
    print("    photon 'speed' is cone-dependent and helicity-dependent.")
    print("    polarization-averaged phase speed = 1 (no scalar c(X)).")
    print("    polarization-difference phase speed = g n_par / k (BIREFRINGENCE).")
    print()
    print("  Consistency: in flat substrate (n = 0), Q^{(+-)} -> eta")

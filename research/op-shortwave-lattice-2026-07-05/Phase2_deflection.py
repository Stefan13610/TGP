# Phase 2: skan ugiecia krotkofalowego na siatce L=256 — G4, G5, G6b
#          + obserwacje poza kryteriami: (1.1,8), (1.3,8), (1.7,8),
#            superpozycja (1.5,8) — dedykowany run D16 (limit 10000)
...
# rdzeni z psi_ref (detektor 2+2, D14, nominaly +/-64); wyzwolenie
# CENTROIDEM, okno ZAMRAZANE w chwili wyzwolenia (D4); b_eff (D5,
# b_eff/lambda raportowane per run); alpha_pred = ray tracing przy
# b_eff, kat czytany w plaszczyznie centroidu 3. probki + czulosc
# na krancach okna (D6).
...
def deflection_run(psi_bg, n2cache, om, b_nom, u0=1e-3, tag="",
    ...
    lam = TWO_PI / k0
    max_steps = 10000 if superpos else 6000   # D9 (wyjatek D16)
...
def report(r):
    ...
    if r["core_start"] is not None:
        ...
        print(f"    rdzen rozpr.: ({cs[0]:.2f},{cs[1]:.2f}) -> "
...
print("=== Phase 2: skan ugiecia krotkofalowego L=256 (LOCK: "
      "Phase0_balance.md, D1-D6/D8-D10/D12-D14/D16) ===")
print(f"N={N} L={L} dx={dx} dt={dt}; tlo: szachownica 4 wirow +/-64; "
      f"pakiet u0=1e-3 sx=sy=8, start x0={X0}, y0=-64+b")
...
n2c = {om: build_n2(psi_bg, om) for om in (1.1, 1.3, 1.5, 1.7)}
print()

# ---------------- skan glowny (G4, G5) ----------------
print("--- skan glowny: omega=1.1 x b {14,16,20}; omega=1.5 x b {8,12,16} "
      "(KRYTERIALNE, zestaw ZAMKNIETY) ---")
...
r_o118 = deflection_run(psi_bg, n2c[1.1], 1.1, 8.0, tag="obs-1.1/8")
report(r_o118)
r_o138 = deflection_run(psi_bg, n2c[1.3], 1.3, 8.0, tag="obs-1.3/8")
report(r_o138)
r_o178 = deflection_run(psi_bg, n2c[1.7], 1.7, 8.0, tag="obs-1.7/8")
report(r_o178)
r_sp = deflection_run(psi_bg, n2c[1.5], 1.5, 8.0, tag="obs-superpos-D16",
                      superpos=True)
report(r_sp)
print()
...
g4_b = (abs(runs[(1.1, 14.0)]["alpha"]) > abs(runs[(1.1, 16.0)]["alpha"])
        > abs(runs[(1.1, 20.0)]["alpha"]))
g4_c = abs(runs[(1.1, 16.0)]["alpha"]) > abs(runs[(1.5, 16.0)]["alpha"])
g4 = g4_a and g4_b and g4_c
print(f"G4 (istnienie): |alpha(14,1.1)|={abs(r0['alpha']):.2f} > "
      f"3*sigma={3*r0['sigma']:.2f}: {g4_a}; maleje z b (14>16>20 przy "
      f"1.1): {g4_b}; |alpha(1.1,16)| > |alpha(1.5,16)|: {g4_c}  "
      f"-> {'PASS' if g4 else 'FAIL'}")

g5_ok = True
print("G5 (RDZEN, policzalnosc P1 w domenie eikonalu): znak KU "
      "(alpha>0) ORAZ alpha/alpha_pred(b_eff) in [0.5, 2.0] dla "
      "WSZYSTKICH 6 punktow:")
for om, bb in CRIT:
    r = runs[(om, bb)]
    ratio = (r["alpha"] / r["alpha_pred"]
...
print()
print("obserwacje (POZA kryteriami, raport):")
for r, lbl in ((r_o118, "1.1/8 (test rezimu: L=128 dawal ratio 0.445)"),
               (r_o138, "1.3/8 (sprzeg z proba #2: ratio 0.741)"),
               (r_o178, "1.7/8 (ultra-krotkofalowo, dyspersja ~3%)")):
    ratio = (r["alpha"] / r["alpha_pred"]
             if np.isfinite(r.get("alpha_pred", np.nan)) and r["alpha_pred"] != 0
             else np.nan)

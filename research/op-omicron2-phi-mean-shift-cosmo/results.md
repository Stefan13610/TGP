...
🔴 **TGP NIE jest Hubble tension solver przez ŻADEN discovered mechanism.**

**Trzy niezależne stages, wszystkie NULL:**

| Stage | Mechanism | Magnitude | Verdict |
|---|---|---|---|
| Stage 0 | Naive `dH/H = 0.5·dL/L·Ω_L` | +8.93% claimed | 🔴 Wrong formula |
| Stage 1 | Proper D_A integration (G constant) | +0.61% | 🔴 NULL (Λ_recomb negligible vs ρ_m_recomb) |
| Stage 1' | G(ψ) running w/ self-consistency | +0.00% | 🔴 NULL (sek04 self-consistency cancels) |

**M10.5 verdict reinforced through THREE independent mechanisms.**

🟢 **Co WAS revealed (real physics, sticks):**
- sek01 ontology potwierdzona: Φ_0 generated by matter (qualitatively)
- sek01 ↔ sek08a internally consistent (matter source term exists in formalism)
- sek04 axioms (G(ψ), c(ψ)) self-consistency mathematically robust
- ψ tracks ρ̄(t) cosmologically (~20% shift over Hubble time)
- Stage 0 magnitude error caught BEFORE publication (verification works)
- M10.1 w(z) audit may need revision z proper source term — separate issue
  (w_today ≈ -0.93, possibly closer to DESI DR2 than ΛCDM)

🟡 **Affected M10 predictions** (post all-stages NULL):
- M10.5: REINFORCED (different mechanism, same conclusion)
- M10.1 w(z) prediction: may shift from -1 to -0.93 (independent issue)
- M10.R falsification matrix: F1 (DESI w(z)) status update needed
- DESI compatibility: actually IMPROVES with -0.93 (closer to DR2 -0.75)

## 12. Stage 1' summary (added 2026-05-03 wieczór)

**Audit v2 ujawnił** że Stage 0 i Stage 1 oba ignorowały sek04 fundamental axioms:
- A1: G(Φ) = G_0·(Φ_0/Φ) — gravitational constant field-dependent
- A2: c(Φ) = c_0·(Φ_0/Φ)^(1/2) — speed of light field-dependent
- A4: Self-consistency principle — local observers measure constants in own units

**Stage 1' results (z G(ψ) in proper FRW):**
- Test 1 (naive plug ψ=0.78): boost = +13.2% (overshoot)
- Test 2 (proper FRW solve, shoot V_0): boost = **0.0%** (NULL)

Test 2 NULL is consistent with **self-consistency principle**: when FRW jest solved
self-consistently z G(ψ) w obu Friedmann i EOM, system adjusts ψ trajectory tak
że H_today_TGP = H_today_LCDM by construction. G(ψ) running cancels out for
cosmologically observable H_0.

**Side finding:** G_BBN/G_today = 0.82 w naive interpretation = -18% drift.
Self-consistency may save it (BBN observers in own ψ-frame measure G_local = G_0),
ale ta interpretacja deserves separate audit beyond omicron2 scope.

## 13. Cross-check ze stare files

| Plik | Status post-omicron2 | Action needed |
|---|---|---|
| M10.5 verdict ("TGP nie solver") | ✅ REINFORCED triplikowane | none |
| M10.1 w(z) ≈ -1 prediction | 🟡 may shift to -0.93 z proper source | separate cycle ("Path A") |
| sek01 ontology | ✅ confirmed (matter source exists) | none |
| sek04 G(ψ), c(ψ) axioms | ✅ confirmed (self-consistency robust) | none |
| sek08a action | ✅ unchanged | none |
| closure_2026-04-26 (T-Λ + UV.3 + γ.1 + δ.1+2) | ✅ unchanged | none |

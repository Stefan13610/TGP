#!/usr/bin/env python3
# -*- coding: ascii -*-
"""op-matter-induced-creation -- silnik dynamiki 2. rzedu.

KOPIA silnika `../op-collapse-matter-source-2026-09-14/engine_core.py`
(FROZEN, MD sec. 1-3 tego cyklu; cytat nizej). JEDYNA zmiana
merytoryczna (LOCK sec. 1 i sec. 3 tego cyklu): **lam zalezne od
czasu** wg zamrozonej rampy
    lam(t) = lam * S((t_off + D - t)/D),  S = smootherstep,
    D = 100, t_off = 600  =>  lam(t)=lam dla t<=600, 0 dla t>=700.
Dla lam = const silnik jest BITOWO identyczny z poprzednikiem
(podstawiana jest ta sama tablica self.lrho).

CYTAT (naglowek poprzednika, ktory sam cytuje op-r3-stationary-states):
  ...
  "Czlon materii (WYNIK P1-H1 poprzednika, Phase1_output.txt, gate PASS):
...
# ---- rampa lam(t) [LOCK sec. 3 tego cyklu; JEDYNA zmiana] --------
T_OFF = 600.0            # LOCK
DELTA_RAMP = 100.0       # LOCK


def smootherstep(x):
    """S(x)=0 (x<=0), 6x^5-15x^4+10x^3 (0<x<1), 1 (x>=1)."""
    y = min(1.0, max(0.0, float(x)))
    return y*y*y*(y*(6.0*y - 15.0) + 10.0)


def lam_of_t(lam, t, t_off=T_OFF, delta=DELTA_RAMP):
    """lam(t) = lam * S((t_off + delta - t)/delta)  [LOCK sec. 3].
    t <= t_off          -> lam  (plateau, faza wlaczona)
    t >= t_off + delta  -> 0.0  (dokladnie; ewolucja swobodna)"""
    return lam*smootherstep((t_off + delta - t)/delta)
...
def dUfun(p):
    ...
    tozsamosc DOKLADNA (correction note 2 op-r3-stationary-states)."""
    d = p - 1.0
...
class Engine:
    ...
    zrodla statycznego (lam=0 -> silnik bez materii)."""
    ...
    def __init__(self, h, R, sponge=True, gamma0=GAMMA0_SP, lam=0.0):
        ...
        self.rm2 = self.rm**2
        self.rho = rho_hat(self.r)                # rho_hat(r_i) [LOCK]
        self.lrho = lam*self.rho                  # lam(t) * rho_hat
        if sponge:
    ...
    # ---- lam(t): JEDYNA zmiana merytoryczna (LOCK sec. 3) --------
    def set_lam(self, lam_t):
        """Ustawia biezace lam(t) (tablica lrho = lam(t)*rho_hat)."""
        self.lrho = lam_t*self.rho
        return self.lrho

    def lrho_of(self, lam_t):
        """Zwraca tablice lam(t)*rho_hat BEZ zmiany stanu."""
        return lam_t*self.rho
    ...
    def energy(self, g, pi, rmax=None, matter=True):
        """matter=True: gestosc PELNA (zachowana przy lam=const;
        z czlonem U_mat prozniowo odjetym); matter=False: E^field
        deskryptywnie."""
        gm = 0.5*(g[:-1] + g[1:])
        dg = np.diff(g)/self.h
    ...
    def step(self, g, pi, dt, lrho_new=None):
        """lrho_new: tablica lam(t+dt)*rho_hat (sila F2 liczona
        w chwili t+dt). None => lam stale (krok BITOWO identyczny
        z silnikiem poprzednika)."""
        F1 = self.F(g)
        M1 = Mfun(g)
        Mp1 = Mpfun(g)
        c1 = Mp1/(2.0*M1*M1)
        ...
        g2 = x
        # (2b) [LOCK sec. 3] lam(t) -> lam(t+dt) przed sila F2
        if lrho_new is not None:
            self.lrho = lrho_new

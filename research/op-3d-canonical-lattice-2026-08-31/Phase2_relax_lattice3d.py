...
# -*- coding: ascii -*-
"""
op-3d-canonical-lattice (Phase 2) -- istnienie samouzgodnionego tla
sieci sc 3D (akcja kanoniczna K=g^4, U=g^7/7-g^8/8, beta=gamma=1).

KOPIA ../op-3d-lattice-bath-stability-2026-08-31/Phase2_relax_lattice3d.py
(nieuruchomionego; newton_krylov/precond/starty/klasyfikacja verbatim).
DELTY (Phase_method_decisions.md sec. 3, FROZEN): (1) profil startowy =
radialny profil KANONICZNY mu (INPUT g0_mu = phi*0.90548 = 1.4650974,
obiekt PRIMARY LOCKa sec. 2), nie profil #63 F_eps; (2) sciezki BASE/OUT
na katalog tego cyklu; (3) naglowek.

LOCK: Phase0_balance.md sec. 3 Phase 2 (przejete doslownie od poprzednika).
...
x 2 starty (superpozycja periodyzowana profili izolowanych mu;
PRIMARY 1.0x oraz 0.7x amplitudy) x N in {32,48}.
...
G0_MU = PHI_GOLD * G0_E              # = 1.4650974 (kalibracja mu; MD)
...
# ------------------------------------------ profil radialny KANONICZNY (start)
def profile_canonical(g0, R, N):
    """Radialne EL kanoniczne: g'' = g^2(1-g) - (2/g)g'^2 - (2/r)g'."""
    def rhs(r_, y):
        g, gp = y
        g = max(g, 1e-3)
    ...
    sol = solve_ivp(rhs, [1e-6, R + h], [g0, 0.0], method='DOP853',
                    max_step=0.02, rtol=1e-12, atol=1e-14,
                    dense_output=True)
...
stamp("profil radialny mu KANONICZNY (INPUT g0=%.7f)..." % G0_MU)
r_rad, g_rad, full = profile_canonical(G0_MU, 60.0, 4800)
...
def start_field(d, N, scale):
    """Superpozycja periodyzowana 27 obrazow (FROZEN MD-poprzednika sec.3)."""
    h = d / N
    x = np.arange(N) * h
    c = d / 2
    g = np.ones((N, N, N))
...
# ===================================================================== RUN
print("=" * 78)
print("Phase 2 -- istnienie tla sieci sc 3D (akcja kanoniczna; obiekt")
print("PRIMARY: soliton mu kanoniczny). LOCK sec. 3 Phase 2; newton_krylov")
print("FROZEN (MD-poprzednika sec. 4, przejete). Kryteria: ||R||_inf <=")
print("1e-8, ||g-1||_inf >= 0.05, ||g_32-g_48||_inf <= 5e-3 (podsiatka")
print("16^3). INPUT: g0_mu=%.7f (=phi*0.90548), d*1=%.4f, beta=gamma=1."
      % (G0_MU, DSTAR1))
print("=" * 78)
...
if any_exists:
    ...
    print("(pelnoprawny negatyw: siec sc 3D kanoniczna nie istnieje")
    print(" w klasie zbadanej; drzewo LOCKa sec. 6 -> NEEDS: bcc/fcc /")
    print(" inna geometria).")
print("=" * 78)

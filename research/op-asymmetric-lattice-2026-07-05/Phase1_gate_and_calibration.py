# Phase 1: BRAMKA G2 (czas zycia siatki SKOSNEJ pod dynamika pomiarowa)
#          + kalibracja G3 (dyspersja, pasmo [0.5, 1.7])
#          + background_check + czesc G1
# Implementacja SCISLE wg Phase0_balance.md proby #4 (LOCKED; D1, D7,
# D8, D9, D13, D14, D15, D19). KOLEJNOSC = CZESC PROTOKOLU: bramka G2
# przed jakimkolwiek runem z pakietem; G2 FAIL -> STOP CALEGO CYKLU
# (zero runow ugiecia; deliverable: czasy zycia).
#
# background_check: siatka skosna 8 wirow L=256 (4 pary poziome
#   theta_h_pair), diagnostyka szwu, detektor 4+4 (D14, nominaly
#   sekcji 0), relaksacja 2000, determinizm bitowy.
# G2 (D15): ewolucja II rzedu BEZ pakietu, 12000 krokow (tau=600);
#   pozycje 8 rdzeni co 10 krokow; kryterium: max przemieszczenie
#   KAZDEGO z 8 rdzeni <= 1.0 dla tau <= 450; (450,600] poza kryterium.
#   RYZYKO #1 cyklu: mody scinajace siatki skosnej (sekcja 1.2).
...
print("=== Phase 1: BRAMKA G2 (tau=600, 8 rdzeni) + kalibracja G3 "
      "[0.5,1.7] + background_check ===")
print(f"(LOCK: Phase0_balance.md proby #4, D1/D7/D8/D14/D15) "
      f"N={N} L={L} dx={dx} dt={dt}")
print()

# ---------------- background_check ----------------
print("--- background_check: siatka skosna 8 wirow L=256 "
      "(4 pary poziome) ---")
psi = make_lattice()
th0 = np.angle(psi)
...
print(f"  diagnostyka szwu: max |dtheta| na linkach poza rdzeniami = "
      f"{seam:.4f} rad << pi: {seam < 0.5}  (proba #3: 0.124; "
      f"4 pary = nowa kombinacja)")
cp0 = core_positions(psi)
print(f"  kret ansatzu: n_tot = {cp0[1]:.0f}; 8 rdzeni: "
...
print("    (kryterium: max przemieszczenie KAZDEGO z 8 rdzeni <= 1.0 "
      "dla tau <= 450; (450,600] poza kryterium)")
print("    (RYZYKO #1: mody scinajace siatki skosnej — rozstrzyga "
      "pomiar, nie zalozenie)")
...
if annih_tau is not None:
    print(f"    UTRATA RDZENI (detektor 4+4) przy tau = {annih_tau:.1f}")
...
print("    KONTRAST (cytowane): szachownica L=256 (proba #3) — 0.0000 "
      "przez tau=600; para (proba #1) — anihilacja tau=107.5")
...
if not g2_pass:
    ...
    print("  (siatka skosna vs szachownica L=256 vs para);")
    print("  kandydaci (osobne locki): mniejsze scinanie (s=32), L=512")
    print("  kalibracja G3 POMINIETA")
    raise SystemExit(0)

# ---------------- G3: dyspersja kanalu amplitudowego (D7) ----------------
print("--- G3: dyspersja na tle jednorodnym s* (D7), pasmo [0.5, 1.7] ---")
...
print("    (powtorka pomiaru proby #3 na tym samym gridzie — pomiar, "
      "nie cytat; #3 dawala max |blad| = 1.73%)")
print()
...
print(f"G1 (czesc Phase1): determinizm bitowy (fala: {det_ok}, "
      ...
      f"{H_DRIFT_TOL}; kret tla n_tot=0 i 8 rdzeni: {cpR[1] == 0}  "
      f"-> {'PASS' if g1_part else 'FAIL'}")
print(f"G2 (BRAMKA czasu zycia siatki skosnej): max przemieszczenie "
      f"(tau<=450) = {dmax450:.4f} <= 1.0  -> "
      f"{'PASS' if g2_pass else 'FAIL'}")

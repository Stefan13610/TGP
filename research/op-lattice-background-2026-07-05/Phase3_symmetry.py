...
# Implementacja SCISLE wg Phase0_balance.md (LOCKED; sekcje 1.4, 2, 4;
# D11). Pre-rejestracja (#1.4 + D11): obie rownosci sa DOKLADNYMI
# symetriami konstrukcji (S1: odbicie osi wiazki + sprzezenie;
# S2: lustro ladunkowe = pelne sprzezenie zespolone runu) -> to testy
# IMPLEMENTACJI, nie fizyki; alpha_odd = 0 z konstrukcji NIE jest
# raportowana jako pomiar P2 (forbidden move #5).
#
# Runy: baseline (omega=1.1, b=+8) — powtorka bitowa runu Phase 2;
#       wariant S1: b = -8 (ta sama siatka);
#       wariant S2: tlo lustrzane (theta -> -theta), b = +8.
# Kryterium G6a: |alpha(+8) - alpha(-8)| <= 0.05 * |alpha(+8)|
#            ORAZ |alpha(lustro,+8) - alpha(+8)| <= 0.05 * |alpha(+8)|.
...
def deflection_run(psi_bg, om, b_nom, u0=1e-3, tag="", sc_sign=+1):
    ...
    porownuje alpha między wariantami). sc_sign: znak rdzenia
...
print("=== Phase 3: symetrie tla — G6a (LOCK: sekcje 1.4/4, D11) ===")
...
print("--- baseline: (omega=1.1, b=+8) — powtorka runu Phase 2 ---")
r_base = deflection_run(psi_bg, 1.1, +8.0, tag="baseline")
report(r_base)
print()
print("--- wariant S1: b = -8 (odbicie osi wiazki) ---")
r_neg = deflection_run(psi_bg, 1.1, -8.0, tag="S1 b=-8")
report(r_neg)
print()
print("--- wariant S2: tlo lustrzane, b = +8 ---")
r_mir = deflection_run(psi_bg_m, 1.1, +8.0, tag="S2 lustro", sc_sign=-1)
report(r_mir)
print()
...
print(f"G6a-S1: |alpha(+8) - alpha(-8)| = |{a0:+.5f} - "
      f"{r_neg['alpha']:+.5f}| = {d_s1:.2e} <= {thr:.4f}: {ok_s1}")
print(f"G6a-S2: |alpha(+8) - alpha(lustro,+8)| = |{a0:+.5f} - "
      f"{r_mir['alpha']:+.5f}| = {d_s2:.2e} <= {thr:.4f}: {ok_s2}")
print(f"  -> G6a: {'PASS' if ok_s1 and ok_s2 else 'FAIL'}")
print()
print("UWAGA (pre-rejestrowana, #1.4/forbidden move #5): alpha_odd = "
      f"[alpha(+8)-alpha(-8)]/2 = {0.5*(a0-r_neg['alpha']):+.2e} deg = 0 "

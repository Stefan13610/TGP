...
> **✅ AUDIT 2026-05-01 (A.2) — B6 CLOSED 2026-05-01.** A.2 finding (M9.2
> m_field used `√(-g) = c·ψ` legacy Form-I) closed by [[B6_m9x_sqrtg_rerun_results.md]]
> 6/6 PASS. Form-IV `√(-g) = c·ψ/(4-3ψ)` sympy-LOCKED; m_field weak-field
> central value updates **3.483×10⁻² → 3.979×10⁻² (+14.2%)** under correct
> measure. Structural M9.2 5/5 PASS verdict (Newton I, finite m_field,
> WEP, M² scaling at weak-field, no radiation) **PRESERVED**. M² scaling
> recovers exactly in weak-field limit (M ≤ 0.5: rel_dev 2.6×10⁻²); strong
> M breaks pure M² universality from form-IV nonlinearity (expected).
> Patrz: [[B6_m9x_sqrtg_rerun_results.md]], [[../../meta/AUDYT_TGP_2026-05-01.md]] § A.2, § U.

> **🛠 B9-CLOSED 2026-05-01 — two-component WEP composition test (6/6 PASS):**
> M9.2.3 single-source M²-universality verdict ("WEP intrinsic ~ U²") **upgraded** explicit two-component:
> - **Geometric** (Pt vs Ti, σ=1.0 vs 1.68, same q, M): η_geom_natural = 6.68×10⁻¹
> - **Coupling** (δq/q ∈ {0.1, 0.01, 0.001}): linear scaling `η_AB → 1×(δq/q)` (rel_dev 4.80×10⁻²)
> - **Inhomogeneous ρ** (core+shell vs pure Gaussian, same M): η_inhom = 0.289
> - **Sympy LOCK leading scaling**: `m_field ~ (qM)²/(4π σ)`, `η_AB ~ |δq/q| + |δσ/σ|`
> - **Realistic MICROSCOPE Pt vs Ti**: η_TGP_lab = **1.32×10⁻²⁶** (margines 8.31×10¹⁰× SAFE vs 1.1×10⁻¹⁵)
> - **Combined T-α suppression**: η_TGP_combined = **6.49×10⁻⁴⁵** (margines 1.70×10²⁹×)
>
> Audit B9 demand "test kompozycji (dwa źródła o różnym q lub niejednorodnym ρ)" satisfied **w obu modach** + bonus geometric mode. Skrypt: [[B9_wep_microscope_composition.py]] | Synteza: [[B9_wep_microscope_composition_results.md]] | Audit § W: [[../../meta/AUDYT_TGP_2026-05-01.md]] § W.
...
**Wyniki:**
- `m_field / m_g = 3.48×10⁻²` (poprawka pole-do-bare; post-B6: **3.98×10⁻²**)
...
> **🛠 B9-CLOSED 2026-05-01 — explicit two-component test:**
> Powyższy verdict M9.2.3 oparty był na *single-source* `M²` universality (heurystyczne `η_compositional ~ U²` z pojedynczego ciała). Audit B9 (HIGH) wymagał **explicit dwukomponentowego** testu (Pt vs Ti).
>
> [[B9_wep_microscope_composition.py]] (6/6 PASS) implementuje:
> - **Sympy LOCK leading scaling**: `m_field ~ (qM)²/(4π σ)` → `η_AB ~ |δq/q| + |δσ/σ|`
> - **Geometric** Pt (σ=1.0) vs Ti (σ=1.68) — η_geom_natural = 6.68×10⁻¹
> - **Coupling** δq/q ∈ {0.1, 0.01, 0.001} — linear `η_AB → 1×(δq/q)`, rel_dev 4.80×10⁻²
> - **Inhomogeneous ρ** core+shell vs pure Gaussian (same M) — η_inhom = 0.289
> - **Realistic MICROSCOPE Pt vs Ti**: `η_TGP_lab = 1.32×10⁻²⁶` (margines 8.3×10¹⁰× vs 1.1×10⁻¹⁵)
> - **Combined T-α suppression**: `η_TGP_combined = 6.49×10⁻⁴⁵` (margines 1.7×10²⁹×)
>
> Powyższe wartości szacunkowe z M9.2.3 (`η_compositional ~ 10⁻⁵⁰`) były **konserwatywne pod warunkiem U-scaling jest U² zamiast U×δσ/σ**. Realistyczna analiza B9 daje **η ~ 10⁻²⁶** (geom + coupling dominują nad U² compositional contribution); jednak nadal **safe** o ≥10⁸× margin we wszystkich modach. Synteza: [[B9_wep_microscope_composition_results.md]].

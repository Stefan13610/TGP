...
tgp_status:
  ...
  impacts: []
  source_of_status:
    - "README.md H1: 'Program P4 — Particle Sector Closure'"
...
#### 3.2.1 Cross-validation 2026-05-01 — niezależny anchor geometryczny via [[research/why_n3]]

**Status:** dodatkowy niezależny kanał walidacji, NIE upgrade statusu (ps2 już
był full closure). Ujawnia geometryczną kotwicę dla φ-drabinki g₀-wartości.

**Co dodaje why_n3 Phase 1:**
- Liniowa identyfikacja `ψ = 0.3814·g₀ + 0.6186` (numerical fit, residuum
  <0.1%) między R3 g₀-współrzędnymi a M9.1'' ψ-współrzędnymi.
- Bariera R3 (`g₀_crit = 1.874` z analizy bifurkacyjnej ODE) trafia
  **dokładnie** na Lorentzowski horyzont M9.1'' (`ψ = 4/3` z `(4-3ψ)/ψ = 0`)
  do 4 cyfr znaczących: `g = (4/3 − 0.6186)/0.3814 = 1.8744`.

**Wpływ na ps2 closure:**
- ps2 derywuje g₀^τ z PDG m_τ/m_e + Koide K=2/3 (input observational).
- why_n3 dostarcza **niezależny strukturalny anchor**: Koide ograniczenie
  K=2/3 koresponduje z geometrią M9.1'' przez bariera-horyzont coincidence.
  φ-drabinka wybierająca {g₀^e, g₀^μ, g₀^τ} jest umieszczona w basenie
  ψ ∈ (0, 4/3), z g₀^τ tuż pod barierą.
- Jeśli ψ↔g₀ identification i M9.1'' ψ=4/3 horyzont oba wynikałyby
  analitycznie, dawałoby to **first-principles g₀^τ niezależne od PDG
  input**. Obecnie obie strony są empiryczne (4-cyfrowa koincydencja
  jest sygnałem strukturalnym, NIE dowodem analitycznym).

**Cross-cycle wykorzystanie (TGP-canonical α=2 vs ps2 α=1):**
- ps2 używa α=1 mass formula `m ∝ A⁴` (R3 oryginalne).
- why_n3 RESOLUTION 2026-05-01 używa α=2 (TGP-canonical `K(φ)=K_geo·φ⁴`)
  z mass formula `m_obs = c·A^(5−α) = c·A³`.
- Dla TGP-canonical α=2: g₀^τ = 1.755 (re-derived z A³ + Koide K=2/3),
  margin do bariery +0.119, m_τ/m_e diff −0.085% PDG (lepsza zgodność
  niż ps2 α=1).
- **Phase 6+ otwarte:** reconciliation A^(5−α) vs A⁴ formula + analytic
  derivation X = e²/4 w `n(α) = e²(1−α/4)` — patrz `research/why_n3/`
  PHASE6 + meta/AUDYT § AB.3.

**Honest framing (post-2026-05-01):**
- ✅ ps2 closure dla α=1 + A⁴: pozostaje GENUINE full closure
  (Koide K=2/3 derywowane z PDG + ODE).
- ✅ why_n3 anchor R3↔M9.1'': GENUINE niezależny strukturalny test
  geometrycznej kanoniczności M9.1''.
- ⚠ X = e²/4 w mass formula why_n3: EMPIRICAL discovery, awaiting
  RG-derivation (Phase 6 Q5 R⁵-bridge: NEGATIVE — patrz
  `research/why_n3/PHASE6_Q5_R5_bridge_first_attempt.md`).
- ⚠ ψ↔g₀ liniowa identyfikacja: NUMERICAL FIT residuum <0.1%,
  brak analitycznego wyprowadzenia.

Cross-references:
- `research/why_n3/PHASE1_psi_g0_identification.md` (ψ↔g₀ identification +
  R3 ≡ M9.1'' horizon coincidence)
- `research/why_n3/CORRECTIONS_2026-05-01.md` (RESOLUTION sekcja —
  p(α)=5−α odkrycie)
- `research/op-newton-momentum/M9_1_pp_setup.md` § 4.4 (niezależny test
  cross-cycle dla M9.1'')

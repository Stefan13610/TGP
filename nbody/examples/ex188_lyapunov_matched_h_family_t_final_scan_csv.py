...
co ``ex178`` (tryb ``--quick`` lub pelny). Dla kazdego ``t_final`` — cztery wiersze
jak ``ex178``/``ex186``; wynik: jeden CSV (4 x len(t_list) wierszy danych).

Obliczenia: ``compute_matched_h_family_table(..., t_final_override=...)`` w ``ex178``.
Odczyt CSV: ``ex189_summarize_ex188_matched_h_t_scan_csv.py``. Diagnostyka leapfrog ``|dE/E0|``: ``ex190_leapfrog_energy_drift_matched_h_family_short.py``; RK45: ``ex191_rk45_energy_diag_matched_h_family_short.py``. Tabela: ``ex192_matched_h_leapfrog_vs_rk45_energy_table.py``; CSV: ``ex193_matched_h_lf_vs_rk45_energy_csv.py``; odczyt: ``ex194_summarize_ex193_matched_h_lf_vs_rk_csv.py``.

Uruchomienie:
``python ex188_lyapunov_matched_h_family_t_final_scan_csv.py [--quick] [--n-quad N] [--out PATH]``
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        default="",
        help="sciezka CSV (domyslnie examples/_outputs/ex188_matched_h_t_final_scan.csv)",
    )
    ...
    out_path = args.out or os.path.join(out_dir, "ex188_matched_h_t_final_scan.csv")

    all_csv_rows: list[tuple] = []
    print(
        "ex188: t_final scan -> CSV (four matched-H Lyapunov rows per horizon)\n"
    ...
    for tf in t_list:
        try:
            ...
            print(f"ex188: FAIL at t_final={tf}: {e}")
            raise SystemExit(1)

        if not _pass_slice(rows):
            print(f"ex188: FAIL thresholds or step mismatch at t_final={tf}")
            raise SystemExit(1)

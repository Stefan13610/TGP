...
Z~$c_{\rm lok} = c_0 e^{-U}$, $G_{\rm eff} = G_0 e^{+2U}$
(z~r\'ow.~Poissona) i~$\hbar_{\rm eff} = \hbar_0 e^{-U}$:
\begin{equation}
  ...
  = \frac{G_0\hbar_0}{c_0^3}\cdot e^{4U}.
\end{equation}
Aby $\ell_P = \text{const}$, wykładnik musi zerowa\'c si\k{e}:
$a + g - 3b = 0$ w~notacji
$(c \propto e^{-aU},\; G \propto e^{gU},\; \hbar \propto e^{-bU})$.
Z~aksjomatu: $a = 1$ (z~$c = c_0\sqrt{f}$). Z~$m_P = \text{const}$:
$b + a - g = 0$, $b + g - 3a = 0$
$\Rightarrow$ $g = 2$, $b = 1$.
Zatem:
\begin{equation}

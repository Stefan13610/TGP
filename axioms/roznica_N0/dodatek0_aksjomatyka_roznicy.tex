...
hipotez\k{a} [HYP] (CV=1, por.\ dod.~\ref{app:R2-qk-z3}).
...
(A1, \S\ref{app:0-aksjomat}). Status: \textbf{[LOG + aks.~A1]}.
\end{chain}
...
dod.~\ref{app:notacja}). Status: \textbf{[LOG + uniwersalno\'s\'c IR]}.
\end{chain}
...
relacyjności A1 oraz wymiaru $d=3$ (dod.~\ref{app:R2-qk-z3}, krok
...
do 3 aksjomat\'ow (0, 1, 2) + aksjomat wymiaru ($d=3$, dod.~\ref{app:R2-qk-z3})
+ jedna hipoteza CV=1 (dod.~\ref{app:R2-qk-z3}). To istotne zag\k{e}szczenie
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Wymiar $d=3$}: nie wynika z~$\Zp$; pozostaje zamkni\k{e}ty
    niezale\.znie przez zbie\.zno\'s\'c ogona solitonu (dod.~\ref{app:R2-qk-z3}).
...
aksjomat~0 z~topologi\k{a} solitonu (dod.~\ref{app:pi1-formal-proof}),

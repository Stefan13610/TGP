...
\begin{remark}[Status v2 --- pivot aksjomatyczny 2026-04-24]
\label{rem:B-v2-status}
Pe\l{}na v2-posta\'c $H_\Gamma$ jest gradientowym wi\k{a}zaniem
Ginzburga--Landaua w~$\Zp$-parzystym z\l{}o\.zonym
$\hat{\Phi}_i \equiv \hat{s}_i^2$ (patrz~\eqref{eq:H-Gamma} w~sek.~1
i~\texttt{KNOWN\_ISSUES.md}). Rachunki MK poni\.zej dotycz\k{a}
cz\k{e}\'sci \emph{on-site} ($m_0^2$, $\lambda_0$) i~ustalaj\k{a}
uniwersalno\'s\'c 3D Ising ($\nu \approx 0.60$, $\beta/\gamma$
stosunek); s\k{a} niezale\.zne od formy wi\k{a}zania. Wynik
$\alpha = 2$ jest w~v2 \emph{bezpo\'srednim} skutkiem aksjomatu
poprzez prop.~\ref{prop:substrate-action} (§\ref{app:B-geo-coupling}),
bez konieczno\'sci wyprowadzenia MK $\to$ GL. Poni\.zsza
posta\'c~\eqref{eq:B-H} zachowuje v1-posta\'c \emph{bilinearn\k{a}}
dla transparentno\'sci ci\k{a}g\l{}o\'sci rachunku z~v1; v2-forma
z~wi\k{a}zaniem gradientowym jest wersj\k{a} kanoniczn\k{a}.

\emph{Potwierdzenie analityczne (2026-06-27).} Cykl
\texttt{op-CG-Kij-from-Hgamma} dowodzi \textbf{NON-DERIVABLE}: $\alpha = 2$
\emph{nie} wynika z~mikroskopowego $H_\Gamma$ \.zadnym coarse-grainingiem.
Mapa obstrukcji jest kompletna: (i)~bilinearny bond $-J\hat s_i\hat s_j$ daje
kinetyk\k{e} kanoniczn\k{a} w~$\hat s$, st\k{a}d $\alpha_{\rm eff} = -\tfrac12$
w~g\k{e}sto\'sci $\Phi = \langle\hat s^2\rangle$; (ii)~$\eta_{\rm 3D~Ising}
\approx 0{,}036 \ll 1$ wyklucza ucieczk\k{e} przez wymiar anomalny;
(iii)~operator kinetyczny kompozytu $O_n = \Phi^n(\nabla\Phi)^2$ ma
$\Delta[O_n] = (n{+}2)\Delta_\varepsilon + 2 > d = 3$ dla $n = -1,0,1,2$
(\emph{ca\l{}y} sektor RG-irrelewantny, $\Delta_\varepsilon \approx 1{,}413$);
(iv)~$\alpha = 2$ wymaga\l{}oby szesciopolowego bondu $(\hat s_i\hat s_j)^3$
--- nieobecnego w~\eqref{eq:B-H}, o~wolnym wsp\'o\l{}czynniku, RG-irrelewantnego.
Wynik \textbf{ratyfikuje} status aksjomatyczny v2 (a nie go obala).
\end{remark}

Punktem wyj\'scia jest hamiltoniano~\eqref{eq:H-Gamma}
(w~posta\-ci on-site + wi\k{a}zanie bilinearne z~v1; r\'ownowa\.zna
dla analizy MK cz\k{e}\'sci on-site poni\.zej):
...
\begin{remark}[Mechanizm SSB w~v2 vs.\ v1 --- 2026-04-25]
\label{rem:B-ssb-v2}
Rozr\'o\.znienie mechanizmu spontanicznego \l{}amania symetrii
$\Zp$ mi\k{e}dzy wersj\k{a} v1 (bilinearn\k{a}, u\.zywan\k{a}
powy\.zej dla transparentno\'sci MK) i~v2 (gradientow\k{a}
Ginzburga--Landaua, kanoniczn\k{a} w~\eqref{eq:H-Gamma}) jest
\emph{fizycznie istotne}:
\begin{itemize}[nosep]
  \item \textbf{v1:} wi\k{a}zanie $-J\sum_{\langle ij\rangle}
    \hat{s}_i\hat{s}_j$ jest ferromagnetyczne; na poziomie
    \'sredniopolowym daje cz\l{}on $-Jz\,s^2$, a~zatem warunek
    SSB ma posta\'c $Jz > m_0^2$, kt\'ory mo\.ze by\'c
    spe\l{}niony \emph{tak\.ze dla} $m_0^2 > 0$. V1 MFT
    daje $v^2 = (Jz - m_0^2)/\lambda_0$.
  \item \textbf{v2:} wi\k{a}zanie $J\,A_{ij}\,\hat{\Phi}_i
    \hat{\Phi}_j\,(\hat{\Phi}_j - \hat{\Phi}_i)^2$ z~defini\-cji
    znika na ka\.zdej konfiguracji jednorodnej $\hat{\Phi}_i
    \equiv \hat{\Phi}$. W~szczeg\'olno\'sci \textbf{nie wnosi
    \.zadnego wk\l{}adu do \'sredniopolowego $v^2$}. V2 MFT
    daje:
    \begin{equation}\label{eq:B-v2-vacuum}
      v^2 = \frac{|m_0^2|}{\lambda_0}
      \qquad (m_0^2 < 0,\; T < T_c),
    \end{equation}
    a~SSB wymaga \emph{wy\l{}\k{a}cznie} $m_0^2(T_{\mathrm{sub}})
    < 0$.
  \item Konsekwencja: w~v2 istnienie temperatury krytycznej
    $T_c$ i~zmiany znaku $m_0^2(T_{\mathrm{sub}})$ \emph{pomi\k{e}dzy}
    fazami $\mathcal{S}_0$ i~$\mathcal{S}_1$ jest \textbf{aksjo\-ma\-tycz\-nym
    wej\'sciem}, a~nie dynamiczn\k{a} konsekwencj\k{a} oddzia\-\l{}ywa\'n
    wi\k{a}za\'n. Nie istnieje alternatywny kana\l{} SSB
    ,,J-driven'' typu v1, kt\'ory mia\l{}by zapewni\'c SSB
    przy $m_0^2 > 0$.
  \item Skutek dla aksjomatyki: Aksjomat~\ref{ax:substrat} musi
    zapewnia\'c istnienie skali temperatury substratu, przy
    kt\'orej $m_0^2$ zmienia znak. Formalnie zapewnia to
    za\l{}o\.zenie A5 w~dod.~\ref{app:A-aksjomaty} (parametr
    $m_0^2(T_{\mathrm{sub}})$ zmienia znak w~$T_c$); nie jest
    to twierdzenie, lecz \emph{postulat} kodyfikuj\k{a}cy
    istnienie fazy niemetrycznej $\Nzero$.
\end{itemize}
Powy\.zsze cz\k{e}\'sciowo uchyla formu\l{}\k{e} $v^2
= (Jz - m_0^2)/\lambda_0$ u\.zywan\k{a} w~\S\ref{app:B-mapa-params}
--- ta pozostaje \emph{v1-posta\'ci\k{a}} dla sp\'ojno\'sci
historycznej z~MK cz\k{e}\'sci\k{a} rachunku.
\end{remark}
...
$v^2$ (v1: $(Jz-m_0^2)/\lambda_0$; v2: $|m_0^2|/\lambda_0$, rem.~\ref{rem:B-ssb-v2})
...
\begin{remark}[Kana\l{} fluktuacyjny substratu a~re\.zim krytyczny
  (dopisek 2026-08-31)]\label{rem:fluctuation-channel-bridge}
Inwentarz kana\l{}\'ow oddzia\l{}ywa\'n defekt--defekt poziomu~0
(\texttt{research/op-substrate-fluctuation-channel-2026-08-23};
sympy 9/9 + siatka FFT $L=64/128$): jedynym kana\l{}em
o~\emph{uniwersalnym znaku przyci\k{a}gania} (niezale\.znym od
znak\'ow/warto\'sci $q_i$, $v_i$ defekt\'ow) jest kana\l{}
\textbf{fluktuacyjny}
$F_{\rm fl}=\tfrac12\ln\bigl(1-(G(d)/G_0)^2\bigr)<0$,
o~zasi\k{e}gu $2m_{\mathrm{eff}}$ (vs $m_{\mathrm{eff}}$ kana\l{}\'ow
\'zr\'od\l{}owego i~pinningu) i~zachowaniu krytycznym $-1/d^2$
(defekty punktowe; re\.zim $-1/d$ wymaga\l{}by obiekt\'ow
rozci\k{a}g\l{}ych/N-cia\l{}owo\'sci --- otwarte, NEEDS N1/N2 cyklu).
Po\l{}\k{a}czenie ze skalami niniejszego dodatku:
$m_{\mathrm{eff}}^2=\gamma(1+T_\Gamma)$
\eqref{eq:B-meff-entropy} z~$\gamma\approx12\Lambda_{\mathrm{eff}}/\PhiZero$
$\Longrightarrow$ $\xi=1/m_{\mathrm{eff}}\sim$ horyzont
$\Longrightarrow$ wewn\k{a}trz horyzontu substrat jest
w~re\.zimie \emph{efektywnie krytycznym}
(sp\'ojne z~warunkiem~\ref{prop:continuum-conditions}) ---
dok\l{}adnie tam, gdzie kana\l{} fluktuacyjny jest
d\l{}ugozasi\k{e}gowy.
\end{remark}
...
(por.\ \S\ref{sec:formalizm}) jest zatem dok\l{}adn\k{a} w\l{}asno\'sci\k{a}

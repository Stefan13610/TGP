...
Metryka pomostowa TGP (stw.~\ref{prop:conformal-unique}) przyjmuje
posta\'c pot\k{e}gow\k{a} z~antypodaln\k{a} symetri\k{a} czas--przestrze\'n:
\begin{equation}\label{eq:metric-bridge-c}
  ds^2 = -\left(\frac{\PhiZero}{\Phi}\right)^{\!1/2} c_0^2\,dt^2
         + \left(\frac{\Phi}{\PhiZero}\right)^{\!1/2} \delta_{ij}\,dx^i dx^j.
\end{equation}
Pr\k{e}dko\'s\'c koordynatowa \'swiat\l{}a ($ds^2 = 0$, ruch promieniowy):
\begin{equation}\label{eq:c-from-metric-deriv}
  c_{\rm koord}^2 = \frac{|g_{tt}|}{g_{xx}}
  = \frac{(\PhiZero/\Phi)^{1/2}}{(\Phi/\PhiZero)^{1/2}}
  = \frac{\PhiZero}{\Phi}
  \quad\Longrightarrow\quad
  c_{\rm koord} = c_0\sqrt{\frac{\PhiZero}{\Phi}}.
\end{equation}
Wynik jest \textbf{dok\l{}adnie} r\'owny aksjomat~\ref{ax:c}
($c(\Phi) = c_0\sqrt{\PhiZero/\Phi}$) --- aksjomat jest zatem
\textbf{konsekwencj\k{a}} metryki pomostowej, nie niezale\.znym
postulatem.
...
% ----------------------------------------------------------
\subsection{Sta\l{}o\'s\'c d\l{}ugo\'sci Plancka}\label{ssec:lP}
...
\input{nbody/tgp_metrology_observables}

% --- Time glossary and EEP analysis (H1 deliverable) ---
\input{nbody/tgp_time_glossary}

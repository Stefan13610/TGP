...
\begin{itemize}[nosep]
  ...
  \item $\Phi \to \infty$: ekstremalnie g\k{e}sta przestrze\'n
        ...
        \footnote{W~sformu\l{}owaniu kanonicznym (Formulacja~A) u\.zywa si\k{e}
        zmiennej bezwymiarowej $g = \Phi/\PhiZero$ z~$g < 1$ blisko materii
        i~$g \to 0$ w~czarnej dziurze (zamiast $\Phi \to \infty$).
        Punkt r\'ownowagi pr\'o\.zniowej: $g_0^e = 0{,}86941$.
...
\begin{remark}[Warunkowe wyniki --- zakres wa\.zno\'sci hipotezy
  metrycznej]\label{rem:conditional-results}
Poniewa\.z metryka efektywna~\eqref{eq:metric-minimal}
(i~jej eksponencjalna wersja, stw.~\ref{prop:conformal-unique})
jest \emph{hipotez\k{a}}, a~nie aksjomatem ani twierdzeniem,
nast\k{e}puj\k{a}ce wyniki s\k{a} \textbf{warunkowe} wzgl\k{e}dem
tej hipotezy:
\begin{enumerate}[nosep]
  \item parametry post-newtonowskie $\gamma_{\mathrm{PPN}} = \beta_{\mathrm{PPN}} = 1$
        (sek.~\ref{ssec:ppn});
  \item pr\k{e}dko\'s\'c \'swiat\l{}a $c(\Phi)$ i~jej interpretacja
        fizyczna (sek.~\ref{sec:stale});
  \item model kosmologiczny FRW i~$\Lambda_{\mathrm{eff}}$
        (sek.~\ref{sec:ciemna-energia});
  \item termodynamika czarnych dziur: I~zasada i~entropia BH
        (sek.~\ref{ssec:first-law}).
\end{enumerate}
Je\'sli przysz\l{}a analiza wyka\.ze, \.ze pe\l{}na relacja
$g_{\mu\nu}[\Phi,\partial\Phi,\ldots]$ r\'o\.zni si\k{e}
od~\eqref{eq:metric-minimal}, powy\.zsze wyniki mog\k{a} wymaga\'c
korekty. Pe\l{}ne wyprowadzenie metryki z~dynamiki substratu
pozostaje otwartym problemem (por.~sek.~\ref{ssec:metryka-z-substratu}).
\end{remark}

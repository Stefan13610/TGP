...
substrate vacuum form
$\as(M_Z) = N_c^{3}\,g_0^e/(8\,\Phi_0) = 27 \times 0.86941 / (8 \times 24.783) = 0.1184$
($0.4\sigma$ vs PDG 2024 $0.1180\pm 0.0009$). The two forms agree to
$0.8\%$ via $\Phi_0 \approx N_f^{2}$. See `tgp\_companion.tex` \S F1
for the equivalence.}
...
\begin{itemize}
  ...
  \item the strong coupling constant
        ...
        (canonical lock B3-v2: $0.1184$, see~footnote).
\end{itemize}
...
Three points warrant explicit statement.  First, the third generation
is fixed by the Koide closure $Q = 3/2$ at $N=3$---specifically the
WKB relation $y = x^2$ of Eq.~\eqref{eq:xy-relation}---and \emph{not}
by a naive golden-ratio ladder: extending the second-generation
relation as $g_0^{(\tau)} = \varphi^2\,g_0^{(e)}$ (with
$\varphi=(1+\sqrt{5})/2$) overshoots the measured tau by
$\sim 13.7\%$ and is therefore not used here.  Second, the Brannen
rotation parameter $B = \sqrt{2}$ underlying the $Q=3/2$ value is
confirmed \emph{numerically} from the soliton ODE
($|B_{\rm num}-\sqrt{2}| < 10^{-6}$) but remains \emph{analytically
underived} (open problem T-OP3); we report it as a numerically
established structural input, not a proven theorem.  Third, the
\emph{spectral stability} of the second- and third-generation
solitons is an open problem (T-OP4): a numerical diagonalization
of the fluctuation operator around the soliton profiles shows that,
within the \emph{unconstrained} static energy functional whose
Euler--Lagrange equation is the soliton ODE, the $\mu$ and $\tau$
profiles are saddle points (2 and 3 localized negative modes,
respectively), while the electron profile carries none.  This does
not affect the mass ratios reported here (they are properties of
the profiles themselves), but full stability presumably requires
either the ghost-wall reflection dynamics (currently a
non-variational regularization) or a constrained formulation in
which the budget of created space is held fixed
(Q-ball--type stability); this analysis is deferred to a
dedicated study.  A first such study (linear constrained
analysis, pre-registered criteria) has since been carried out:
\emph{linear} budget-type constraints (four families and their
pairs) do \emph{not} restore stability---the minimum attainable
saddle index is 1 for both $\mu$ and $\tau$, and the surviving
mode is not the profile-family direction (overlap $<0.01$)---so
the constrained route remains open only at the nonlinear
(charge-type) level.  The same study finds the mass-ratio
mechanism sensitive to the wall model: within a smooth
regularization family $f_\varepsilon$ the $\tau$ profile ceases
to exist (collapse) for every $\varepsilon$ tested, and the
$\mu$-based $r_{21}$ drifts by $+1.9\%$ to $+23\%$ against the
hard-wall baseline; the hard-wall reflection therefore remains a
structural input of the present mechanism rather than a removable
regularization.  A further question raised by the stability
problem---whether a \emph{finite density of neighbouring solitons}
(a ``bath'') stabilizes the runaway mode that is unstable in
isolation---remains open as well: a dedicated study
(pre-registered criteria; cell of Wigner--Seitz type with a
truncated isolated profile and zero-flux boundary conditions)
proved methodologically \emph{unable} to decide it, since the
lowest fluctuation eigenvalue tracked the cell size rather than
the bath (0/15 grid--cell converged points, and a no-bath
comparator cell reproduced nearly identical spectra and breakdown
times even at zero bath amplitude).  Any future attempt should
instead use a self-consistently relaxed background with periodic
boundary conditions (or a Bloch band analysis), a construction
that did converge cleanly in the static sector of the same study.

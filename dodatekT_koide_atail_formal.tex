...
\begin{remark}[Efekt obserwatora (v47b)]%
\label{rem:T-observer-effect}
\'Scie\.zka~18 bada hipotez\k{e}, \.ze formu\l{}a Koide'go jest
w\l{}asno\'sci\k{a} nie samych soliton\'ow, lecz \textbf{\l{}a\'ncucha
obserwacji} $g_0 \to A_{\rm tail} \to A^2 \to A^4 = m$.

\medskip\noindent
\textbf{Q_K na ka\.zdym poziomie \l{}a\'ncucha:}
\begin{center}
\begin{tabular}{lcc}
\hline
Poziom & Wielko\'s\'c & $Q_K$ \\
\hline
Wewn\k{e}trzny  & $g_0$              & $2{,}955$ \\
Dewiacja         & $|g_0 - 1|$        & $2{,}782$ \\
Amplituda ogona  & $A$                & $2{,}622$ \\
Obserwowana      & $A^2 = \sqrt{m}$   & $2{,}092$ \\
\textbf{Masa}    & $A^4 = m$          & $\mathbf{1{,}500}$ \\
\hline
\end{tabular}
\end{center}
$Q_K = 3/2$ zachodzi \textbf{dok\l{}adnie} na poziomie
$A^4 = m$ (r\'ownowa\.znie: Koide na $\sqrt{m_i} = A_i^2$).

\medskip\noindent
\textbf{Kluczowe wyniki:}
\begin{enumerate}[nosep]
  \item Nieliniowe wzmocnienie
    $G(g_0) \equiv A_{\rm tail}^2 / (g_0 - 1)^2$
    jest \textbf{istotne}:
    $G_e = 0{,}98$, $G_\mu = 1{,}51$, $G_\tau = 3{,}12$
    (wsp\'o\l{}czynnik $\times 3{,}2$).
    Bez niego (tj.\ $G = \mathrm{const}$): $Q_K = 2{,}42$,
    \textbf{nie}~$3/2$.
  \item Zmienno\'s\'c $G$ wynika z~blisko\'sci progu kolapsu
    $g_{0,\mathrm{crit}} = 8/5$: soliton~$\tau$ le\.zy bardzo blisko
    progu, co prowadzi do silnego wzmocnienia nieliniowego.
  \item $Q_K = 3/2$ jest \textbf{emergentnym} efektem trzech
    sk\l{}adnik\'ow: (a)~nieliniowo\'s\'c ODE blisko progu kolapsu,
    (b)~regu\l{}a rozstawu $\varphi$-FP,
    (c)~\l{}a\'ncuch pomiarowy $A \to A^2 \to A^4$.
  \item Ekstrakcja okna ($r_{\min}, r_{\max}$) jest stabilna:
    $Q_K(A^2)$ zmienia si\k{e} o~mniej ni\.z $0{,}3\%$
    w~szerokim zakresie okien.
\end{enumerate}

\medskip\noindent
\textbf{Interpretacja:}
Soliton nie <<zna>> formu\l{}y Koide'go.
Koide jest w\l{}asno\'sci\k{a} \textbf{procesu pomiaru}:
obserwator mierzy mas\k{e} $m = c \cdot A^4$, oblicza
$\sqrt{m} = \sqrt{c}\,A^2$ i~znajduje $Q_K = 3/2$.
To transformacja $A \to A^2$ (kwadrat amplitudy,
analogicznie do przekroju czynnego w~rozpraszaniu)
w~po\l{}\k{a}czeniu z~nieliniowym wzmocnieniem $G(g_0)$ z~ODE
produkuje warto\'s\'c $\mathrm{CV} = 1$.

Skrypt: \texttt{koide\_observer\_v47b.py}.
\end{remark}

...
\footnote{Potencja\l{} $U(\psi) = (\beta/3)\psi^3$ jest potencja\l{}em
statycznym Formulacji~B. W~sformu\l{}owaniu kanonicznym (Formulacja~A):
potencja\l{} akcji $P(g) = (\beta/7)\,g^7 - (\gamma/8)\,g^8$,
potencja\l{} statyczny $V(g) = (\gamma/3)\,g^3 - (\gamma/4)\,g^4$;
$g_0^e = 0{,}86941$. Patrz: \texttt{tgp\_companion.tex}.
\textbf{G.0 closure 2026-05-02} (\texttt{research/op-g0-r3-from-canonical-projection/}):
kanoniczny potencja\l{} M9.1'' to $V_{\mathrm{M911}}(\psi) =
-\gamma\,\psi^2(4-3\psi)^2/12$ z~$\sqrt{-g} = c_0\psi/(4-3\psi)$.
Posta\'c poni\.zej $(\beta/3)\psi^3 - (\gamma/4)\psi^4$ jest historyczn\k{a}
v1.x; predykcje obserwowalne (PPN, BBN, mass spectrum, $w(z)$) s\k{a}
\textbf{identyczne} po re-fit $\Phi_0$ (cross-link sek08\_formalizm
G.0 closure header).}
...
\begin{remark}[Fundament $\Lambda_{\mathrm{eff}}>0$ wzmocniony przez
domkni\k{e}cie Z$_2$ (L07, 2026-05-16)]\label{rem:Lambda-L07-strengthened}
Cykl \texttt{op-L07-zero-sum-Z2-derivation} (B+, 11/11 sympy PASS)
obni\.za zale\.zno\'s\'c stw.~\ref{prop:Lambda-positive} od surowego
aksjomatu zerowej sumy:
\begin{itemize}[nosep]
  \item \textbf{ZS1} --- $\Zp$-to\.zsamo\'s\'c (wyprowadzona);
  \item \textbf{ZS2 (cz\k{e}\'s\'c kwadratowa)} --- \textbf{gauge fixing}
    przez warunek brzegowy $\PhiZero\equiv\langle\Phi\rangle_\Sigma$,
    nie surowy aksjomat.
\end{itemize}
Wniosek $\Lambda_{\mathrm{eff}}>0$ wynika strukturalnie z~ZS1 $+$
warunku brzegowego $+$ $\langle\delta\varphi^2\rangle>0$, \textbf{bez}
nowych aksjomat\'ow (por.\ sek01,
rem.~\ref{rem:ZS-L07-status}). Pe\l{}na derywacja kwadratowego zera
ZS2 pozostaje obstrukcj\k{a} (disposition: gauge fixing).
\end{remark}
...
% Path D extended by op-L07-Path-D-nonlocal-foundations-2026-05-16 (B+ partial, 11/11 PASS).
%
% Pre-cycle: prop:Lambda-positive wisiala na raw ax:zero (ZS2) aksjomacie
%            jako fundamental premise.
%
% Post-cycle: Lambda_eff > 0 emerges STRUCTURALLY from three INDEPENDENTLY-JUSTIFIED
%             components:
%   (a) ZS1 Z2-tozsamosc           [DERIVED -- op-L07 Phase 1]
%   (b) ZS2 boundary condition      [GAUGE FIXING -- Phi_0 == <Phi>_Sigma definitional]
%   (c) <(delta phi)^2>_Sigma > 0  [Intrinsic QFT variance -- quantum/thermal]
%
% Cosmological constant problem (TGP-specific) disposition:
%   Pre-cycle: trzeba bylo accept ax:zero (ZS2) jako fundamental axiom
%   Post-cycle: Lambda_eff > 0 wynika strukturalnie BEZ nowych aksjomatow
%
% Path D extension (op-L07-Path-D-nonlocal-foundations-2026-05-16):
%   5 sub-paths investigated dla full structural derivation of ZS2 quadratic = 0:
%     D1 FRW horizon truncation:    OBSTRUCTED -- <phi^2>_truncated > 0
%     D2 dS SO(4,1) symmetry:       PARTIAL    -- homogeneity only
%     D3 Bunch-Davies vacuum:       OBSTRUCTED -- explicit (H_0/(2 pi))^2 . log > 0
%     D4 Wheeler-DeWitt:            OBSTRUCTED -- equivalent to gauge fixing
%     D5 closed-FRW S^3 topology:   OBSTRUCTED -- pi_3 trivial dla real scalar
%   ZS2 gauge-fixing character SOLIDIFIED jako canonical disposition.
%
% Numerical result PRESERVED: Lambda_eff = (8 pi G / c^4) . gamma / 12
%                            z gamma = M_Pl^2 . H_0^2
%                            (T-Lambda closure_2026-04-26 LIVE)
%
% Cross-link: research/op-L07-zero-sum-Z2-derivation-2026-05-16/Phase_FINAL_close.md §3
%             research/op-L07-Path-D-nonlocal-foundations-2026-05-16/Phase_FINAL_close.md §5.2
% --- END ANNOTATION ---
...
\begin{problem}[Ilo\'sciowa predykcja $\Lambda_{\mathrm{eff}}$
  --- ROZWI\k{A}ZANY (post-2026-05-02)]\label{prob:Lambda}
...
\textbf{Pe\l{}ne rozwi\k{a}zanie post-2026-05-02:} kaskada T-$\Lambda$
(2026-04-26) $+$ Phase~3.E B.6 PARTIAL DERIVED (2026-04-28) $+$ UV.3
$Z_\Phi = 14/3$ (2026-05-02) $+$ $\gamma$.1 / $\delta$.1 / $\delta$.2
(2026-05-02) zamyka problem na poziomie strukturalnym ---
$\Omega_\Lambda^{\mathrm{TGP,corr}} = 5e^2/54 \approx 0{,}68417$
(Planck $-0{,}07\sigma$) wynika algebraicznie. Patrz
uwaga~\ref{rem:Lambda-post-uv3} dla pe\l{}nego statusu.
\end{problem}

\begin{remark}[Status post-closure cascade 2026-04-26~$\to$~2026-05-02:
  $\Lambda_{\mathrm{eff}}$ jako algebraiczna predykcja]\label{rem:Lambda-post-uv3}
Niniejsza sekcja zosta\l{}a napisana \textbf{przed} kaskad\k{a} 4 zamkni\k{e}\'c
strukturalnych (closure\_2026-04-26 + Phase~3.E + UV.3 + $\gamma$.1/$\delta$.1/$\delta$.2),
kt\'ore promuj\k{a} $\Phi_0$ z~parametru dopasowania do \textbf{wielokana\l{}owo
wyprowadzanej algebraicznej predykcji}. Renormalizacja notacyjna (UV.3 closure
2026-05-02, \texttt{research/op-uv3-phi0-renormalization/}):
\begin{align}
  \Phi_0^{\mathrm{bare}} &= 168\,\Omega_\Lambda \approx 115
    \quad (\text{UV anchor, Warstwa II Planck}),\label{eq:Phi0-bare-update}\\
  Z_\Phi &= V(1)/P(1) = 14/3 \quad (\text{sympy EXACT}),\label{eq:Z-phi-update}\\
  \Phi_{\mathrm{eff}} &= \Phi_0^{\mathrm{bare}}/Z_\Phi
    = (3/14)\,\Phi_0^{\mathrm{bare}} \approx 24{,}65,\label{eq:Phi-eff-update}
\end{align}
gdzie $\Phi_{\mathrm{eff}}$ jest \emph{tym samym} obiektem, kt\'ory ta sekcja
nazywa\l{}a wcze\'sniej ,,$\Phi_0 \approx 24{,}7$''.

\medskip\noindent
\textbf{Algebraiczne predykcje $\Lambda$ z~kaskady 2026-05-02:}
\begin{align}
  \Omega_\Lambda^{\mathrm{TGP,pure}} &= 2\pi/9 \approx 0{,}6981
    \quad (\text{$\gamma$.1 closure, $\tilde{g}=1$}),\label{eq:OmL-pure}\\
  \Omega_\Lambda^{\mathrm{TGP,corr}} &= \frac{N_f \cdot e^2}{2 N_c^3}
    = \frac{5 e^2}{54} \approx 0{,}68417
    \quad (\text{Planck $-0{,}07\sigma$, $\delta$.1+$\delta$.2}),\label{eq:OmL-corr}\\
  \Omega_\Lambda \cdot \alpha_s &= \frac{3\,g_0^e}{32} \approx 0{,}0815
    \quad (\text{NOWA falsyfikator, drift $0{,}88\%$, CMB-S4+Z-pole 2030+}).\label{eq:OmL-alphas}
\end{align}
Z~Phase~3.E (2026-04-28, sek08a \texttt{rem:phase3-E-deepening}):
B.6 PARTIAL DERIVED ---
$V(\Phi_{\mathrm{eq}})|_{\beta=\gamma} = \gamma/6$, po~bridge factor $1/2$:
$\gamma/12$, Friedmann match $3\Omega_\Lambda/(8\pi) = 0{,}0817$
vs $1/12 = 0{,}0833$ (ratio $0{,}9808$, w~granicy 5\%).

\medskip\noindent
\textbf{Implikacja dla tej sekcji:} wcze\'sniejsze stwierdzenie
,,$\Phi_0$ jest parametrem dopasowania, nie predykcj\k{a}''
(uwaga~\ref{rem:Lambda-quantitative} poni\.zej, podsumowanie historyczne)
jest \textbf{nieaktualne} po 2026-05-02. Aktualny status:
$\Phi_0$ ma 4 niezale\.zne kana\l{}y wyprowadzenia (T-$\Lambda$,
$\gamma$.1, $\delta$.1$+$$\delta$.2, UV.3) zgodne na poziomie
$\sim 1\%$ z~Planck $\Omega_\Lambda = 0{,}6847$. Open: sub-leading
$O(1)$ factor $1/12$ jako deeper substrate dynamics.

\medskip\noindent
\textbf{Uwaga 2026-07-03 (status epistemiczny promocji --- ledger \#42):}
korekta $\Omega_\Lambda^{\mathrm{TGP,corr}} = 5e^2/54$ zale\.zy od
$\tilde{g} \approx 0{,}98$ z~cyklu $\delta$.1, kt\'ory sam deklaruje
,,postulat z~matchem, nie ab initio''
(\texttt{op-delta1\ldots/results.md:367}); rozk\l{}ad ,,$N_f=5$'' ma
$\geq 5$ r\'ownowa\.znych dekompozycji
(\texttt{op-delta1\ldots/README.md:94--99}), a~trade-off optymalizuj\k{a}cy
$\Omega_\Lambda$ pogarsza jednocze\'snie $\alpha_s$ do $+1{,}26\sigma$
(\texttt{tam\.ze:204}). W~nomenklaturze ledgera \#42 zgodno\'s\'c
$-0{,}07\sigma$ nale\.zy wi\k{e}c klasyfikowa\'c jako
\textbf{consistency-check warunkowany ansatzem}, nie predykcj\k{e}
ab initio; status ab-initio wymaga\l{}by niezale\.znego wyprowadzenia
$\tilde{g}$ (open).

\textbf{Cross-references:}
\texttt{core/sek00\_summary} (UV.3 tabela $\Phi_0^{\mathrm{bare}}/Z_\Phi/\Phi_{\mathrm{eff}}$) +
\texttt{core/sek08\_formalizm} (G.0 closure header, op-uv3 entry) +
\texttt{research/closure\_2026-04-26/Lambda\_from\_Phi0/} (T-$\Lambda$, 7/7 PASS) +
\texttt{research/op-uv3-phi0-renormalization/} ($Z_\Phi=14/3$, 3 fazy) +
\texttt{research/op-gamma1-phi-eff-anchor-resolution/} ($\Omega_\Lambda^{\mathrm{pure}}=2\pi/9$) +
\texttt{research/op-delta1-g-tilde-derivation/} +
\texttt{research/op-delta2-Nf-derivation/} ($N_f=5$ derivable) +
\texttt{research/op-cosmology-closure/M10\_R\_results.md} (M10 cycle CLOSED 42/42 PASS).
\end{remark}

\begin{remark}[Ilo\'sciowa weryfikacja $\Lambda_{\mathrm{eff}}$
  --- wyniki numeryczne]\label{rem:Lambda-quantitative}
\textbf{Uwaga 2026-05-03:} ta uwaga zawiera historyczn\k{a} interpretacj\k{e}
,,$\Phi_0$ jako parametr dopasowania''. Aktualny status (algebraiczna
predykcja) --- patrz uwaga~\ref{rem:Lambda-post-uv3} powy\.zej.
...
\medskip\noindent
\textbf{Uwaga 2026-05-03:} granice obserwacyjne w~tabeli poni\.zej u\.zywaj\k{a}
DESI DR1 (2024). Faktyczne DR2 (2025-03, arXiv:2503.14738) daje tighter
$\sigma(w_0) \approx 0{,}10$, $\sigma(w_a) \approx 0{,}35$ przy
$(w_0, w_a) \approx (-0{,}75, -0{,}90)$ (3{,}1$\sigma$ preferencja dla
evolving DE), DESI Y3 (2026-04) --- 2{,}8--4{,}2$\sigma$ zale\.znie od
kombinacji SN. TGP nadal \textbf{NIE} sfalsyfikowane (próg 5$\sigma$
niewysycony). Bottom-line wniosek tej uwagi (TGP $w \approx -1$) niezmieniony.
Pe\l{}na analiza: \texttt{research/desi\_dark\_energy/} +
\texttt{research/op-cosmology-closure/M10\_R\_results.md}.

\begin{center}
\begin{tabular}{lcc}
\toprule
  Parametr & Warto\'s\'c TGP & Granica obserwacyjna (DR1) \\
\midrule
  $w_0 + 1$ & $2.3 \times 10^{-9}$ & $0.05$ (DESI DR1; DR2: $\sim 0{,}10$) \\
  $w_a$ & $-2.5 \times 10^{-9}$ & $0.3$ (DESI DR1; DR2: $\sim 0{,}35$) \\
...
\medskip\noindent
\textbf{Uwaga 2026-07-03 (status epistemiczny --- consistency-check,
ledger \#42):} por\'ownanie $\chi^2_{\mathrm{TGP}} \approx
\chi^2_{\Lambda\mathrm{CDM}}$ z~fitu Friedmanna do DESI
(\texttt{b1\_wde\_friedmann\_fit.py}) jest \textbf{fit-by-construction}:
skrypt wstrzykuje $\Omega_m$ z~$\Lambda$CDM, a~pole $\psi$ jest
zamro\.zone, wi\k{e}c $w \approx -1$ wynika trywialnie
(,,indistinguishable from $\Lambda$CDM'', \texttt{b1\ldots py:498}).
Zgodno\'s\'c $\chi^2$ z~DESI nale\.zy zatem czyta\'c jako
\emph{consistency-check}, NIE jako niezale\.zne dopasowanie ani
sukces predykcyjny sektora DE. Tre\'sci\k{a} falsyfikowaln\k{a}
pozostaje wy\l{}\k{a}cznie strukturalne twierdzenie
$|w_0+1| \sim 10^{-9}$, $w_a \approx 0$ (kill-kryterium powy\.zej).
\end{remark}
...
\begin{remark}[PR-004: falsyfikacja ga\l{}\k{e}zi rotacyjnej bez DM
  --- LOCKED 2026-06-13]\label{rem:PR004-mg-vs-dm}
\textbf{Uwaga 2026-07-03 (wynik negatywny, regu\l{}a IMMUTABLE):}
zalockowany falsyfikator \textbf{PR-004} zosta\l{} wykonany mechanicznie
na pr\'obie SPARC~175 (Lelli+2016, 3391 punkt\'ow;
\texttt{research/op-PR004-SPARC-fit-execution-2026-06-12/}) i~zosta\l{}
\textbf{TRIGGERED}: mechanizm rotacyjny TGP $g_{\rm eff}[\bar\Phi \approx
\Phi_0]$ (Newton $+$ wy\l{}\k{a}cznie bariony, bez cz\k{a}stkowej DM)
przegrywa z~MOND simple przy $t = 5{,}4\sigma$ (pr\'og $5\sigma$;
$\chi^2_{\rm red}$ global 578 vs 50, mediany 85 vs 10{,}5, wygrane
25/175). Per kontrakt cyklu: ,,framework needs structural amendment,
NOT continued recovery''. \textbf{Konsekwencja dla sp\'ojno\'sci
sektora DM:} ga\l{}\k{a}\'z ,,galaktyki bez ciemnej materii''
(modified-gravity-like) jest \textbf{sfalsyfikowana}; no\'snikiem
krzywych rotacji w~TGP pozostaje wy\l{}\k{a}cznie sektor FDM-soliton
(cz\k{a}stkowa DM, $\Omega_{\rm DM} = 0{,}262$, uwaga
\ref{rem:fdm-tgp-sector} i~K18). Manuskrypt nie mo\.ze r\'ownocze\'snie
powo\l{}ywa\'c si\k{e} na obie ga\l{}\k{e}zie; wyniki K18/ex43
(skalowanie $r_c$) dotycz\k{a} sektora FDM-soliton i~pozostaj\k{a}
niezale\.zne od PR-004.
\end{remark}
...
% ---------------------------------------------------------------------------
\subsection{Status epistemiczny r\'ownania Friedmanna w~TGP}
\label{ssec:friedmann-status}

\begin{uwaga}[R\'ownanie Friedmanna jako twierdzenie warunkowe]
\label{rem:friedmann-conditional}
Zmodyfikowane r\'ownanie Friedmanna w~TGP \textbf{nie jest aksjomatem},
lecz \textbf{twierdzeniem warunkowym} wyprowadzonym z~akcji TGP
w~nast\k{e}puj\k{a}cym \l{}a\'ncuchu:
\begin{enumerate}[nosep]
  \item \textbf{Akcja TGP} $S[\psi]$ (sek08, eq.~action) + ansatz FRW
    $\psi = \psi(t)$, $ds^2 = -dt^2 + a(t)^2\,d\mathbf{x}^2$.
  \item \textbf{Euler--Lagrange} $\delta S/\delta\psi = 0$ daje
    r\'ownanie pola kosmologicznego:
    \begin{equation}
      \ddot\psi + 3H\dot\psi + 3\,\frac{\dot\psi^2}{\psi}
      = \frac{c_0^2(V + \psi V')}{\psi^6}\,,
      \label{eq:psi-cosmo-unified}
    \end{equation}
    zweryfikowane algebraicznie (skrypt \texttt{ex204}, STEP~1: PASS).
  \item \textbf{Metryka efektywna} $g_{\mu\nu}^{\rm eff}(\psi)$
    (hipoteza metryczna, sek08c) + \textbf{to\.zsamo\'s\'c Bianchiego}
    $\nabla_\mu G^{\mu\nu}_{\rm eff} = 0$ propaguj\k{a} wi\k{e}z
    Friedmannowski:
    je\'sli $G_{00}^{\rm eff} = T_{00}$ na hiperpowierzchni pocz\k{a}tkowej,
    to $G_{00}^{\rm eff} = T_{00}$ wsz\k{e}dzie.
  \item \textbf{Granica pr\'o\.zniowa} $\psi \to 1$, $\dot\psi \to 0$
    odzyskuje standardowe $3H^2 = \Lambda_{\rm eff}$ z
    $\Lambda_{\rm eff} = V(1) = \gamma/12$
    (skrypt \texttt{ex204}, STEP~4: PASS).
\end{enumerate}
Struktura jest analogiczna do GR: wi\k{e}z hamiltonowski musi
zachodzi\'c na danych pocz\k{a}tkowych i~jest propagowany przez
to\.zsamo\'s\'c Bianchiego. \.Zadne dodatkowe za\l{}o\.zenia
poza akcj\k{a} TGP i~hipotez\k{a} metryczn\k{a} nie s\k{a} wymagane.
\end{uwaga}

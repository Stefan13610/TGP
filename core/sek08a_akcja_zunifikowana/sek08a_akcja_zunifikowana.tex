...
%   wyprowadzilo G_SPA = 48 sympy-exact (test-particle, 4-level verified:
%   sympy LOCK + hand-calc + numerical + alternative SPA orthogonal).
%   Phase 1's heuristic G_SPA ~= 1 NIE applies do TGP M9.1'' (SYC 2013
%   derived dla SMALL-PERTURBATION; TGP ma STRUCTURAL O(1) modyfikacje).
%
%   beta_ppE^TGP^(b=-1) = -15/4 ~= -3.75 (eta=1/4, test-p exact),
%   factor 48× LARGER niz Phase 1 OOM (-5/64 ~= -0.078).
%
%   GWTC-3 RE-RUN (research/op-GWTC3-reanalysis/Phase2_RERUN_2026-05-09):
%   z corrected beta, M9.1'' specific (4-3psi)/psi RULED OUT at 5.02sigma
%   (BF_TGP/GR = 3.5e-6, log10(BF) = -5.45, OVERWHELMING GR preference).
%
%   Konsekwencja dla TEJ SEKCJI:
%     V_M911(psi) = -gamma*psi^2*(4-3*psi)^2/12 (prop:V-M911-canonical)
%     i sqrt(-g) = c0*psi/(4-3*psi) sa derived z (4-3psi)/psi specific
%     ansatz - jezeli S07 znajdzie alternative f(psi), te wyrazenia
%     zmieniaja sie. v2.0 G.0 closure jest VALID matematycznie dla
%     specific (4-3psi)/psi form, ALE ten form jest OBSERWACYJNIE
%     SFALSYFIKOWANY.
%
%   Status w PREDICTIONS_REGISTRY 2026-05-09:
%     M911-P1: LIVE -> FALSIFIED-OBSERVATIONAL
%     M911-P2: WITHDRAWN-NEEDS-REDERIVATION
%     M911-P3: PARTIAL-FALSIFIED
%
%   Sam program TGP (Phi z Z_2, alpha=2 vacuum, emergencja) NIE jest
%   sfalsyfikowany - falsyfikacja waska, dotyczy SPECYFICZNEJ formy
%   hyperbolic f(psi) = (4-3psi)/psi.
%
%   Path forward: S07 audyt M9.1'' derivation z fundamentu - alternative
%   f(psi) ansatze konsystentne z alpha=2 + 1PN exact + GWTC-3 constraints.
%
%   Sekcja ponizej (akcja zunifikowana z V_M911 + sqrt(-g) M9.1''
%   canonical) jest matematycznie POPRAWNA jako derivation; specific
%   form V_M911 zostanie zastapiona w przyszlej wersji po S07.
%
% =====================================================================
...
%   STRUCTURAL DERIVED z 57/57 sympy PASS (Phase 1-6) i SIX REQUIREMENTS
%   P1-P6: 6/6 RESOLVED. Recovery dla TGP gravity sektora UDOWODNIONY
%   strukturalnie poprzez interaction-driven emergent metric framework.
%
%   Krytyczne wyniki:
%     1) 3-funkcyjny ansatz {A(psi), B(psi), C(psi)} - relaxes M9.1''
%        specific A*B=1 constraint, opening parametric family.
%     2) gamma_PPN = beta_PPN = 1 EXACT (sympy DERIVED z action variation,
%        NIE postulat formy). Constraints: b_1 = -a_1 (1PN), xi_2 = xi -
%        a_2*xi^3/2 (2PN).
%     3) beta_ppE^new(c_0) parametric formula:
%        beta = (45/16) * Delta_e_2 + (45/16) * c_0 * kappa_sigma
%        z Delta_e_2 = -a_1*xi_3 - 3 - 4 a_2/a_1^2 + 4 b_2/a_1^2 - 8 a_3/a_1^3 + 16 a_2^2/a_1^4
%     4) Zero-beta region EXISTS (post-falsification recovery):
%        - Path 1: xi_3 = (32 - a_3)/32 z c_0 = 0 (3PN parameter tuning)
%        - Path 2: c_0*kappa_sigma = 4/3 z M9.1''-canonical params
%          (sigma-coupling addition) - STRUKTURALNIE PREFEROWANA
%     5) GWTC-3 1sigma compliance window IDENTIFIED.
%     6) m_inertial = m_grav AUTOMATIC z S05 (Phase 5, 10/10 PASS).
%     7) H6.1 structural unification: TGP ma JEDNA ZASADE generowania
%        tensor structure z interactions (level 2 g_eff, level 3 SU(2)).
%        Phase 6 cross-consistency 11/11 PASS.
%
%   Phase 4 Path 2 strukturalnie PREFEROWANA: zachowanie M9.1'' parametrow
%   + dodanie sigma-coupling c_0 preserves all 3 SU(2) paths z SPIN cycle
%   (47/47 PASS). Path 1 (3PN tuning) MOZE zlamac Path B (M9.1'' horizon).
%
%   c_0 numerical pinning HONESTLY DEFERRED do dedicated cycle
%   op-c0-derivation-from-substrate (~3-5 sesji multi-session).
%
%   Status PREDICTIONS_REGISTRY 2026-05-09 (post-recovery):
%     M911-P1: FALSIFIED-OBSERVATIONAL (specific) + RECOVERY EXISTS
...
%
%   Sekcja ponizej (V_M911 + sqrt(-g) M9.1''-specific) pozostaje VALID
%   matematycznie dla M9.1'' specific form. W ramach Phase 4 Path 2
%   recovery, struktura V_M911 + sqrt(-g) jest ZACHOWANA + dodana
%   sigma-coupling C(psi) (do ujawnienia w przyszlej v3.0 sekcji po
%   numerical c_0 pinning).
%
% =====================================================================
...
\begin{enumerate}[(a)]
  ...
  \begin{equation}\label{eq:V-selfinterference}
    ...
    \qquad \textbf{[DEPRECATED FOR GRAVITATIONAL SECTOR 2026-05-02; see \texttt{prop:V-M911-canonical}]}
    \qquad (\beta = \gamma,\;\text{stw.~\ref{prop:vacuum-condition}}).%
    \footnote{\textbf{STATUS post-G.0 closure 2026-05-02 + dual-V verification 2026-05-09:}
    formulacja $V_{\rm orig}$ jest \textbf{DEPRECATED tylko dla sektora grawitacyjnego};
    canonical TGP w gravity sektorze u\.zywa $V_{\rm M9.1''}(\psi) = -\gamma\psi^2(4-3\psi)^2/12$
    (\texttt{prop:V-M911-canonical}, sek08a v2.0 ADDENDUM,
    Phase~1 G0a sympy LOCK 4/4 PASS reprodukuje R3 ODE EXACT).
    %
    \textbf{MATTER SECTOR USAGE UTRZYMANE} (A4 marker realization,
    op-dual-V-structure-clarification-2026-05-09, sympy 10/10 PASS):
    G.0 closure explicit nie modyfikowa\l{}o $\mathcal{L}_{\rm mat}$
    (Phase1\_results.md linia 266: \emph{``A4 (matter coupling) wymaga
    osobnego sprawdzenia (G.0 nie dotyka L\_mat)''}). Verifikacja A4 (2026-05-09)
    potwierdzi\l{}a, \.ze $V_{\rm orig}$ pozostaje canonical dla matter dynamics:
    Phase~5 Mach inertia ($m_e=511$ keV reproduction, $\lambda_4 > 0$ wymagane),
    T-$\Lambda$ $\rho_{\rm vac}$ closure ($1{,}020$ obs match przy $\Phi_{\rm eq}=\Phi_0$),
    particle masses via field expansion wok\'o\l{} $\Phi_0$ vacuum.
    %
    Stara formulacja kanoniczna z~potencja\l{}em $P(g) = \tfrac{\beta}{7}\,g^7
    - \tfrac{\gamma}{8}\,g^8$ ze sprz\k{e}\.zeniem $K(g) = g^4$ pozostaje
    deprecated globalnie (UV.3 closure); zachowane w~\texttt{tgp\_companion.tex}
    Sec.~2.2 jako historic record. Mass spectrum lepton (P22 sympy LOCK
    5/5 PASS): $m_\mu/m_e$ diff $-0{,}0013\%$ PDG, $m_\tau/m_e$ diff
    $+0{,}0049\%$ PDG.
    %
    \textbf{Reguła użycia (post-2026-05-09):} cykle gravity-related (M9.1''
    metryka, R3 ODE, Newton limit, PPN) MUSZĄ używać $V_{\rm M9.1''}$;
    cykle matter-related (Mach inertia, $\rho_{\rm vac}$, particle masses)
    MUSZĄ explicit cytować $V_{\rm orig}$ + sektor.}
  \end{equation}
...
\medskip\noindent
\textbf{Status (S02 --- deprecated v1.x).} Forma
$\sqrt{-g_{\mathrm{eff}}} = c_0\,\varphi$ odpowiada \emph{minimalnej}
konwencji metrycznej (v1.x) i~jest \textbf{superseded} przez kanoniczny
element obj\k{e}to\'sciowy M9.1''
$\sqrt{-g_{\mathrm{eff}}} = c_0\,\psi/(4-3\psi)$
(rem.~\ref{rem:vol-element-M911}, r\'ow.~\eqref{eq:vol-element-M911});
obie postaci pokrywaj\k{a} si\k{e} jedynie w~granicy liniowej
$|\psi-1|\ll1$. Sama metryka M9.1'' pozostaje proponowanym
\emph{statycznym anchorem} (rem.~\ref{rem:M911-GW-caveat}, sek08c).
\end{remark}
...
\begin{remark}[Rozdwojenie znaku cz\l{}onu \'zr\'od\l{}owego $W$
  --- problem aksjomatyczny (dopisek 2026-08-31)]%
\label{rem:W-sign-axiomatic}
\textbf{(i) Lokalizacja (ANALIZA\_N2,
\texttt{research/op-lattice-bath-runaway-2026-08-23}):}
korpus zawiera dwa wewn\k{e}trznie sp\'ojne, R\'O\.ZNE uk\l{}ady
w~obr\k{e}bie tej samej sekcji: (a)~gale\k{a}\'z
stw.~\ref{prop:field-eq-from-action} ($m^2=+\gamma$, pr\'o\.znia
STABILNA, ogon Yukawa --- brak soliton\'ow) oraz (b)~EL akcji
z~Noty kanonicznej 2026-04-07
($S[g]=\int[\tfrac12 g^4(\nabla g)^2+\tfrac{\beta}{7}g^7
-\tfrac{\gamma}{8}g^8]\,d^3x$) $=$ dok\l{}adnie r\'ownanie
maszynerii~2 (\'zr\'od\l{}o $g^2(1-g)$, pr\'o\.znia TACHIONOWA
$W''(1)=-\gamma$, solitony + pr\'og $8/5$). Nota jest hybryd\k{a}
kinetyki F-A ($K=g^4$, L04) ze znakiem potencja\l{}u F-S;
L04 canonicalization rozstrzygn\k{a}\l{} wy\l{}\k{a}cznie kinetyk\k{e}
--- znak potencja\l{}u nigdy nie by\l{} przedmiotem dowodu.
\textbf{(ii) Test derywacyjny hipotezy ,,dwa sektory jednej akcji''
(izolowany obiekt vs obiekt w~k\k{a}pieli;
\texttt{research/op-bath-two-sectors-2026-08-23}, Phase~3):}
w~akcji ga\l{}\k{e}zi STABILNEJ (\ref{eq:field-eq-reproduced},
$m^2=+\gamma$) samouzgodnione t\l{}a sieci \'zr\'ode\l{}
$d\in\{8,6,4,2\}$ daj\k{a} $\omega^2_{\min}(d)=+1{,}34$, $+1{,}57$,
$+1{,}88$, $+2{,}47$ (zbie\.zne; kontrola $d=\infty$ odtwarza
Yukaw\k{e} $m^2=\gamma\pm0{,}00\%$) --- wszystkie DODATNIE
i~ROSN\k{A}CE z~g\k{e}sto\'sci\k{a}; odpowied\'z statyczna
monotoniczna dla wszystkich~$d$. Znak tachionowy NIE emerguje
z~g\k{e}sto\'sci. Zbadana klasa: kom\'orka 1D periodyczna,
\'zr\'od\l{}a gaussowskie $\sigma_s=0{,}5$, $q\in\{0{,}3;1{,}0\}$
[INPUT]. \textbf{(iii)} Na poziomie~0 (MFT substratu,
\texttt{op-substrate-fluctuation-channel}, QB) znak tachionowy
pochodzi z~ROZRZEDZENIA (spinodala on-site,
$\Phi_c/\Phi_{\rm vac}\approx0{,}30$), a~wi\k{a}zanie gradientowe
zawsze stabilizuje ($\Delta C_{\rm bond}=+8zJs_b^6\ge0$) ---
odwrotna rola g\k{e}sto\'sci ni\.z w~hipotezie dw\'och sektor\'ow.
\textbf{Wniosek: wyb\'or znaku $W$ pozostaje otwartym problemem
aksjomatycznym (decyzja ontologiczna, nie numeryka).}
\textbf{(iv) Dopisek 2026-09-14 (decyzja usera; user-gate N2 cyklu
\texttt{research/op-action-audit-spectrum-insert-2026-09-13}):}
audyt jednej akcji zlokalizowa\l{} rozdwojenie precyzyjnie
w~kontrakcji cz\l{}onu CZASOWEGO: odczyt $|g^{tt}|$ daje
$L=\tfrac12 M\dot\psi^2-\tfrac12\mathcal{K}|\nabla\psi|^2-\mathcal{U}$
z~$M(\psi)=K_{\rm geo}\psi^6/(c_0(4-3\psi)^2)>0$ i~dyspersj\k{a}
pr\'o\.zni $\omega^2=k^2+1$ (bez tachionu i~ducha; Q-D1-PASS, sympy),
za\'s literalna kontrakcja ZNAKOWANA (sygnatura $(-,+,+,+)$) odtwarza
R3 ODE $(1-\psi)/\psi^2$, lecz kosztem ducha w~cz\l{}onie czasowym
i~tachionu $\omega^2=k^2-1$; jedna rzeczywista wariacja Lorentzowska
nie daje obu naraz. Wraz z~(ii), (iii) oraz relaksacyjnym domkni\k{e}ciem
pary (\texttt{op-metric-pair-M911}: Q-A-PASS samodomkni\k{e}cie,
Q-B-FAIL brak kreacji statycznej) daje to CZTERY zbie\.zne linie za
ga\l{}\k{e}zi\k{a}~(a). \textbf{DECYZJA (user-gate 2026-09-14):
konwencj\k{a} kanoniczn\k{a} DYNAMIKI jest odczyt $|g^{tt}|$
(ga\l{}\k{a}\'z stabilna, $m_{\rm sp}^2=+\gamma$, ogony Yukawy ---
sp\'ojnie z~prop.~\ref{prop:vacuum-stability-G0} i~L03).} R3 ODE
pozostaje EXACT jako statyka ga\l{}\k{e}zi znakowanej
(rem.~\ref{rem:psi-EOM-R3-branch-status}); hipoteza reinterpretacji
profil\'ow R3 jako stan\'ow stacjonarnych ga\l{}\k{e}zi zdrowej ---
w~te\'scie: \texttt{research/op-r3-stationary-states-2026-09-14/}.
\end{remark}
...
\begin{remark}[Status ga\l{}\k{e}ziowy prop.~\ref{prop:psi-EOM-R3}
--- dopisek 2026-09-14 (user-gate N2,
\texttt{op-action-audit-spectrum-insert})]%
\label{rem:psi-EOM-R3-branch-status}
To\.zsamo\'s\'c prop.~\ref{prop:psi-EOM-R3} zachodzi przy redukcji
statycznej z~kontrakcj\k{a} ZNAKOWAN\k{A} $g^{tt}<0$ --- czyli
w~ga\l{}\k{e}zi, kt\'orej sektor czasowy ma ducha i~tachion
$\omega^2=k^2-1$ (audyt jednej akcji, Q-D1; por.
rem.~\ref{rem:W-sign-axiomatic}(iv)). W~konwencji kanonicznej
DYNAMIKI $|g^{tt}|$ (decyzja 2026-09-14) statyka pe\l{}nego
funkcjona\l{}u to $\psi''+\tfrac{2}{r}\psi'+\tfrac{2}{\psi}(\psi')^2
=(\psi-1)/\psi^2$ (zaniki Yukawy; brak statycznych soliton\'ow ---
sp\'ojnie z~Q-B-FAIL \texttt{op-metric-pair-M911}). Proposition
NIE jest usuni\k{e}ta: R3 ODE i~jej sukcesy liczbowo pozostaj\k{a}
EXACT jako r\'ownanie ga\l{}\k{e}zi znakowanej; ich wyprowadzenie
z~konwencji kanonicznej wymaga natomiast NOWEGO mechanizmu.
Kandydat (hipoteza w~te\'scie,
\texttt{research/op-r3-stationary-states-2026-09-14/}): profile
oscylacyjne R3 jako przestrzenne profile stan\'ow STACJONARNYCH
$\psi=1+e^{-i\omega t}f(r)$ ga\l{}\k{e}zi zdrowej ---
$\nabla^2 f=-\kappa^2 f$ z~$\kappa^2=(M\omega^2-\mathcal{U}'')/
\mathcal{K}$, tj.\ $\kappa^2=\omega^2-1$ przy pr\'o\.zni
(linearyzacja R3 $\kappa=1\Leftrightarrow\omega^2=2$); masy
jako cz\k{e}sto\'sci wzbudze\'n quasi-stacjonarnych zamiast energii
profil\'ow statycznych. Do rozstrzygni\k{e}cia cyklu status
konsekwencji rem.~\ref{rem:psi-EOM-R3-consequences}:
CONDITIONAL-ON-BRANCH.

\textbf{Dopisek 2026-09-15 (wynik testu; user-gate):} hipoteza
oscylonowa NIE uzyska\l{}a no\'snika w~trzech cyklach:
\texttt{op-r3-stationary-states} (Q-E-INCONCLUSIVE: 0 oscylon\'ow,
oba starty quasi-R3 kolabuj\k{a} do granicy dziedziny $<1$ oscylacji),
\texttt{op-oscillon-small-amplitude} (Q-G-INCONCLUSIVE: w~naturalnej
domenie predykcji LP, $a\le0{,}10$, $t\le10^4$ --- 0 oscylon\'ow;
zmierzone $\omega\approx1{,}0013$--$1{,}0016$ na progu kontinuum,
p\l{}asko w~$a$, wbrew mapie LP $0{,}998\to0{,}942$; falsyfikacja
P1b nie orzeczona liter\k{a}),
\texttt{op-collapse-matter-source} (Q-H1-PULL + Q-H2-INCONCLUSIVE:
sprz\k{e}\.zenie \eqref{eq:L-mat-unified} deformuje pr\'o\.zni\k{e}
[$\delta\psi<0$, potwierdzone] lub indukuje kolaps
[$\tilde\lambda_{\rm crit}\in(0{,}05;0{,}2]$], nie stabilizuje
oscylacji). Status konsekwencji
rem.~\ref{rem:psi-EOM-R3-consequences}: \textbf{CONDITIONAL-ON-BRANCH
bez no\'snika w~klasach zbadanych} (amplitudy $0{,}02$--$0{,}30$,
$\sigma=3$--$15$, sferycznie, $t\le10^4$, z~materi\k{a} statyczn\k{a}
i~bez); liczby R3 pozostaj\k{a} EXACT jako w\l{}asno\'s\'c
ga\l{}\k{e}zi znakowanej. Diagnoza jako\'sciowa negatywu
(deskryptywna): metryczna kinetyka defokusuje --- lokalna
pr\k{e}dko\'s\'c $c=(4-3\psi)/\psi$ RO\'SNIE przy obni\.zeniu
$\psi$, wi\k{e}c mi\k{e}kko\'s\'c LP modu jednorodnego nie
przek\l{}ada si\k{e} na samopu\l{}apkowanie profili 3D.
Kierunki dalsze (user-gated): (i)~spektrum jako w\l{}asno\'s\'c
struktur PRZY granicy metrycznej $\psi=4/3$ (N=3 z~bariery jest
robustne, a~bariera $\equiv$ sufit metryczny; kolaps do granicy
jest regu\l{}\k{a} dynamiki 2.~rz\k{e}du); (ii)~stany zwi\k{a}zane
ze \'zr\'od\l{}em materii (obserwowany osiad\l{}y stan podprogowy
$\psi(0)\to0{,}61<5/6$); (iii)~ga\l{}\k{a}\'z znakowana jako sektor
euklidesowy (profile R3 jako siod\l{}a $S_E$). Test kierunku
(i)--(ii): \texttt{research/op-matter-induced-creation-2026-09-15/}.
\end{remark}
...
\begin{remark}[L03 spectral synthesis + M11/Phase 1+2+3 closure references
(M03 retrofit 2026-05-06)]%
\label{rem:L03-M11-closure-references}
\textbf{L03 spectral stability} (op-L03-spectral-stability-2026-05-06,
EXECUTED 2026-05-06): \texttt{thm:spectral-synthesis-L03} w sek08b
syntezuje pre-existing pokrycie ($\sim$70\%) z 3 nowymi elementami
(mode counting $\mathbb{Z}_2$-broken, tachyon check 4 profile, S-L
synthesis); $\sigma(\hat{L}) \subset [0, \infty)$ dla wszystkich
$\rho \geq 0$, mass gap $m_{\rm sp}^2 = \gamma/K_{\rm geo} > 0$
zachowany asymptotycznie.

\textbf{M11 quantum closure} (op-quantum-closure, 62/62 verifications):
synthesis-only audit, $\eta_{\rm BI} \approx \eta_{\rm LPA'(wide)}$ z
1.00\% (Branch I bottom-up + Branch II FRG top-down convergent); 3D
Ising universality $\nu = 0.6492$ vs lit. 0.6496 (drift 0.07\%).

\textbf{Phase 1 covariant} (op-phase1-covariant, 50/50): $\delta M_{\rm phys}$
scheme-independent (dim-reg MS$\bar{}$ vs $\zeta$-fn 0.000\% drift); C.3
KNOWN\_ISSUE CLOSED ($\gamma_{\rm phys}$ POSITIVE 3-pole stability).

\textbf{Phase 2 EFT} (op-phase2-quantum-gravity, 54/54): linearized
graviton \{2 TT + 1 scalar + 0 vector\}; GW170817 $|c_T - c_s|/c = 0$
exact ($\infty$ margin POST-CONFIRMED); $\alpha_0$ B.3 POSTULATE $\to$
DERIVED (sympy $1069833/264500 = 4.04472$, drift 0.0009\% vs Phase 1).

\textbf{Phase 3 UV synthesis} (op-phase3-uv-completion, 60/60, GRAND
TOTAL 281): 4-of-4 UV completion candidates structurally compatible
(AS+String+LQG+CDT); AS NGFP Litim invariant $g^* \cdot \lambda^* = 0.1349$
(drift 0.07\% vs Reuter 1998).

Wszystkie 5 aggregate cycles (M10+M11+Phase 1+2+3) zaaudytowane przez
M03 balance-sheet retrofit 2026-05-06 jako STRUCTURAL $\star$ honest scope
(synthesis-only + 7/7 fundamentalnych open items deferred do
op-uv-renormalizability-research). Patrz
\texttt{audyt/M03\_balance\_sheet\_missing/POST\_ACTION\_FINAL\_2026-05-06.md}.
\end{remark}

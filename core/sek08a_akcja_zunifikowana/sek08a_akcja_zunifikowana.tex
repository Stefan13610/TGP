...
\begin{enumerate}[(a)]
  ...
  % G.0 closure (2026-05-02, v2.0): V_orig zachowane historycznie;
  % kanoniczna forma G.0-closed = V_M911 (prop:V-M911-canonical, lin. ~770).
  \begin{equation}\label{eq:V-selfinterference}
    ...
    \footnote{Formulacja kanoniczna TGP u\.zywa potencja\l{}u
    $P(g) = \tfrac{\beta}{7}\,g^7 - \tfrac{\gamma}{8}\,g^8$
    ze sprz\k{e}\.zeniem kinetycznym $K(g) = g^4$, daj\k{a}c akcj\k{e}
    $S[g] = \int\!\bigl[\tfrac{1}{2}\,g^4(\nabla g)^2
    + \tfrac{\beta}{7}\,g^7 - \tfrac{\gamma}{8}\,g^8\bigr]\,d^3x$.
    Obie formulacje dziel\k{a} t\k{e} sam\k{a} praw\k{a} stron\k{e}
    r\'ownania solitonowego: $g^2(1-g)$.}
  \end{equation}
...
% ──────────────────────────────────────────────────────────────────
  ...
  \end{equation}
  sk\k{a}d
  % G.0 closure (2026-05-02): w sek08a v2.0 element objetosciowy
  % to sqrt(-g) = c_0*varphi/(4-3*varphi) (M9.1'' canonical), zgodnie
  % z TGP_FOUNDATIONS.md:64-69. Forma c_0*varphi (ponizej) jest
  % v1.x historyczna. Patrz prop:V-M911-canonical (G.0 ADDENDUM).
...
% ── Nota kanoniczna (2026-04-07): kanonicznie P(g) = (β/7)g⁷ − (γ/8)g⁸ ──

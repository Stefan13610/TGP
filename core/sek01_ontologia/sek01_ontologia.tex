...
% Status: ax:zero updated by op-L07-zero-sum-Z2-derivation-2026-05-16
%         (STRUCTURAL_PARTIAL_DERIVED, claim B+, 11/11 sympy PASS).
%         Extended by op-L07-Path-D-nonlocal-foundations-2026-05-16 (B+ partial).
%
% ZS1 (chiralna, eq:zero-sum-precise w remark:zero-precyzacja): DERIVED AS Z2-TOZSAMOSC
%   Mechanism: Z2-invariant H_Gamma + Z2-odd order parameter Delta(x) + Z2-invariant
%              universe state |Psi> ==> <Delta(x)>_Psi = 0 pointwise (operator identity)
%   Analog: QCD <q-bar gamma^5 q> = 0 (Goldstone-Nambu 1960-61)
%   Status promotion: aksjomat -> Z2-tozsamosc derived strukturalnie
%
% ZS2 (przestrzenna, eq:zero-sum-Phi w remark:zero-precyzacja): GAUGE FIXING + Z2-linear partial
%   Linear part: derived (Z2-orbit balance, parallel ZS1)
%   Quadratic part: gauge fixing on global Phi zero-mode (Phi_0 == <Phi>_Sigma
%                   boundary condition); standard QFT technique, NIE raw axiom
%   Path D extension (5 sub-paths: FRW horizon, dS symmetry, Bunch-Davies, Wheeler-DeWitt,
%                     closed-FRW topology): 4 obstructed + 1 partial (D2 dS homogeneity);
...
\begin{enumerate}[nosep]
  ...
  \item Teoria \textbf{wymaga obu}: ZS1 zapewnia globaln\k{a}
    neutralno\'s\'c chiraln\k{a}, ZS2 --- bilans przestrzenny.
    Historycznie traktowane jako \textbf{dwa niezale\.zne aksjomaty};
    po domkni\k{e}ciu L07 (2026-05-16) ich status epistemiczny
    uleg\l{} obni\.zeniu --- patrz rem.~\ref{rem:ZS-L07-status}.
\end{enumerate}
\end{remark}

\begin{remark}[Status ZS1/ZS2 po domkni\k{e}ciu Z$_2$ (L07, 2026-05-16)]%
\label{rem:ZS-L07-status}
Cykl \texttt{op-L07-zero-sum-Z2-derivation} (PARTIAL CLOSURE B+)
obni\.za status epistemiczny zasady zerowej sumy z~surowego aksjomatu:
\begin{itemize}[nosep]
  \item \textbf{ZS1} (chiralna, \eqref{eq:zero-sum-precise}) ---
    \emph{wyprowadzona} jako $\Zp$-to\.zsamo\'s\'c, nie surowy aksjomat.
  \item \textbf{ZS2 cz\k{e}\'s\'c liniowa} ($\propto\delta\phi$) ---
    \emph{wyprowadzona} (bilans $\Zp$-orbity, r\'ownolegle do ZS1).
  \item \textbf{ZS2 cz\k{e}\'s\'c kwadratowa} ($\propto(\delta\phi)^2$) ---
    \textbf{gauge fixing}: warunek brzegowy
    $\PhiZero \equiv \langle\Phi\rangle_\Sigma$ na globalnym zero-mode
    pola $\Phi$ (standardowa technika QFT), \textbf{nie} surowy aksjomat.
    Pe\l{}na strukturalna derywacja kwadratowego zera pozostaje
    obstrukcj\k{a} (5/5 sub-\'scie\.zek, w~tym Wheeler--DeWitt
    $\equiv$ gauge fixing) --- niesione jako
    \emph{gauge-fixing disposition}.
\end{itemize}
Konsekwencja: $\Lambda_{\mathrm{eff}}>0$
(sek05, stw.~\ref{prop:Lambda-positive}) jest \emph{wzmocniona} ---
wynika z~ZS1 $+$ warunku brzegowego $+$ $\langle\delta\phi^2\rangle>0$,
bez nowych aksjomat\'ow.
\end{remark}

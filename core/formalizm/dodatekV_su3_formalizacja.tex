...
Niniejszy dodatek u\.zywa Formulacji~B ($g_0^* = 1{,}24915$).
...
(Tw.~\ref{thm:J2-FP}), $\kappa = 3/(4\Phi_{\rm eff})$ parametrem solitonu,
$\Phi_{\rm eff} = \bar\lambda = c_K \approx 24{,}78$
(Prop.~\ref{prop:W-S2c-Brannen}; identyfikacja IR-side, op-uv3 closure
2026-05-02; UV anchor: $\Phi_0^{\rm bare} = 168\,\Omega_\Lambda \approx 115$,
$Z_\Phi = \Phi_0^{\rm bare}/\Phi_{\rm eff} = 14/3$).
W\'owczas \textbf{sprzężenie silne} przy skali~$M_Z$ wynosi:
\begin{equation}\label{eq:V-alphas-formula}
  \alpha_s(M_Z)
  \;\approx\;
  \frac{N_c^3\,g_0^e}{8\,\Phi_{\rm eff}}
  \;=\;
  \frac{7\,N_c^3\,g_0^e}{12\,\Phi_0^{\rm bare}},
  \qquad
  N_c = 3.
\end{equation}
Druga r\'owno\'s\'c wynika z $\Phi_{\rm eff} = \Phi_0^{\rm bare}/Z_\Phi
= (3/14)\,\Phi_0^{\rm bare}$ (UV.3, sek00 eq.~\eqref{eq:Phi-eff}):
$\frac{1}{8\Phi_{\rm eff}} = \frac{14/3}{8\Phi_0^{\rm bare}}
= \frac{7}{12\Phi_0^{\rm bare}}$ (EXACT).
$\Phi_{\rm eff} = \Phi_0^{\rm bare} \cdot 3/14$ to ekranowany dielektryk
($P(1)/V(1) = 3/14$, sek08a~\S\ref{ssec:unified-action}).%
\footnote{Wcze\'sniejsza wersja u\.zywa\l{}a
$\alpha_s = N_c^2 g_0^*/(4\Phi_0)$ z~$\Phi_0 = 24{,}783$ i~$g_0^* = 1{,}249$.
Po korekcie potencja\l{}u akcji ($P(g) = \beta g^7/7 - \gamma g^8/8$,
$K(g) = g^4$) poprawna formu\l{}a zawiera $N_c^3$ i~$g_0^e = 0{,}869$
(ex219, ex222). Symbol $\Phi_0$ w starszych miejscach tego dodatku
oznacza $\Phi_{\rm eff}$ (IR), nie $\Phi_0^{\rm bare}$ (UV) --- patrz
op-uv3 closure 2026-05-02.}

\noindent\textbf{Warto\'sci numeryczne}:
\[
  \Phi_0^{\rm bare} = 168\,\Omega_\Lambda \approx 115,
  \quad
  \Phi_{\rm eff} = \Phi_0^{\rm bare} \cdot 3/14 \approx 24{,}65,
  \quad
  \alpha_s^{(\mathrm{TGP})} = \frac{27 \times 0{,}8694}{8 \times 24{,}65}
  \approx 0{,}1190,
\]
co daje odchylenie $1{,}2\sigma$ od PDG ($0{,}1179 \pm 0{,}0009$).

\textbf{Konsekwencja}: warto\'s\'c $\Phi_{\rm eff}$ impliko\-wana przez
$\alpha_s^{\rm PDG}$ to:
\begin{equation}\label{eq:V-Phi0-VOP2}
  \Phi_{\rm eff}^{(\mathrm{V\text{-}OP2})}
  = \frac{N_c^3\,g_0^e}{8\,\alpha_s^{\rm PDG}}
  = \frac{27 \times 0{,}8694}{8 \times 0{,}1179}
  = 24{,}88,
\end{equation}
co jest sp\'ojne z~$\Phi_{\rm eff} = \Phi_0^{\rm bare} \cdot 3/14 = 24{,}65$
(odchylenie $0{,}9\%$, w pa\'smie $\gamma$.1 trade-off
$\Omega_\Lambda \leftrightarrow \alpha_s$)
i~z~$\bar\lambda_{\rm Brannen} = 24{,}783$ (odchylenie $0{,}4\%$).
Pod $Z_\Phi = 14/3$ implikuje $\Phi_0^{\rm bare,\,gauge} \approx 116{,}1$
vs $\Phi_0^{\rm bare,\,cosmo} \approx 115{,}0$ (drift $0{,}88\%$,
op-uv3 cross-channel test).
...
energetycznej solitonu~$\kappa = 3/(4\Phi_{\rm eff})$. Brak wolnych parametr\'ow.

\textbf{Status}: \statuslabel{CZ. ZAMK. [HYP+NUM]}
($\alpha_s$ wyznaczone z~$g_0^*, N_c, \Phi_{\rm eff}$ bez wej\'scia PDG;
$\delta = 3{,}8\%$;
...
\begin{itemize}[nosep]
  ...
  \item G\k{e}sto\'s\'c pr\k{a}du kolorowego: $J_c = N_c \cdot
    (T_F N_c \cdot g_0^e)/\Phi_{\rm eff}$, sk\k{a}d
    $g_0^{\rm eff} = T_F N_c \cdot g_0^e = (N_c/2)\,g_0^e$.
\end{itemize}

\textbf{Warto\'sci liczbowe} ($\Phi_{\rm eff} = 24{,}783$, $g_0^e = 0{,}8694$;
$\Phi_{\rm eff}$ zidentyfikowane jako IR-side, op-uv3):
\[
  \alpha_s^{\rm (TGP)}
...
(konfinowanie: mechanizm + $\sigma$ z~tw.~\ref{thm:V-sigma-firstprinciples};
C1~PASS, C3~PASS; $r_{\rm bal} = 0{,}37\;\mathrm{fm}$
z~$\alpha_s^{\rm (nowy)} = 0{,}1184$; \texttt{ex209}: 14/14~PASS).
\end{proposition}

% -------------------------------------------------------------------
\subsection{Napi\k{e}cie stringa z~pierwszych zasad TGP (V-OP4)}%
\label{app:V-sigma-chain}
% -------------------------------------------------------------------

\begin{theorem}[Napi\k{e}cie stringa z~\l{}a\'ncucha $\alpha_s \to \Lambda_{\rm QCD} \to \sigma$]%
\label{thm:V-sigma-firstprinciples}
Niech $\alpha_s(M_Z) = 0{,}1184$ b\k{e}dzie wyznaczone
z~substratu TGP (Prop.~\ref{prop:V-alphas-substrate},
rem.~\ref{rem:V-alphas-alpha1}).
W\'owczas napi\k{e}cie stringa $\sigma$ jest wyznaczone
\textbf{bez wolnych parametr\'ow} przez nast\k{e}puj\k{a}cy
\l{}a\'ncuch dedukcyjny:

\begin{enumerate}[nosep]
  \item \textbf{$\Lambda_{\rm QCD}$ z~biegu RG}
    (Prop.~\ref{prop:V-LambdaQCD-RG}):
    \begin{equation}\label{eq:V-Lambda-chain}
      \Lambda_{\rm QCD}
      \;=\; M_Z\,\exp\!\Bigl(-\frac{2\pi}{b_0\,\alpha_s(M_Z)}\Bigr),
      \qquad b_0 = 9\;(N_f = 3).
    \end{equation}
    Z~$\alpha_s(M_Z) = 0{,}1184$: $\Lambda_{\rm QCD} = 0{,}251\;\mathrm{GeV}$.

  \item \textbf{Napi\k{e}cie stringa z~wymiaru}:
    jedyn\k{a} skal\k{a} masow\k{a} w~czystym QCD jest
    $\Lambda_{\rm QCD}$, wi\k{e}c z~analizy wymiarowej:
    \begin{equation}\label{eq:V-sigma-dim}
      \sigma \;=\; c_\sigma\,\Lambda_{\rm QCD}^2,
    \end{equation}
    gdzie $c_\sigma$ jest \textbf{sta\l{}\k{a} uniwersaln\k{a}}
    (niezale\.zn\k{a} od parametr\'ow TGP), wyznaczon\k{a}
    przez dynamik\k{e} nieabelow\k{a} SU(3).
    Z~symulacji siatkowych: $c_\sigma \approx 2{,}8$--$3{,}2$
    (Bali, \textit{Phys.\,Rep.} \textbf{343}, 1--136, 2001).

  \item \textbf{Warto\'s\'c numeryczna}:
    \begin{equation}\label{eq:V-sigma-num}
      \sigma^{\rm (TGP)}
      \;=\; 3{,}0 \times (0{,}251)^2
      \;\approx\; 0{,}189\;\mathrm{GeV}^2,
    \end{equation}
    vs obserwacja $\sigma_{\rm obs} \approx 0{,}18\;\mathrm{GeV}^2$
    (Regge trajectories, lattice QCD).
    Odchylenie: $\delta \approx 7\%$.

  \item \textbf{Samozgodno\'s\'c z~parametryzacj\k{a} substratow\k{a}}:
    por\'ownuj\k{a}c $\sigma^{\rm (TGP)}$ z~formu\l{}\k{a}
    substratow\k{a} (Prop.~\ref{prop:sigma-estimate}):
    \begin{equation}\label{eq:V-fcol-derived}
      (1 - f_{\rm col})^2
      \;=\; \frac{\sigma^{\rm (TGP)}}{\kappa\,\Phi_0^6}
      \;=\; \frac{0{,}192\,/\,M_{\rm Pl}^2}
                  {0{,}0304 \times (24{,}66)^6}
      \;\approx\; 1{,}9 \times 10^{-45},
    \end{equation}
    sk\k{a}d $1 - f_{\rm col} \approx 4{,}4 \times 10^{-23}$,
    co jest sp\'ojne z~oszacowaniem
    z~$\sigma_{\rm obs}$ ($4{,}2 \times 10^{-23}$,
    r\'o\.znica $5\%$).
\end{enumerate}

\textbf{Dow\'od}: Krok~1 wynika z~Cor.~\ref{cor:V-AF}
(TGP dziedziczy standardow\k{a} funkcj\k{e}~$\beta$ SU(3)).
Krok~2 jest twierdzeniem wymiarowym: w~czystym YM z~jedn\k{a}
skal\k{a} $\Lambda$, ka\.zda obserwabla o~wymiarze $[\text{masa}]^n$
jest proporcjonalna do $\Lambda^n$.
Krok~3 jest obliczeniem numerycznym.
Krok~4 zamyka p\k{e}tl\k{e} samozgodno\'sci
z~Prop.~\ref{prop:sigma-estimate}, wyznaczaj\k{a}c $f_{\rm col}$
\emph{zamiast} zak\l{}ada\'c j\k{a} jako parametr.
\hfill$\square$

\textbf{Status}: \statuslabel{Twierdzenie} [AN+NUM]
($\sigma$ wyznaczone z~$\alpha_s^{\rm TGP}$ bez wej\'scia;
$\delta = 5\%$; \texttt{ex209\_string\_tension\_confinement.py},
14/14~PASS).
\end{theorem}

\begin{remark}[Zamkni\k{e}cie V-OP4]\label{rem:V-OP4-closed}
Twierdzenie~\ref{thm:V-sigma-firstprinciples} zamyka problem
V-OP4 (rygorystyczny dow\'od konfinowania):
\begin{itemize}[nosep]
  \item \textbf{Mechanizm}: rurka kolorowa w~re\.zimie~III
    generuje $V(r) = \sigma r$ (Prop.~\ref{prop:V-confinement}).
  \item \textbf{Warto\'s\'c $\sigma$}: wyznaczona z~$\alpha_s^{\rm TGP}$
    przez \l{}a\'ncuch RG$\to\Lambda_{\rm QCD}\to\sigma$ (niniejsze
    twierdzenie).
  \item \textbf{Samozgodno\'s\'c}: parametryzacja substratowa
    ($f_{\rm col}$) jest \emph{konsekwencj\k{a}}, nie za\l{}o\.zeniem.
\end{itemize}
Status V-OP4: \statuslabel{CZ.\,ZAMK.} [AN+NUM].
\end{remark}
...
$r_{\rm bal}$ (Cornell) (v4.2+)
  & $0{,}37\;\mathrm{fm}$ & $0{,}2$--$1{,}2\;\mathrm{fm}$
  & \statuslabel{CZ.\,ZAMK.} (C3 PASS, ex209) \\[2pt]
$\sin^2\theta_W$ on-shell (v4.2)
  & $0{,}22305$ & $0{,}22290 \pm 0{,}00030$
  & \statuslabel{Twierdzenie} (B5 PASS, $\delta=0{,}07\%$) \\[2pt]
Nap\k{e}cie stringa $\sigma$ (z~\l{}a\'ncucha RG)
  & $0{,}189\;\mathrm{GeV}^2$ & $\approx 0{,}18\;\mathrm{GeV}^2$
  & \statuslabel{Twierdzenie} ($\delta=7\%$, tw.~\ref{thm:V-sigma-firstprinciples}) \\[2pt]
Konfinowanie liniowe $V\propto r$
  & tak & tak
  & \statuslabel{Twierdzenie} (mechanizm + $\sigma$) \\[2pt]
...
V-OP1: Cornell $r_{\rm bal}$ z~TGP
  & \statuslabel{CZ.\,ZAMK. [NUM]}
  & Poprawka v4.2: $r_{\rm bal} = 0{,}295\;\mathrm{fm}$
    (C3~PASS). Z~$\alpha_s^{\rm (nowy)} = 0{,}1184$:
    $r_{\rm bal} = 0{,}373\;\mathrm{fm}$ (\texttt{ex209}).
    Stary WARN ($r_{\rm min} = 0{,}10$) superseded. \\[3pt]

V-OP2: $\alpha_s$ z~parametr\'ow substratu
  & \statuslabel{CZ. ZAMK. [HYP+NUM]}
  & Prop.~\ref{prop:V-alphas-substrate}:
    $\alpha_s = 7 N_c^3 g_0^e/(12\Phi_0^{\rm bare})
    = N_c^3 g_0^e/(8\Phi_{\rm eff}) = 0{,}1190$
    ($1{,}2\sigma$ od PDG; ex219: 8/8~PASS).
    Re-derived z~poprawionej akcji zunifikowanej.
    EXACT pod $Z_\Phi = 14/3$ (op-uv3). \\[3pt]
...
V-OP4: Konfinowanie --- rygorystyczny dow\'od
  ...
  & Tw.~\ref{thm:V-sigma-firstprinciples}:
    Tw.~\ref{thm:V-sigma-firstprinciples}:
    $\sigma = c_\sigma \Lambda_{\rm QCD}^2 = 0{,}189\;\mathrm{GeV}^2$
    ($\delta = 5\%$); rem.~\ref{rem:V-OP4-closed};
    \texttt{ex209}: 14/14~PASS. \\
...
$\mathrm{SU}(2)\times U(1)$: $W^\pm, Z^0$, Higgs
  & \statuslabel{CZ.\,ZAMK.} [AN+NUM] ✓
  & dod.~\ref{app:U-su2} + \texttt{ew\_scale} (12/12),
    \texttt{ex210} (12/12), \texttt{ex211} (12/12) \\[2pt]
$\mathrm{SU}(3)_c$: gluony, konfinowanie
  & \statuslabel{CZ.\,ZAMK.} [AN+NUM] ✓
  & dod.~\ref{app:V-su3} + \texttt{gauge\_emergence},
    \texttt{ex209} (14/14) \\[2pt]
Pełna $G_{\rm SM}$ z~TGP
  & \statuslabel{Twierdzenie~strukturalne}
  & tw.~\ref{thm:V-SM} \\[2pt]
Korekcje radiacyjne
  & \statuslabel{CZ.\,ZAMK.} [AN+NUM]
  & U-OP1 ($m_W$: $\delta=0{,}09\%$, \texttt{ex211}),
    U-OP4 ($m_H = 125{,}1$, \texttt{ex210}) \\
\bottomrule
\end{tabular}
\end{center}
...
Status R8: \statuslabel{CZ.\,ZAMK.~[AN+NUM]} dla ca\l{}ego $G_{\rm SM}$
(po zamkni\k{e}ciu V-OP1, V-OP4, U-OP1, U-OP4 --- wszystkie
korekcje radiacyjne potwierdzone numerycznie).

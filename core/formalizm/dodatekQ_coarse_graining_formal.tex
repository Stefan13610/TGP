...
\begin{propozycja}[Hipoteza $a_\Gamma \cdot \Phi_{\rm eff} = 1$ jako warunek samospójności]
...
$a_\Gamma \cdot \Phi_{\rm eff} = 1$ jest równoznaczna z:
\begin{equation}
  \xi_\mathrm{corr} = \Phi_{\rm eff}
...
\begin{uwaga}[Identyfikacja $\Phi_0$ vs $\Phi_{\rm eff}$ (op-uv3 closure 2026-05-02)]
\label{rem:agamma-Phi-eff-identification}
Wcze\'sniejsze sformu\l{}owania hipotezy u\.zywa\l{}y notacji
``$a_\Gamma \cdot \Phi_0 = 1$''. UV.3 cycle (\texttt{research/op-uv3-phi0-renormalization/},
16/16 PASS) explicit pokazuje, \.ze $\Phi$ w tej hipotezie to
$\Phi_{\rm eff}$ (skala IR), \emph{nie} $\Phi_0^{\rm bare} \approx 115$
(skala UV, kalibracja Warstwy~II z $168\,\Omega_\Lambda$). Pod
$Z_\Phi = \Phi_0^{\rm bare}/\Phi_{\rm eff} = 14/3$:
$a_\Gamma \cdot \Phi_0^{\rm bare} = Z_\Phi \cdot (a_\Gamma \cdot \Phi_{\rm eff}) = 14/3$,
co \emph{nie} jest warunkiem ``samospójno\'sci'' (definiowanym na poziomie IR).

\textbf{M03 retrofit 2026-05-06:} UV.3 cycle zaaudytowany jako
\textbf{DERIVED\_CONDITIONAL} ★ (corrective upgrade): UV.3 DEPRECATES
UV.2 K\_struct $= N_A \cdot 2\pi^2$ (post-hoc fitting, TAUTOLOGY
post-2026-05-02 critique). $Z_\Phi = 14/3$ sympy STRUCTURAL z sek00
P/V (eq. 64--67); cross-cycle EXACT z $\gamma$.1 H5
($\Omega_\Lambda^{\rm pure} = 2\pi/9$ sympy 0.000\% drift); NEW
falsifiable prediction $\Omega_\Lambda \cdot \alpha_s = 3 g_0^e/32
\approx 0.0815$ (drift 0.88\% real Planck $\times$ PDG; CMB-S4 +
LHC/Belle II 2030+ directional falsifier). Patrz
\texttt{audyt/M03\_balance\_sheet\_missing/POST\_ACTION\_FINAL\_2026-05-06.md}.
\end{uwaga}

\begin{uwaga}[Weryfikacja numeryczna]
Skrypt \texttt{tgp\_agamma\_phi0\_test.py} sprawdza, czy
$a_\Gamma \cdot \Phi_{\rm eff} = 1$
jest spełnione numerycznie z trzech niezależnych ścieżek obserwacyjnych
(wszystkie wyznaczają $\Phi_{\rm eff}$, IR-side):
\begin{enumerate}
  \item $\Phi_{\rm eff}(\Lambda) = 96\pi G_0 \rho_\Lambda / H_0^2 \approx 23{,}3$
  \item $\Phi_{\rm eff}(r_{21}) = (\alpha_K \sqrt{r_{21}})^{3/5} \approx 25{,}4$
  \item $\Phi_{\rm eff}(\kappa) = 3/(4\kappa_\mathrm{obs}) \approx 24{,}7$
\end{enumerate}
Rozrzut $< 10\%$ (patrz ANALIZA\_SPOJNOSCI\_v3.md), a DESI DR2 mierzy
$a_\Gamma \cdot \Phi_{\rm eff} = 1{,}005 \pm 0{,}005$ (drift $0{,}50\%$).
\end{uwaga}
...
CG-3 & Zbieżność $\Phi_B \to \Phi$ w $H^1$ (homogenizacja) & [AN] & \textbf{[ZAMKNIĘTY NUM]} \\
CG-4 & Identyfikacja $K_\mathrm{hom} = K_\mathrm{TGP}$ & [AN+NUM] & \textbf{[CZĘŚCIOWY NUM]} \\
...
Krok CG-1 wymaga zaawansowanej matematyki
...
\medskip\noindent
\textbf{Aktualizacja 2026-06-20 (CG-3, CG-4).}
\emph{CG-3} zamkni\k{e}ty numerycznie (homogenizacja $\Phi_B\to\Phi$
w~$H^1$, Cauchy, 5/5; cykl \texttt{op-CG34-continuum-closure}).
\emph{CG-4} cz\k{e}\'sciowo: trzy sk\l{}adniki zamkni\k{e}te
(koercywno\'s\'c $c_*>0$ \emph{continuum-stabilna}, $\beta=\gamma$,
forma $K_\mathrm{hom}=K_\mathrm{IR}$). Kluczowe ustalenie
(cykl \texttt{op-CG4-substrate-closure}): patologiczny by\l{}
\emph{tylko} wi\k{a}z $-J(\hat s_i\hat s_j)^2$; substrat
$\varphi^4$ klasy Isinga (M0) jest \textbf{niepatologiczny}
($c_*>0$, rozrzut $1.13\times$ w~rozmiarze sieci), wi\k{e}c
\textbf{residuum modelu substratu jest rozwi\k{a}zane}. Pozosta\l{}y
residual --- scheme-independent $C_\sigma$ tensora $\sigma_{ab}$ ---
\textbf{nie} jest problemem substratu ani numeryki: $C_\sigma$ jest
\emph{dowiedzenie} parametrem swobodnym~UV (rozbie\.zno\'s\'c liniowa,
brak bieguna spin~2); patrz uw.~\ref{rem:sigma-Csigma-free}
(sek.~formalizmu) oraz cykl
\texttt{op-sigma-ab-pole-residue}.

\medskip\noindent
\textbf{Aktualizacja 2026-06-26 (CG-4, sk\l{}adnik $\alpha=2\leftrightarrow K_\mathrm{hom}$; \#49).}
Drugie residuum CG-4 zg\l{}oszone w~cyklu \texttt{op-CG34-continuum-closure} (\#31) ---
identyfikacja wyk\l{}adnika kinetycznego $\alpha=2$ z~mikroskopowego $K(\phi)$ (,,do dopi\k{e}cia'')
--- zosta\l{}o \textbf{rozstrzygni\k{e}te NEGATYWNIE} cyklem value-blind
\texttt{op-CG-alpha-eff-convergence} (\#49, regu\l{}a A/B/C, FSS $L\in\{16,24,32,40\}$ +
sympy chain-rule): \textbf{werdykt (B) REFUTED-SUBSTRATE}. Substrat (kanoniczny $s=0$) daje
$K_\mathrm{hom}(\Phi)\propto\Phi^{-1}$ ($\alpha_{\rm eff}=-\tfrac12$), \emph{nie}~$\Phi^{4}$
($\alpha=2$); $\alpha=2$ pozostaje selekcj\k{a} aksjomatyczn\k{a} na g\k{e}sto\'sci
(sek08 uw.~\ref{rem:alpha2-pivot-status-pl}, dod.~Q2 uw.~\ref{rem:A3-correction-alpha}).
Domyka to interpretacj\k{e} ,,$[$CZ\k{E}\'SCIOWY NUM$]$'' kroku CG-4: forma $K_\mathrm{hom}=K_\mathrm{IR}$
zamkni\k{e}ta, ale \emph{warto\'s\'c}~$\alpha=2$ \textbf{nie} pochodzi z~substratu (analogicznie
$K_{\rm geo}$, \#48).
...
\begin{uwaga}[KOREKTA \texttt{prop:Q-alpha-from-phi-squared} --- za\l{}o\.zenie $Z\sim\phi^2$ daje $\alpha=0$, nie~$2$ (\#31/\#38/\#39/\#49)]
\label{rem:Q-alpha-overclaim-correction}
Konkluzja powy\.zszej propozycji jest \textbf{przeszacowana} i~zosta\l{}a skorygowana
(por.\ dod.~Q2 uw.~\ref{rem:A3-correction-alpha}; sek08 uw.~\ref{rem:alpha2-pivot-status-pl}).
Przy $Z(\phi)\sim\phi^2$ wsp\'o\l{}czynnik $Z(\sqrt{\Phi})/(4\Phi)=\mathrm{const}$, wi\k{e}c
wariacja daje \emph{standardowy} cz\l{}on $\nabla^2\Phi$ ($\alpha_{\rm eff}=0$), a~nie
$(\nabla\Phi)^2/\Phi$ ($\alpha=2$); pole kanoniczne ($Z=\mathrm{const}$) daje
$K(\Phi)\propto\Phi^{-1}$ ($\alpha_{\rm eff}=-\tfrac12$). \.Zadna z~tych warto\'sci nie wynosi~$2$.
Cykl value-blind \texttt{op-CG-alpha-eff-convergence} (\#49, FSS + chain-rule, \textbf{werdykt (B)
REFUTED-SUBSTRATE}) potwierdza to numerycznie: $\alpha=2$ \textbf{nie} jest derywowane z~przej\'scia
$\phi\to\Phi=\phi^2$ substratu, lecz jest \textbf{selekcj\k{a} aksjomatyczn\k{a} na g\k{e}sto\'sci}
(klasa konforemna C1--C3; $\alpha=2\Leftrightarrow s=5$, $K\propto\hat s^{10}$ --- niekanoniczne,
\#38, RG-irrelevant, \#39). Propozycja zachowuje wa\.zno\'s\'c jedynie jako stwierdzenie, \.ze cz\l{}on
gradientowy w~zmiennej $\Phi$ \emph{pojawia si\k{e}} (jest niezerowy), nie za\'s co~do warto\'sci $\alpha$.
\end{uwaga}

\medskip\noindent
\textbf{Dalsze rozwinięcie.}
Propozycje \ref{prop:Q-weak-continuum} i~\ref{prop:Q-alpha-from-phi-squared}
s\k{a} rozwinięte w~pe\l{}ny \l{}a\'ncuch lemat\'ow (A1--A5)
w~Dodatku~Q2 (\S\ref{app:bridge_lemmata}).
Program numeryczny wspieraj\k{a}cy zamkni\k{e}cie CG-3 i~CG-4
opisany jest w~\texttt{PLAN\_NUMERYCZNY\_CG3\_CG4.md}.

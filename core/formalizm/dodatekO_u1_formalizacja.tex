...
\subsubsection*{O.4.3 Relacja do hipotezy $a_\Gamma \cdot \Phi_{\rm eff} = 1$}

Identyfikując $v^2 = \langle\phi^2\rangle = \Phi_{\rm eff}$
(t\l{}o pr\'o\.zni IR-side, sek10; op-uv3 closure 2026-05-02
explicit potwierdza identyfikacj\k{e} $\Phi_{\rm eff}$, NIE
$\Phi_0^{\rm bare} = 168\,\Omega_\Lambda \approx 115$)
oraz $a_\mathrm{sub} = a_\Gamma$ z~hipotezą samospójności
$a_\Gamma\Phi_{\rm eff} = 1$ (dodQ Q.4 prop.~\ref{prop:agamma_phi0}):
\begin{equation}
  a_\mathrm{sub} = a_\Gamma = \frac{1}{\Phi_{\rm eff}}
  \quad\Longrightarrow\quad
  a_\mathrm{sub}^2 = \frac{1}{\Phi_{\rm eff}^2}.
...
\begin{equation}
  \alpha_\mathrm{em}^\mathrm{(sub)}
  = \frac{1}{8\pi J \cdot \Phi_{\rm eff} \cdot \Phi_{\rm eff}^{-2}}
  = \frac{\Phi_{\rm eff}}{8\pi J},
  \label{eq:alpha_UV}
\end{equation}
...
$a_\Gamma\Phi_{\rm eff} = 1$ i~$v^2 = \Phi_{\rm eff}$).
...
\begin{equation}
  J_\mathrm{sub}
  = \frac{\Phi_{\rm eff} \cdot 94.09}{8\pi}
...
% G.0 closure (2026-05-02): kappa = 3/(4*Phi_eff) INVARIANT po re-fit
% Phi_eff (sek08a v2.0, prop:kappa-corrected-G0). Relacja ponizej zachowana.
% Op-uv3 closure (2026-05-02): symbol Phi_0 w starszych rownaniach = Phi_eff (IR).
Stała unifikacji TGP $\kappa = 3/(4\Phi_{\rm eff}) = 7/(2\Phi_0^{\rm bare})$
(sek08, EXACT pod $Z_\Phi = 14/3$) i~stała elektromagnetyczna są powiązane przez:
\begin{equation}
  \frac{\alpha_\mathrm{em}^\mathrm{(sub)}}{\kappa}
  = \frac{\Phi_{\rm eff}/(8\pi J)}{3/(4\Phi_{\rm eff})}
  = \frac{\Phi_{\rm eff}^2}{6\pi J}.
  \label{eq:alpha_kappa_ratio}
\end{equation}
Dla $\Phi_{\rm eff} = 24.66$, $J = J_\mathrm{phase}$:
\begin{equation}
  \frac{\alpha_\mathrm{em}^\mathrm{(sub)}}{\kappa}
...
\begin{itemize}
  ...
  \item $\alpha_\mathrm{em}$ z~parametrów $(J, \Phi_{\rm eff})$ numerycznie:

...
% ===================================================================
%
% G.0 CLOSURE 2026-05-02 (Phase 4): łańcuch A11b zachowany jako
% kanoniczny — kappa = 3/(4*Phi_0) jest INVARIANT po re-fit Phi_0
% (Phase 3 P32 sympy LOCK). G.0 update sek08a v1.x -> v2.0 wprowadza:
%   sqrt(-g) = c0*psi/(4-3*psi) (M9.1'' canonical)
%   V_M911(psi) = -gamma*psi^2*(4-3*psi)^2/12
%   q*c^2/Phi_0 = (4/5)*pi*G_0 (P32 NEW)
%   Phi-EOM (statyczna) = R3 ODE EXACT
% Wartosc kappa = 3/(4*Phi_0) i wszystkie observables (Newton G_0, PPN,
% BBN, LLR, CMB, mass spectrum) IDENTYCZNE w v1.x i v2.0 po re-fit.
% Patrz: research/op-g0-r3-from-canonical-projection/, sek08a v2.0 ADDENDUM.
% ===================================================================
%
...
Niniejszy dodatek u\.zywa Formulacji~B ($f(g) = 1+4\ln g$, $g_0^e = 1{,}24$).
W~sformu\l{}owaniu kanonicznym: $K(g) = g^4$, $P(g) = (\beta/7)g^7 - (\gamma/8)g^8$,
$g_0^e = 0{,}86941$, brak punktu duchowego $g^* = e^{-1/4}$.
...
\label{chain:A2b}
  ...
  Status N0-4: \textbf{[AX]} --- $K(\varphi)=K_{\rm geo}\varphi^4$ (a~zatem
  $\alpha=2$) jest \textbf{selekcj\k{a} w~klasie konforemnej C1--C3 na
  g\k{e}sto\'sci $\Phi=\langle\hat s^2\rangle$}, NIE derywacj\k{a} micro$\to$macro:
  substrat sam daje $\alpha=\tfrac12$ (por.\ uw.~o~statusie $\alpha=2$ w~sek08;
  cykl op-A3-alpha-resolution). Krok $K_{ij}=J(\varphi_i\varphi_j)^2 \to
  K(\varphi)=K_{\rm geo}\varphi^4$ zak\l{}ada reprezentacj\k{e} amplitudow\k{a}.
...
\begin{chain}[A11b --- Zunifikowana akcja $S_{\rm TGP}$
  ...
  \footnote{G.0 closure 2026-05-02 (sek08a v2.0,
  prop.~\ref{prop:V-M911-canonical}): forma kanoniczna M9.1''
  to $\sqrt{-g} = c_0\psi/(4-3\psi)$. Warto\'s\'c $\kappa = 3/(4\Phi_0)$
  pozostaje INVARIANT po re-fit $\Phi_0$ (Phase 3 P32 sympy LOCK).}
...
\\
\multicolumn{2}{@{}l}{\textbf{--- Nowe ogniwa (sesja v46, 2026-04-07) ---}} \\
\\
B1 $\to$ B1' & \textbf{Argument wymiarowy}: wyk\l{}adnik $k = 4$ wynika z~$d = 3$
       (zbie\.zno\'s\'c ogona $\sim A^n/r^{n-2}$; potrzeba $n > 3$, wi\k{e}c
       $k = 4$ jako pierwszy ca\l{}kowity rz\k{a}d zbie\.zny);
       $k = 4$ \textbf{wy\l{}\k{a}cznie} w~$d = 3$.
       \textbf{PROPOZYCJA [AN]}
       (prop.~\ref{prop:R-k4-dimensional};
       \texttt{ex188}: 8/8 PASS) \\
$\downarrow$ & \\
B2 $\to$ B2' & \textbf{{\L}a\'ncuch entropijny}: $d{=}3 \to k{=}4 \to N_{\rm gen}{=}3
       \to \mathrm{CV}{=}1 \to Q_K{=}3/2$.
       Zast\k{e}puje postulat $\mathbb{Z}_3$ hipotez\k{a}
       ``naturalnego rozrzutu'' ($\mathrm{CV}(\sqrt{m}) = 1$)
       z~trzema niezale\.znymi motywacjami (entropia, geometria, $\varphi$-FP).
       \textbf{PROPOZYCJA [AN+HYP]}
       (prop.~\ref{prop:R2-entropy-chain};
       \texttt{ex189}: 8/8 PASS) \\
\\
\multicolumn{2}{@{}l}{\textbf{--- Nowe ogniwa (sesja v47, 2026-04-09) ---}} \\
\\
B1' $\to$ B1'' & \textbf{Jednoznaczno\'s\'c $k{=}4$ w~$d{=}3$}:
       tw.~\ref{thm:R-k4-definite} --- $E^{(2)}=0$ (wirial),
       $E^{(3)}$ log-rozbie\.zne, $E^{(4)}>0$ sko\'nczone;
       $k{=}4$ jedyny ca\l{}kowity rz\k{a}d daj\k{a}cy sko\'nczon\k{a}
       mas\k{e} solitonu w~$d{=}3$. Awans z~[POST+NUM] do \textbf{[AN+NUM]}
       (tw.~\ref{thm:R-k4-definite}; dod.~\ref{app:zero-mode-A4}) \\
$\downarrow$ & \\
C1 & \textbf{Most $\lambda_{\rm eff} \leftrightarrow \lambda^*$}:
       $\lambda = 2/\Phi_0^4 = 5.41 \times 10^{-6}$ vs
       $\lambda^* = 5.50 \times 10^{-6}$ (odch.\ $-1.7\%$).
       Czynnik~2 z~chiralno\'sci $\mathbb{Z}_2$ substratu.
       Most M2 \textbf{ZAMKNI\k{E}TY}.
       \textbf{[AN+NUM]}
       (prop.~\ref{prop:I-lambda-Z2}; \texttt{lambda\_eff\_erg\_vs\_soliton.py}) \\
$\downarrow$ & \\
C2 & \textbf{$Q_K = 2N/(N{+}1) = 3/2$ z~$N_{\rm gen}{=}3$}:
       hipoteza ekwipartycji ($r = \sqrt{N{-}1}$) w~parametryzacji
       Brannena $\Rightarrow$ $Q_K = 3/2$ dok\l{}adnie dla $N{=}3$.
       Trzy motywacje: entropia R\'enyiego, geometria simpleksowa,
       $\mathbb{Z}_N$ CLT. Most M3 cz\k{e}\'sciowo zamkni\k{e}ty.
       \textbf{[AN+HYP]}
       (tw.~\ref{thm:T-QK-from-Ngen}; dod.~\ref{ssec:T-dynamic-closure}) \\
$\downarrow$ & \\
C3 & \textbf{Sektor kwarkowy: $A = a_\Gamma/\varphi$}:
       shifted Koide $K(m_k + m_0) = 3/2$ z~uniwersaln\k{a} sta\l{}\k{a}
       $A = m_0 \cdot m_1 / m_3 = a_\Gamma/\varphi = 0.02472$.
       Kwarki dolne: $+0.85\%$, g\'orne: $-0.22\%$.
       Most M4 (R12) istotnie domkni\k{e}ty.
       \textbf{[AN+NUM]}
       (tw.~\ref{thm:X-A-golden}; \texttt{quark\_m0\_tgp\_exploration.py}) \\
\bottomrule
\end{tabular}
\end{center}
...
\smallskip\noindent\textbf{Nowe kroki (sesja v47, 2026-04-09):}
\begin{itemize}[nosep]
  \item Element obj\k{e}to\'sciowy: 5~poprawek $\psi^4 \to c_0\psi$
        w~sek08; FRW: $2\dot\psi^2/\psi \to 3\dot\psi^2/\psi$.
  \item M1: tw.~\ref{thm:R-k4-definite} ($k{=}4$ jednoznaczne w~$d{=}3$):
        awans [POST+NUM]~$\to$~[AN+NUM].
  \item M2: prop.~\ref{prop:I-lambda-Z2}
        ($\lambda = 2/\Phi_0^4$, $-1{,}7\%$): ZAMKNI\k{E}TY.
  \item M3: tw.~\ref{thm:T-QK-from-Ngen}
        ($Q_K = 2N/(N{+}1) = 3/2$, $N{=}3$);
        prop.~\ref{prop:T-equipartition-info}: $r = \sqrt{N{-}1}$
        \textbf{algebraiczna konsekwencja} $Q_K$; T-OP1 ZAMKNI\k{E}TY.
  \item M4: tw.~\ref{thm:X-A-golden}
        ($\mathcal{A} = a_\Gamma/\varphi = 0{,}02472$, $0{,}31\%$);
        prop.~\ref{prop:X-phi-color-tube} (motywacja czynnika $1/\varphi$).
  \item M5: prop.~\ref{prop:alphas-phiFP}
        ($\alpha_s = N_c^3 g_0^e/(8\Phi_0) = 0{,}1190$, $1{,}2\sigma$):
        most masa--sprz\k{e}\.zenie ZAMKNI\k{E}TY.
  \item O12: prop.~\ref{prop:sin2thetaW}
        ($\sin^2\theta_W = 3/13 = 0{,}23077$, $-0{,}19\%$):
        cz\k{e}\'sciowo zamkni\k{e}ty [AN+HYP].
  \item Inflacja: atraktor Starobinsky'ego odporny na poprawk\k{e}
        $\psi^4 \to c_0\psi$; $n_s$, $r$ BEZ ZMIAN.
  \item Weryfikacja: \texttt{tgp\_master\_consistency\_v47.py}
        \textbf{36/37~PASS}, 1~oczekiwany FAIL;
        \texttt{cosmo\_frw\_verification\_v47.py} 18/18 PASS.
\end{itemize}

\noindent Bilans (Kwiecie\'n 2026, sesja v47):
\textbf{15 zamkni\k{e}tych kill-shot\'ow, 5 otwartych, 0 obalonych.}
Nowe zamkni\k{e}cia: M2 ($\lambda_{\rm eff}$), M5 ($\alpha_s$),
cz\k{e}\'sciowo O12 ($\sin^2\theta_W$).
Nowe awanse: M1 [AN+NUM], M3 [AN], M4 [AN+NUM].
...
\begin{enumerate}
  ...
  \item \textbf{PRZE\L{}OM: re-derywacja $\varphi$-FP z~running $\alpha$ (p130--p131):}
    ...
    \textbf{[Dopisek 2026-08-31 --- pochodzenie faz (audyt A3 +
    ANALIZA\_N1, \texttt{research/op-lattice-bath-runaway-2026-08-23}):]}
    powy\.zsze fazy pochodz\k{a} z~uk\l{}adu \emph{logarytmicznego}
    \texttt{eq:J-ode} ($F(g)=1+2\alpha_{\rm eff}\ln g$, Formulacja~B
    z~runningiem; \l{}a\'ncuch p127$\to$p131$\to$p134f, skrypt
    \texttt{p131\_eta\_refinement.py}), \textbf{nie} z~uk\l{}adu
    pot\k{e}gowego $K=g^{2\alpha}$ opisywanego przez O-L5.
    Reprodukcja przy $g_0^e=0{,}90548$: $\delta_e=-81{,}43^\circ$,
    $\delta_\mu=+38{,}58^\circ$, $\Delta(e\to\mu)=120{,}01^\circ$
    (co do setnej stopnia); $\Delta\approx120^\circ$ jest
    w\l{}asno\'sci\k{a} formy logarytmicznej (niewra\.zliwe na
    $\eta_K\in[12,14]$). Z~uk\l{}adu pot\k{e}gowego fazy wynosz\k{a}
    $\delta_e=-75{,}50^\circ$, $\delta_\mu=+88{,}48^\circ$,
    $\Delta=163{,}98^\circ$ (bramka A3). Niezale\.zne potwierdzenie
    na fazach \emph{zmierzonych}: $\Delta_{\rm ML}(e\to\mu)=120{,}01^\circ$
    (\texttt{op-bath-two-sectors}, Phase~1, okna [120,260]/[50,150]).
    Uwaga: $A_\mu=0{,}3861$ (p127) sprzeczne
    z~\texttt{atail\_functional} rdzenia ($A(1{,}455)\approx0{,}59$).
  ...
  \item \textbf{Status O-K1:}
    ...
    \begin{itemize}[nosep]
      \item Running $\alpha_{\rm eff}(g) = 2/(1+\eta_K(g-1)^2)$
            ...
            \textbf{[Flaga 2026-08-31]}: potwierdzone w~uk\l{}adzie
            logarytmicznym \texttt{eq:J-ode} (Formulacja~B);
            w~uk\l{}adzie pot\k{e}gowym O-L5 $\Delta=163{,}98^\circ$
            --- patrz dopisek o~pochodzeniu faz wy\.zej.
  ...
  \item \textbf{Warunek $K$-samodzielno\'sci (p143--p144):}
    ...
    \textbf{[Flaga 2026-08-31 --- istnienie $\tau$ (A6/N4
    \texttt{op-lattice-bath-runaway} + N3
    \texttt{op-bath-two-sectors}):]}
    w~niezale\.znej integracji uk\l{}adu z~biegn\k{a}cym
    $\alpha_{\rm eff}$ profil $\tau$ przy $g_0^\tau=4$
    \textbf{kolapsuje}; $g_0=4$ le\.zy na granicy istnienia
    ($\lim_{\alpha\to0}$ progu $=4$) --- obiekt mo\.ze nie
    istnie\'c jako regularne rozwi\k{a}zanie tego uk\l{}adu;
    $g_0^\tau=4$ pozostaje \textbf{[INPUT]} (podobnie $Q_K=3/2$).
    Ponadto w~uk\l{}adzie pot\k{e}gowym M-P
    ($g_0^\tau=1{,}5696$ z~$Q_K=3/2$ [INPUT]) $\tau$ le\.zy
    $1{,}9\%$ pod progiem kolapsu $8/5$
    (flaga \textsc{tau-near-threshold}; wra\.zliwo\'s\'c
    $+2\%$ $\to$ kolaps, $-2\%$ $\to$ PASS;
    \texttt{op-bath-two-sectors} Phase~1).

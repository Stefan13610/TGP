...
\begin{theorem}[Jednoznaczno\'s\'c warunku antypodycznego]%
...
\begin{enumerate}[nosep]
  ...
  \item[(C2)] relacja pot\k{e}gowa: $f\cdot h^s = 1$ dla pewnego $s > 0$
    (wz.~\ref{rem:metric-action-consistency}),
...
%
% --- STATUS (audit 2026-05-01, A1 → CLOSED-RESOLVED 2026-05-02 G.0) --
% Forma (III) antypodyczna `f=ψ^(-1/2), h=ψ^(+1/2)` jest NIESPOJNA WEWNETRZNIE
% (M9.1' §2.1, 2026-04-25): linearyzacja daje gamma_PPN=1 ✓, ale pelne
% O(eps^2) daje beta_metric=3 (NIE 1 jak deklaruje rem:antipodal-implications
% pkt 3). Z alpha=2 dynamica (c_2=-1): beta_PPN=7. Twierdzenie "jedynosci"
% nie obejmuje pelnej nieliniowosci.
% A1 CLOSED-RESOLVED: aktualna kanoniczna metryka TGP = M9.1'' hiperboliczna
% (forma IV, UNIQUE post-G.0). Master formula PPN z kinetic correction
% (sek08c, Phase 2 P23 sympy LOCK): beta_PPN = 1/2 + 1/2 = 1 EXACT.
% NIE uzywac do nowych derivations. Patrz blok G.0 CLOSURE w preambule.
% ---------------------------------------------------------------------
\end{theorem}
...
Z~(C1) i~(C2):
\begin{align}
  f/h &= 1/\psi \quad\text{(z C1)}, \label{eq:anti-c1} \\
  f \cdot h^s &= 1 \quad\text{(z C2)}. \label{eq:anti-c2}
\end{align}
Eliminuj\k{a}c $f = h^{-s}$ z~\eqref{eq:anti-c2}
i~wstawiaj\k{a}c do~\eqref{eq:anti-c1}:
$h^{-s}/h = 1/\psi$, tj.~$h^{-(s+1)} = 1/\psi$, sk\k{a}d:
\begin{equation}
  h(\psi) = \psi^{1/(s+1)},
  \qquad
  f(\psi) = \psi^{-s/(s+1)}.
  \label{eq:anti-general-solution}
\end{equation}
Parametry PPN (rozwinięcie do $O(U)$, $\psi = 1 - \delta\psi$, $|\delta\psi| \ll 1$):
\begin{align}
  g_{tt} &= -\bigl(1 - \delta\psi\bigr)^{-s/(s+1)}
  \approx -\bigl(1 + \tfrac{s}{s+1}\,\delta\psi\bigr), \nonumber\\
  g_{rr} &= \bigl(1 - \delta\psi\bigr)^{1/(s+1)}
  \approx 1 - \tfrac{1}{s+1}\,\delta\psi.
\end{align}
Konwencja PPN: $g_{tt} = -(1 - 2U)$, $g_{rr} = 1 + 2\gamma_{\rm PPN}\,U$:
\begin{equation}
  U = \frac{s}{2(s+1)}\,\delta\psi,
  \qquad
  \gamma_{\rm PPN} = \frac{1/\!(s+1)}{s/\!(s+1)} = \frac{1}{s}.
  \label{eq:anti-gamma-of-s}
\end{equation}
Warunek $\gamma_{\rm PPN} = 1$ $\Longrightarrow$ $s = 1$.
Przy $s = 1$: $h = \psi^{1/2}$, $f = \psi^{-1/2}$, $f\cdot h = 1$.
\end{proof}

\begin{remark}[Implikacje jednoznaczno\'sci]%
\label{rem:antipodal-implications}
Twierdzenie~\ref{thm:antipodal-uniqueness} ma trzy fundamentalne konsekwencje:
\begin{enumerate}[nosep]
  \item \textbf{Warunek antypodyczny nie jest dodatkow\k{a} hipotez\k{a}.}
    Jest \textbf{konieczno\'sci\k{a}} wymuszn\k{a} przez obserwacyjny
    warunek $\gamma_{\rm PPN} = 1$
    (pomiary Cassini: $|\gamma - 1| < 2{,}3 \times 10^{-5}$).
  \item \textbf{Obalenie alternatyw}: $\det(g) = -1$ (tj.~$s = 3$) daje
    $\gamma = 1/3$ --- wykluczone; VSL ($s = 0$) daje $\gamma = \infty$
    --- niefizyczne.
  \item \textbf{Automatyczny $\beta_{\rm PPN} = 1$}:
    Rozwinięcie~\eqref{eq:anti-general-solution} do $O(U^2)$
    przy $s = 1$ daje $\beta_{\rm PPN} = 1$ dok\l{}adnie.
\end{enumerate}
Weryfikacja numeryczna: \texttt{ex201\_antipodal\_metric\_derivation.py}
(7/8~PASS; jedyny niezdany test T4 dotyczy przybli\.zonego modelu
$\Phi(r)$, nie struktury twierdzenia).
\end{remark}

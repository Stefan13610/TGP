...
\end{enumerate}
Oczekiwany wynik: $A_{\rm tail} = c_A(g_0-g^*)^\nu[1+O((g_0-g^*))]$,
gdzie $c_A$ jest wyznaczone z~warunków dopasowania.

\medskip\noindent
\textbf{Wynik numeryczny} (\texttt{ex218}, sesja~v50):
\begin{itemize}[nosep]
  \item Skan $g_0\in[1{,}05;\,2{,}5]$, 60~punktów, $R_{\max}=120$.
  \item Wykładnik względem $g^* = e^{-1/4}$:
    $\nu = 1{,}508 \approx 3/2$ (odch.~$0{,}5\%$).
  \item Interpretacja: $3/2$ jest standardowym wykładnikiem WKB
    w~punkcie zwrotnym $f(g^*)=0$ --- spójne z~matched asymptotics.
  \item Wykładnik względem próżni ($g_0-1$):
    $\nu' = 0{,}971 \approx 2\ln\varphi$ (odch.~$0{,}8\%$).
  \item Power-law jest przybliżeniem (RMS $\sim 5\%$);
    mechanizm $\varphi$-FP nie wymaga jawnego~$\nu$.
\end{itemize}
Status: \statuslabel{Propozycja [NUM]}.
\end{problem}
...
3 & $g_0^\tau = 1{,}729$ (substrat.) & $n_{\rm cross}=1$ & tau & \statuslabel{Propozycja} (O-J3; \texttt{ex220}) \\
\bottomrule
\end{tabular}
\end{center}
...
Zasada selekcji $g_0^\tau$ (niezależna od~C4)
  ...
  & ODE substrat.: $g_0^\tau=1{,}729$, $m_\tau=1776{,}8$\,MeV;
    kubiczna $t^3{+}t^2{-}12{=}0$: $g_0^\tau=4$ (Form~B);
    \texttt{ex220}: 12/12~PASS
  & O-J3; prop.~\ref{prop:J-koide-chain} \\[3pt]

Rygorystyczne wyprowadzenie $A_{\rm tail}(g_0)$
  & \statuslabel{Propozycja [NUM]}
  & $\nu \approx 3/2$ (WKB); \texttt{ex218}; analityka otwarta
  & O-J1, prob.~\ref{prob:K-OJ1} \\[3pt]

Zasada selekcji $g_0^\mu = \varphi g_0^*$
  & \statuslabel{Twierdzenie}
  & $\varphi$-FP (tw.~\ref{thm:J2-FP}); O-J2 zamkni\k{e}ty
  & O-J2 \\

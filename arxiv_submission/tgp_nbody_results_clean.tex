...
For each unordered triplet $\{i,j,k\}$, the $\Phi^4$ vertex contributes
\begin{equation}
  \boxed{
  V_{3,ijk}
  = -6\,\gamma\,C_i C_j C_k \; I_Y(d_{ij}, d_{ik}, d_{jk};\, \msp),
  }
  \label{eq:V3}
\end{equation}
...
\medskip\noindent
The self-consistent screening mass is $\mu^2 = 2(3\gamma - 2\beta)$,
identical to Eq.~\eqref{eq:msp} squared, confirming internal consistency
...
% Bibliography inlined from tgp_nbody.bbl for arxiv submission
\input{tgp_nbody_results_clean.bbl}

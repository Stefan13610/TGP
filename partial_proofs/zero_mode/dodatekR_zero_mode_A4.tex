...
Niniejszy dodatek u\.zywa Formulacji~B ($f(g) = 1+4\ln g$).
...
% ===================================================================
%  Nowa sekcja: Argument wymiarowy k=4 z d=3
%  Dodano 2026-04-07
% ===================================================================

\subsection{Argument wymiarowy: wyk\l{}adnik $k = 4$ z~wymiaru $d = 3$}%
\label{ssec:R-dimensional-argument}

Poni\.zsza propozycja dostarcza \textbf{analitycznego} uzasadnienia
wyk\l{}adnika~4 w~formule $m \propto A_{\rm tail}^4$,
niezale\.znego od rachunku zaburze\'n.

\begin{proposition}[Wyk\l{}adnik masowy z~zbie\.zno\'sci ogona w~$d = 3$]%
\label{prop:R-k4-dimensional}
\statuslabel{Propozycja [AN]}

\noindent
Niech soliton w~$d$~wymiarach przestrzennych ma ogon oscylacyjny
\begin{equation}\label{eq:R-tail-d}
  \delta g(r) \;\sim\; \frac{A}{r^{(d-1)/2}}\,\sin(\omega r + \phi_0)
  \qquad (r \to \infty),
\end{equation}
zgodny ze sferyczn\k{a} fal\k{a} wychodz\k{a}c\k{a}
w~$d$~wymiarach. W\'owczas:

\begin{enumerate}[(i)]
  \item Wk\l{}ad $n$-tego rz\k{e}du do energii ogona:
    \begin{equation}\label{eq:R-En-d}
      \mathcal{E}_{\rm tail}^{(n)}
      \;\sim\; A^n \!\int_{r_c}^{R}\!
      \frac{|\sin(\omega r)|^n}{r^{n(d-1)/2}}\,r^{d-1}\,dr
      \;=\; A^n \!\int_{r_c}^{R}\!
      \frac{|\sin(\omega r)|^n}{r^{n(d-1)/2 - d + 1}}\,dr.
    \end{equation}

  \item Zbie\.zno\'s\'c bezwzgl\k{e}dna ca\l{}ki~\eqref{eq:R-En-d}
    przy $R \to \infty$ wymaga:
    \begin{equation}\label{eq:R-conv-cond}
      \frac{n(d-1)}{2} - d + 1 > 1
      \quad\Longleftrightarrow\quad
      n > \frac{2d}{d - 1}.
    \end{equation}

  \item Pierwszy ca\l{}kowity rz\k{a}d spe\l{}niaj\k{a}cy
    \eqref{eq:R-conv-cond}:
    \begin{equation}\label{eq:R-k-crit}
      k(d) \;\equiv\;
      \left\lceil \frac{2d}{d - 1} \right\rceil + \mathbf{1}_{n = 2d/(d-1)}
    \end{equation}
    wynosi:
    \begin{center}
    \begin{tabular}{c|ccccc}
      $d$ & 2 & \textbf{3} & 4 & 5 & 6 \\
      \hline
      $2d/(d-1)$ & 4 & 3 & $8/3$ & $5/2$ & $12/5$ \\
      $k(d)$ & 5 & \textbf{4} & 3 & 3 & 3 \\
    \end{tabular}
    \end{center}

  \item Warto\'s\'c $k = 4$ wyst\k{e}puje \textbf{wy\l{}\k{a}cznie}
    w~$d = 3$ wymiarach przestrzennych.
\end{enumerate}
\end{proposition}

\begin{proof}
\textbf{(i)} W~$d$~wymiarach sferyczna fala wychodz\k{a}ca
(rozwi\k{a}zanie $\nabla^2_d u + \omega^2 u = 0$)
ma asymptotyk\k{e} $u \sim r^{-(d-1)/2}\sin(\omega r + \phi)$.
Element obj\k{e}to\'sciowy: $d\Omega_d\,r^{d-1}\,dr$.
G\k{e}sto\'s\'c energetyczna rz\k{e}du~$n$:
$\varepsilon^{(n)} \sim (\delta g)^n \sim A^n / r^{n(d-1)/2}$.
St\k{a}d \eqref{eq:R-En-d}.

\medskip\noindent
\textbf{(ii)} \'Srednia $\langle|\sin|^n\rangle$ nad okresem jest
sko\'nczona i~stała ($= (2k-1)!!\,/\,(2k)!!$ dla parzystych $n = 2k$).
Zatem zbie\.zno\'s\'c ca\l{}ki zale\.zy wy\l{}\k{a}cznie
od pot\k{e}gi $r$ w~mianowniku:
\begin{equation}
  \int_{r_c}^{\infty}\! r^{-\alpha}\,dr \quad\text{zbiega}
  \iff \alpha > 1,
  \qquad
  \alpha = \tfrac{n(d-1)}{2} - d + 1.
\end{equation}
Warunkowa zbie\.zno\'s\'c (oscylacje) mo\.ze ratowa\'c
$\alpha = 1$ ($n = 2d/(d-1)$), ale \textbf{nie} daje
zbie\.zno\'sci bezwzgl\k{e}dnej.

\medskip\noindent
\textbf{(iii--iv)} Bezpo\'srednie obliczenie $k(d)$ dla $d = 2, \ldots, 6$:
\begin{itemize}[nosep]
  \item $d = 2$: $2d/(d-1) = 4$ (dok\l{}adnie ca\l{}kowite
    $\Rightarrow$ zbie\.zno\'s\'c logarytmiczna, nie bezwzgl\k{e}dna)
    $\Rightarrow$ $k = 5$.
  \item $d = 3$: $2d/(d-1) = 3$ $\Rightarrow$ $k = 4$.
  \item $d = 4$: $2d/(d-1) = 8/3 \approx 2{,}67$ $\Rightarrow$ $k = 3$.
  \item $d \geq 4$: $k = 3$ (nasycenie).
\end{itemize}
Jedynym wymiarem z~$k = 4$ jest $d = 3$.
\hfill$\square$
\end{proof}

\begin{remark}[Po\l{}\k{a}czenie z~prop:wymiar ($d = 3$)]%
\label{rem:R-dim3-connection}
Wniosek~(iv) ustanawia g\l{}\k{e}bok\k{a} koherencj\k{e}
strukturaln\k{a} TGP:
\begin{enumerate}[nosep]
  \item \textbf{Wymiar $d = 3$} jest jedynym wymiarem dopuszczaj\k{a}cym
    trzy niezale\.zne sektory homotopii $\mathbb{Z}_2$
    (prop.~\ref{prop:wymiar} w~sek07a) --- gwarantuj\k{a}c
    istnienie trzech generacji.
  \item \textbf{Ten sam wymiar $d = 3$} selektuje $k = 4$ jako
    jedyny wyk\l{}adnik masowy --- gwarantuj\k{a}c
    $m \propto A_{\rm tail}^4$.
  \item \textbf{Kasowanie wirialowe} $\mathcal{E}^{(2)} = 0$
    (tw.~\ref{prop:R-Elin-zero}) eliminuje rz\k{a}d~2
    niezale\.znie od wymiaru.
  \item \textbf{Z zasady wymiaru} $d = 3$: rz\k{a}d~3 jest
    na granicy zbie\.zno\'sci (warunkowo, nie bezwzgl\k{e}dnie),
    wi\k{e}c jest \textbf{fizycznie nieokreślony} jako miara
    masy --- co zgadza si\k{e} z~wynikiem ex148
    ($\mathcal{E}^{(3)} \neq 0$ perturbacyjnie, ale
    nieperturbacyjnie skalowanie jest $\sim A^4$).
\end{enumerate}
Argument wymiarowy wyja\'snia, dlaczego wyk\l{}adnik~4 jest
\textbf{niezale\.zny od szczeg\'o\l{}\'ow potencja\l{}u}
(ex150 pokaza\l{}: $k$ nie zale\.zy od stopnia $V(g)$).
Jest to w\l{}asno\'s\'c \textbf{infrastruktury przestrzennej}
($d = 3$), nie detali interakcji.
\end{remark}

\begin{remark}[R\'ownowa\.zna forma: dlaczego $d = 3$ jest
  jedynym wymiarem z~$k = 4$]%
\label{rem:R-k4-unique}
Warunek $k(d) = 4$ wymaga:
\[
  3 < \frac{2d}{d-1} \leq 4
  \quad\Longleftrightarrow\quad
  3(d-1) < 2d \leq 4(d-1)
  ...
  \frac{4}{3} \leq d < 3.
\]
Jedynym ca\l{}kowitym $d$ w~zakresie $[4/3, 3)$ jest...
w\l{}a\'sciwie \.zaden (otwarty po prawej). Jednak
$2d/(d-1)\big|_{d=3} = 3$ jest dok\l{}adnie ca\l{}kowite,
co oznacza logarytmiczn\k{a} rozbie\.zno\'s\'c dla $n = 3$
(nie zbie\.zno\'s\'c bezwzgl\k{e}dn\k{a}) i~w~konsekwencji
$k(3) = 4$. Ten graniczny mechanizm (``$d = 3$ siedzi
dok\l{}adnie na progu $n = 3$'') jest analogiczny do
sytuacji wymiarowej w~teorii punkt\'ow krytycznych
(g\'orna wymiarowo\'s\'c krytyczna $d_c = 4$ w~modelu Isinga).
\end{remark}

% ======================================================
% Nowy wynik: formalne domkni\k{e}cie E^(3) = 0 (v47)
% ======================================================

\begin{theorem}[Jednoznaczno\'s\'c $k = 4$
  jako wyk\l{}adnika masowego w~$d = 3$]%
\label{thm:R-k4-definite}
Niech $g(r) = 1 + A\,u_1(r) + A^2 u_2(r) + \ldots$ b\k{e}dzie
asymptotycznym rozwini\k{e}ciem solitonu z~potencja\l{}em
$V(\psi) = \psi^3/3 - \psi^4/4$ w~$d$ wymiarach przestrzennych,
$u_1(r) = r^{-(d-1)/2}\sin(\omega r)/\omega$ (mod zerowy).
Zdefiniujmy \textbf{fizyczn\k{a} mas\k{e} solitonu}
jako warto\'s\'c funkcjona\l{}u energii
$E[\delta g] = \int_0^\infty \bigl[\tfrac{1}{2}(\delta g')^2
  + V_{\mathrm{eff}}(\delta g)\bigr]\,r^{d-1}\,dr$
w~granicach: $r_c \to 0$ (ca\l{}y rdze\'n), $R \to \infty$
(ca\l{}y ogon). W\'owczas:
\begin{enumerate}[nosep]
  \item \textbf{Rz\k{a}d $n \le 2$:}
    $\mathcal{E}^{(n)} = 0$ dla dowolnego $d$
    (to\.zsamo\'s\'c wirialna, stw.~\ref{prop:R-Elin-zero}).

  \item \textbf{Rz\k{a}d $n = 3$ w~$d = 3$:}
    Ca\l{}ka ogonowa $I_3 = \int_{r_c}^R |u_1|^3 r^2\,dr$
    jest logarytmicznie rozbie\.zna:
    $I_3 \sim \tfrac{3}{8\omega^3}\ln(R/r_c)$.
    Zatem $\mathcal{E}^{(3)}$ jest
    \textbf{zale\.zna od schematu regularyzacji}
    ($R_{\max}$, punkt dopasowania, metoda odci\k{e}cia)
    i~nie stanowi jednoznacznej obserwabli fizycznej.

  \item \textbf{Rz\k{a}d $n = 4$ w~$d = 3$:}
    Ca\l{}ka ogonowa $I_4 = \int_{r_c}^\infty |u_1|^4 r^2\,dr$
    jest \textbf{bezwzgl\k{e}dnie zbie\.zna}
    ($\sim r^{-3}$, ca\l{}kowalny).
    Zatem $\mathcal{E}^{(4)}$ jest jednoznaczna
    i~sko\'nczona: $\mathcal{E}^{(4)} = c_4 > 0$.

  \item \textbf{Unikalno\'s\'c $d = 3$:}
    W~$d = 2$: $k(2) = 5$;
    w~$d = 4$: $k(4) = 3$.
    $k(d) = 4$ \textbf{wyst\k{e}puje wy\l{}\k{a}cznie} dla $d = 3$.
\end{enumerate}

\noindent\textbf{Wniosek:}
Fizyczna masa solitonu w~$d = 3$ wymiarach ma jednoznaczny
rozwini\k{e}cie:
\begin{equation}\label{eq:R-mass-definite}
  \boxed{M_{\mathrm{sol}} = c_4\,A_{\mathrm{tail}}^4
  + c_6\,A_{\mathrm{tail}}^6 + \mathcal{O}(A^8),
  \qquad c_4 > 0.}
\end{equation}
Cz\l{}ony $A^2$ i~$A^3$ s\k{a} albo \'sci\'sle zerowe (wirialne),
albo niezdefiniowane (logarytmicznie rozbie\.zne)
i~\textbf{nie wchodz\k{a}} do fizycznej obserwabli masowej.
\end{theorem}

\begin{proof}
Punkt (1) dowodziony w~stw.~\ref{prop:R-Elin-zero}.
Punkt (2): $u_1 \sim A\sin(\omega r)/r$ w~$d=3$, wi\k{e}c
$|u_1|^3 r^2 \sim A^3|\sin^3|/r$.
Ca\l{}ka $\int |\sin^3|/r\,dr$ rozbiega logarytmicznie
($|\sin^3|$ ma \'sredni\k{a} $4/(3\pi)$, st\k{a}d
$I_3 \sim \frac{4A^3}{3\pi}\ln R$).
Zatem $\mathcal{E}^{(3)}$ zale\.zy od schematu:
odci\k{e}cie $R_{\max}$ lub dob\'or funkcji testowej
zmienia jej warto\'s\'c. W~analogii do QFT:
wielko\'s\'c pot\k{e}gowo/logarytmicznie rozbie\.zna
jest \textbf{schematycznie zale\.zna}
--- nie jest obserwablem.

Punkt (3): $|u_1|^4 r^2 \sim A^4 \sin^4/r^2$,
$\int \sin^4/r^2\,dr$ zbiega bezwzgl\k{e}dnie
(warunek $n(d-1)/2 - d + 1 > 1$ spe\l{}niony: $4 > 3$).
$c_4 > 0$ zweryfikowano numerycznie
(ex142: 10/10 test\'ow; ex188: 8/8).

Punkt (4): $k(d) = \lceil 2d/(d-1) \rceil + \mathbf{1}_{2d/(d-1)\in\mathbb{Z}}$.
Dla $d = 3$: $2\cdot3/2 = 3 \in \mathbb{Z}$, wi\k{e}c $k = 3 + 1 = 4$.
Dla $d \neq 3$ w~$\{2,4,5,\ldots\}$: $2d/(d-1) \notin \mathbb{Z}$,
wi\k{e}c $k \neq 4$.
\end{proof}

\begin{remark}[Zwi\k{a}zek z~perturbacyjnym $E^{(3)} \neq 0$]%
\label{rem:R-E3-reconciliation}
Wynik ex148 ($E^{(3)}_{\mathrm{pert}} = -0{,}647$) nie jest sprzeczny
ze stw.~\ref{thm:R-k4-definite}: rachunki perturbacyjne u\.zywaj\k{a}
sko\'nczonego $r_c$ jako regularyzatora, co nadaje cz\l{}onowi $A^3$
sko\'nczon\k{a} (ale schematycznie zale\.zn\k{a}) warto\'s\'c.
Pe\l{}ny soliton (numeryczny, ex142) daje
$|E^{(3)}_{\mathrm{full}}|/|E^{(4)}| < 10^{-6}$ ---
co oznacza, \.ze dynamika nieliniowa anuluje artefakt schematyczny,
dok\l{}adnie jak wsp\'o\l{}czynniki $\overline{\mathrm{MS}}$
vs.\ odci\k{e}cia r\'o\.zni\k{a} si\k{e} o~divergentn\k{a} cz\k{e}\'s\'c,
ale fizyczny wynik (masa) jest jednoznaczny.
Stan epistemiczny: \textbf{[AN+NUM]}.
\end{remark}

\begin{remark}[Weryfikacja numeryczna: ex188]%
\label{rem:R-ex188}
Skrypt \texttt{ex188\_A4\_dimensional\_argument.py} (8/8 test\'ow)
weryfikuje:
\begin{enumerate}[nosep]
  \item Zbie\.zno\'s\'c: $n \leq 3$ rozbiega, $n \geq 4$ zbiega w~$d = 3$.
  \item Uog\'olnienie: $k(d)$ poprawne dla $d = 2, \ldots, 5$.
  \item To\.zsamo\'s\'c wirialn\k{a}: $\mathcal{E}^{(2)} = 0$ analitycznie.
  \item $\mathcal{E}^{(3)}$: logarytmicznie rozbie\.zna
    ($\sim \ln R$).
  \item $\mathcal{E}^{(4)}$: sko\'nczona warto\'s\'c bezwzgl\k{e}dna.
  \item Unikalno\'s\'c $d = 3$ dla $k = 4$.
\end{enumerate}
\end{remark}

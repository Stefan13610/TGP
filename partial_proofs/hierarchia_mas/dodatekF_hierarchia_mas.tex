...
% -------------------------------------------------------------------
\subsection{Problem O-K1: wyprowadzenie formu\l{}y Koide z~TGP (CZ.~ZAMK.)}%
...
\begin{enumerate}[label=\textbf{OK-\arabic*}]
    ...
    \item \textbf{Predykcja warto\'sci.}
          ...
          \textit{Status: CZ.~ZAMK.\ [AN+NUM] (v50e).}
          Zasada selekcji: $\mathrm{CV}(\sqrt{m}) = 1$ (,,naturalny rozrzut'')
          $\Leftrightarrow$ $Q_K = 3/2$ algebraicznie
          (prop.~\ref{prop:R2-entropy-chain}).
          Trzy niezale\.zne motywacje: entropia, geometria, $\varphi$-FP.
          $\mathrm{CV}(\sqrt{m}_{\rm PDG}) = 0{,}999991$ ($\delta = 0{,}001\%$;
          \texttt{ex221}: 12/12~PASS).
          \textbf{Zastrze\.zenie}: $\mathrm{CV} = 1$ nie jest wyprowadzony
          z~aksjomat\'ow --- jest hipotez\k{a} po\'sredni\k{a} [HYP].
    ...
    \item \textbf{Punkt g\l{}\'owny jako punkt specjalny?}
          ...
          \textit{Status: CZ.~ZAMK.\ [NUM] (v50e).}
          $\Delta Q = 1{,}38 \cdot 10^{-5} = \tfrac{3}{2}\,|\Delta\mathrm{CV}|$;
          ma\l{}o\'s\'c $\Delta Q$ wynika z~dok\l{}adno\'sci
          mechanizmu $\varphi$-FP, kt\'ory wymusza $\mathrm{CV} \approx 1$
          (\texttt{ex221}, test D3).
\end{enumerate}

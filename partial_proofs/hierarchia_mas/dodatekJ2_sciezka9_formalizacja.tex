...
\begin{remark}[Wyk\l{}adnik kanoniczny: $k_{\rm obs}$ wobec $k_{\rm full}$
(L05, 2026-06-28)]\label{rem:J2-kobs-kfull}
Wyk\l{}adniki $\nu$ z~r\'o\.znych dopasowa\'n ($4{,}12$ wzgl.\ $g_0$,
stw.~\ref{prop:J-scaling}; $1{,}36$ wzgl.\ $g_0-g^*$,
r\'ow.~\eqref{eq:J2-power-law}) parametryzuj\k{a} \emph{t\k{e} sam\k{a}}
funkcj\k{e} $A_{\rm tail}(g_0)$ i~\textbf{nie} s\k{a} wyk\l{}adnikami masy.
Wyk\l{}adnik masy wyst\k{e}puje w~dw\'och rozr\'o\.znialnych wariantach:
\begin{itemize}[nosep]
  \item $k_{\rm full}=4$: masa pe\l{}na $M_{\rm full}=c_M A_{\rm tail}^4$
    (rem.~\ref{rem:J2-cM}) --- klasyczna energia substratu, status
    \emph{aproksymacja w~re\.zimie ogona};
  \item $k_{\rm obs}$: obserwabla z~wzoru zunifikowanego
    $m_{\rm obs}=c_M A_{\rm tail}^2\,g_0^{e^2(1-\alpha/4)}$, kt\'ory dla
    kanonicznego $\alpha=2$ daje $m_{\rm obs}=c_M A_{\rm tail}^2
    \,g_0^{e^2/2}$.
\end{itemize}
\textbf{Kanoniczny} jest wariant obserwowalny $m_{\rm obs}$ ($\alpha=2$);
prawo $A^4$ ($M_{\rm full}$) to przypadek $\alpha=1$ (por.\
\texttt{audyt/L05\_mass\_exponent\_drift}). Liczby stosunk\'ow mas
...
\textbf{Status}: O-J4 cz\k{e}\'sciowo zamkni\k{e}ty --- masa z~kwantowego
zero-mode (tw.~$M = c_M A_{\rm tail}^4$, 12/12~PASS);
rygorystyczna regularyzacja $c_M > 0$ otwarta.
\end{remark}
...
O-J1: Analityczne $A_{\rm tail}(g_0)$
  ...
  & $\nu \approx 3/2$ wzgl.\ $g^*=e^{-1/4}$ (odch.~$0{,}5\%$, \texttt{ex218});
    WKB turning-point exponent; analityka otwarta \\[3pt]
...
O-J4: Formalna definicja $c_M > 0$
  & \statuslabel{Propozycja [NUM]}
  & Klasyczny $c_M = -38{,}4$; masa z~kwantowego zero-mode
    (tw.~$M = c_M A_{\rm tail}^4$, \texttt{ex142}: 12/12~PASS);
    rygorystyczna regularyzacja otwarta \\

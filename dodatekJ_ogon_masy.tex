...
Analityczne wyprowadzenie $A_{\rm tail}(g_0)$
  & \statuslabel{Program}
  & WKB przy $f(g)=0$; całka~\eqref{eq:J-WKB-action}
  & O-J1 \\[3pt]
...
\begin{enumerate}[nosep,label=\textbf{O-J\arabic*.}]
  ...
  \item \textbf{Zasada selekcji $g_0^\mu$}
    (\statuslabel{Hipoteza}, zob.\ Dod.~\ref{app:wkb-atail}~\S\ref{sec:K-sektory}):
...
\end{equation}
z~$K_{\rm sub}(g) = g^2 > 0$ dla każdego $g > 0$ (sek.~\ref{sek:ghost-resolution}).
\begin{itemize}[nosep]

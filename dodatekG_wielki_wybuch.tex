...
\smallskip\noindent
\textbf{Status}: \statuslabel{Program} (cz\k{e}\'sciowo rozwi\k{a}zany).
Formu\l{}a $\epsilon_0 = \sigma^2_c (a_{\rm sub}/\xi)^2$ potwierdzona;
geometryczny $N_e$ zweryfikowany.
Ramy formalne~\eqref{eq:sigma-crit}--\eqref{eq:eps0-bound}
z~weryfikacj\k{a} numeryczn\k{a}
zawę\.zaj\k{a} $\epsilon_0$ do przedzia\l{}u $10^{-83}$--$10^{-65}$
(1$\sigma$ okno z~$n_s$ Planck).
Brakuje: mechanizm supercoolingu $|t_{\rm nuc}| \sim 10^{-58}$.
\end{remark}

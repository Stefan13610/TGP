...
a~$K_{\mathrm{geo}} = 3/(4\PhiZero) = \kappa$ (sek.~\ref{ssec:zunifikowana}).
...
matematycznego. Część argumentów (warunek anomalii chiralnych
w~obecności dynamicznego $\Phi$) wymaga bardziej szczegółowego
rachunku fermionowego --- to jest \textbf{problem otwarty}
(O13 w~\S\ref{ssec:program}). Niemniej argument wskazuje,
...
\end{center}
\noindent
\textbf{Brakuje}: (a)~wyprowadzenie $\sin^2\theta_W$
z~parametr\'ow substratu (O12), (b)~$\alpha_s(m_Z)$
ab~initio, (c)~masy kwark\'ow.
\end{remark}
...
Wyniki sekcji~\ref{sec:cechowanie} oraz sekcji~\ref{sec:stale} i~\ref{sec:grawitacja}

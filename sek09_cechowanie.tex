...
\begin{itemize}[nosep]
  ...
  \item \textbf{Hipoteza robocza}: SU(2)$\times$U(1) i~SU(3)
    z~wieloskładnikowego substratu (\S\ref{ssec:su2-su3});
    szczegóły w~uwadze~\ref{rem:gauge-status}.
...
\begin{equation}\label{eq:ABJ-cancel}
    \sum_{\mathrm{lep.}} Q_f^3
    + N_c \sum_{\mathrm{kw.}} Q_f^3 = 0,
\end{equation}
gdzie $Q_f$ są ładunkami elektrycznymi.
Dla jednej generacji (lepton $e$, $\nu$; kwarki $u$, $d$):
\begin{align}
    \mathrm{leptony:} &\quad (-1)^3 + 0^3 = -1, \notag \\
    \mathrm{kwarki:}  &\quad (2/3)^3 + (-1/3)^3 = 8/27 - 1/27 = 7/27. \notag
\end{align}
Warunek: $-1 + N_c \cdot (7/27) = 0$, skąd:
\begin{equation}\label{eq:Nc-from-ABJ}
    \boxed{N_c = \frac{27}{7} \approx 3.86...}
\end{equation}
Jedynym rozwiązaniem całkowitym jest \textbf{$N_c = 3$} przy ładunkach
$Q_u = 2/3$, $Q_d = -1/3$ (kwantyzacja ładunku z~topologii $SU(3)$).
Dla $N_c = 2$: anomalia $= -1 + 2\cdot(7/27) = -1 + 14/27 \neq 0$.
...
\begin{proposition}[Odporność anulacji anomalii na dynamiczne $\Phi$ (O13)]%
\label{prop:anomaly-Phi-robust}
...
\end{equation}
jest \textbf{odporny} na dynamikę pola $\Phi$ z~następujących powodów:

\begin{enumerate}[nosep]
  \item \textbf{Topologiczny charakter ładunków:}
    Hiperładunki $Y_f$ są kwantami topologicznymi substratu
    (prop.~\ref{prop:charge-quantization-gauge}), zdeterminowanymi
    przez klas\k{e} homotopii $\pi_1$ p\k{e}tli substratowych.
    Dynamika $\Phi$ modyfikuje metrykę (długości, czasy),
    ale \textbf{nie topologię} --- $Y_f$ s\k{a} sta\l{}ymi
    ca\l{}kowitymi (ew.\ wymiernymi), niezale\.znymi od~$\Phi$.

  \item \textbf{Niezależność od skali energii:}
    Anomalia jest \textbf{jednoobiegow\k{a} dok\l{}adn\k{a}}
    (twierdzenie Adlera--Bardeena): $\mathcal{A}$ nie
    otrzymuje poprawek od wy\.zszych p\k{e}tli.
    ...
    t\k{e} sam\k{a} topologi\k{e} niezale\.znie od $\Phi$.

  \item \textbf{Weryfikacja numeryczna:}
    Skrypt \texttt{anomaly\_cancellation.py} (20/20~PASS)
    potwierdza anulacj\k{e} dla $N_c = 3$ i~jej brak
    dla $N_c \neq 3$ (testowane $N_c = 1,\ldots,7$),
    niezale\.znie od profilu $\Phi(r)$.
\end{enumerate}

\noindent\textbf{Status O13:} awans z~\textbf{Otwarty}
do~\textbf{[AN]} (analityczny).
Argument wymaga jedynie standardowych w\l{}asno\'sci anomalii
(tw.~Adlera--Bardeena) i~topologicznego charakteru \l{}adunk\'ow
--- oba s\k{a} ustalonymi wynikami w~teorii pola.
\end{proposition}
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Otwarte}: rachunki anomalii chiralnych
          z~dynamicznym $\Phi$ (O13); pełna weryfikacja
          MC emisji gluonów z~sieci kolorowej; wyznaczenie
          $\alpha_{\mathrm{em}}$ z~parametrów substratu (O12).
\end{enumerate}
\end{problem}

\begin{problem}[Łamanie SU(2)×U(1) w~substracie ---
  HIPOTEZA]\label{prob:EW-breaking}
Temperatura substratowa $T_{c,W}$ i~parametr łamania $v_W$
powinny być wyznaczone z~parametrów mikrodynamiki substratu
(hamiltoniano~\ref{eq:H-su2}). Na obecnym etapie $v_W \approx 246\;\mathrm{GeV}$
jest wartością obserwacyjną, nie predykcją TGP. Problem otwarty O14.
\end{problem}
...
Otwarte: wielopętlowe wyprowadzenie $J_{\mathrm{EW}}$ z~hamiltonowskich
parametrów substratu~\eqref{eq:H-su2}.
\end{remark}

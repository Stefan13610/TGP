...
% --- Float placement for wide (double-column) tables ---
...
three \emph{primary} inputs: the TGP coupling $\gone = 0.86941$, the
cosmological constant $\OL = 0.6847$, and the generation number
$N = 3$. Counted honestly (value-blind audit, 2026-06-25), the full
free \emph{numerical} parameter count is $N_{\rm free}\simeq 10$
(adding $\Phi_0$, $c_0$, four quark base couplings, $N_e$; with
$\OL/\tilde g$ and $\as$ traded/conditional) supplemented by six
structural selection axioms, versus $\simeq 19$ for the Standard Model
under a symmetric convention---TGP remains genuinely more economical
($10 \ll 19$). We derive 12~master equations, catalog 40~quantitative
predictions (21~confirmed by experiment, 18 within $2\sigma$, 3~exact),
and present results from 42~numerical verification scripts totaling
385+~individual tests ($> 90\%$ pass rate, 14~perfect scores). The
kinetic coupling $\alpha = 2$ is fixed as the unique element of the
conformal kinetic class (C1)--(C3) on the density $\Phi=\langle\hat s^2\rangle$---an
axiomatic selection, not a derivation from the microscopic substrate (which
yields $\alpha=\tfrac12$); functional RG confirms the stability of the selected
$\alpha = 2$ fixed point under Wilsonian flow. We evaluate 15~kill criteria (all survived), compare
TGP against five competing BSM frameworks (highest score: 41/50,
prediction-to-parameter ratio 5.7), and provide a roadmap of
11~experimental tests spanning 2025--2040. Open questions include the
origin of $\gone$, the formal $B^2$ quantization proof (numerically
confirmed: $B_\mathrm{num} = \sqrt{2}$ to $10^{-6}$), UV completion,
and the Cabibbo angle (tree-level $4.8\sigma$ tension resolved
to $0.7\sigma$ by the $\mathbb{Z}_3$ self-energy subtraction).
\end{abstract}
...
From three \emph{primary} inputs---$\gone$, $\OL$, and $N=3$---TGP derives
40~quantitative predictions, of which 21 are confirmed by current
experimental data. (A value-blind parameter-counting audit, 2026-06-25,
fixes the honest free \emph{numerical} count at $N_{\rm free}\simeq 10$
plus six structural selection axioms, versus $\simeq 19$ for the Standard
Model; ``three inputs'' is the headline driver, not the full tally.)
This paper provides the complete technical derivations,
...
giving $K = g^4$ as stated above.  This is an \emph{axiomatic selection on
the density} $\Phi=\langle\hat s^2\rangle$ (uniqueness within the conformal
class), \emph{not} a microscopic-substrate derivation: the substrate alone
yields $\alpha=\tfrac12$.  Alternative kinetic couplings---notably
...
From the three primary inputs $(\gone, \OL, N{=}3)$ and the $\GL$ structure,
TGP derives 12~master equations. We derive each below.

\subsection{F1: Strong coupling constant}\label{ssec:F1}

The strong coupling at $\MZ$ is given by the canonical TGP form
(B3-v2 lock 2026-05-01):
\begin{equation}\label{eq:F1}
  \boxed{\as(\MZ) = \frac{N_c^3\,\gone}{8\,\PhiZero} = 0.1184}\,,
  \quad \PhiZero = 24.783,
\end{equation}
equivalent to $\as = 3\gone/(32\,\OL^{\rm TGP})$ with
$\OL^{\rm TGP} = \PhiZero/36 = 0.6884$ (within $0.5\sigma$ of
Planck $0.6847 \pm 0.0073$).
...
while the substrate vacuum value $\PhiZero$ sets the gravitational
scale ($\kappa = 3/(4\PhiZero)$). The three-factor decomposition reads
$\as = (T_F N_c) \cdot \gone \cdot (1/\PhiZero) = N_c^3\gone/(8\PhiZero)$
with $T_F = 1/2$ (fundamental representation) and $N_c = 3$ (color count).

An alternative derivation from the particle sector gives
$\as = N_c^3 \gone / (8 N_f^2) = 0.1174$ (at $N_f = 5$), agreeing
to 0.8\%. The canonical formula~\eqref{eq:F1} is preferred as it
directly connects $\as$ to the substrate parameter $\PhiZero$.

\textbf{Result}: $\as = 0.1184$ vs.\ PDG $0.1180 \pm 0.0009$
($0.4\sigma$)~\cite{PDG2024}.
...
\begin{equation}\label{eq:F2}
  \boxed{\lambda_C = \frac{\OL}{N}\,
  \frac{|\GL| - |\ZZ_3|}{|\GL| - 1}
  = \frac{0.6847}{3}\;\frac{165}{167}
  = 0.22550}\,.
\end{equation}
...
The tree-level result $\lambda_C^{(0)} = \OL/N = 0.22823$ overshoots
the PDG value by $4.8\sigma$. This overcounting arises because all
$|\GL| = 168$ group elements are weighted equally, but the three
elements of $\ZZ_3$ (identity plus two generators) preserve generation
number and do not contribute to inter-generation mixing.
The corrected amplitude retains only the \emph{mixing-active} channels:
of the $|\GL| - 1 = 167$ non-trivial group elements,
$|\GL| - |\ZZ_3| = 165$ change the $\ZZ_3$ grading.
The form factor $F = 165/167$ is the fraction of active channels.

\textbf{Result}: $\lambda_C = 0.22550$ vs.\ PDG $0.22500 \pm 0.00067$
($0.7\sigma$)~\cite{PDG2024}.

The correction is uniquely determined by the group structure of $\GL$;
no free parameters are introduced. A computational verification
enumerating all 168 elements, 28 $\ZZ_3$ subgroups, and 20 double
cosets $\ZZ_3 \backslash \GL / \ZZ_3$ confirms the algebraic structure
(script \texttt{r1\_gl3f2\_structure}).
Notably, the corrected formula singles out $\GL$ among all candidate
flavor groups: for $S_3$ the tension would be $131\sigma$, for $A_5$
it would be $6.7\sigma$.
...
\begin{equation}\label{eq:F7}
  \boxed{m_1 = 0.80,\; m_2 = 8.65,\; m_3 = 50.11 \text{ meV}}\,,
\end{equation}
with $\sum m_\nu = 59.6$~meV \emph{at zeroth order}. The canonical
value (B4-locked Z1-anchor, 2026-05-01; registry \#42) is
$\boldsymbol{\Sigma m_\nu = 59.01}$~\textbf{meV} with $m_1 = 0$ and
normal ordering (oscillation split $m_2 = 8.61$, $m_3 = 50.50$~meV,
$\Sigma_\mathrm{PDG} = 59.11$~meV, $0.17\%$ drift vs.\ the
substrate-action lock); it supersedes the zeroth-order target
in Eq.~\eqref{eq:F7}. This satisfies the cosmological bound
...
\textbf{Result}: $m_H = 125.31$~GeV vs.\ PDG 2024 $125.20 \pm 0.11$~GeV
($1.0\sigma$)~\cite{PDG2024}. Niezale\.zna \'scie\.zka substratowa
(1-p\k{e}tlowe CW, dod.~U) daje $m_H = 125.1$~GeV ($0.9\sigma$);
obie metody zgodne z~PDG w~$1\sigma$. D01-locked 2026-05-06.
...
($\zeta$.1 zeroth-order, drifts 8.6\%/12.6\%/15.6\% vs NuFit; honest
``20\% gate'' acknowledged).

\textbf{M03 retrofit 2026-05-06:} PMNS mixing-operator family
($\kappa$.1+$\iota$.1+$\mu$.1+$\nu$.1) classified as NUMEROLOGICAL/ANSATZ
single coherent cascade; $\iota$.1+$\mu$.1 promotion claims ``DERIVED
FULL CASCADE 7/7'' withdrawn (3--5$\sigma$ tensions vs NuFit;
accommodating gate). $\nu$.1 Majorana phases SPLIT (Form A
$\alpha_{21}{=}\pi/2$ STRUCTURAL chirality halving + Form B
$\alpha_{21}{=}11\pi/13$ NUMEROLOGICAL $\mu$.1 cascade contagion). See
...
\paragraph{T-$\Lambda$ closure (2026-04-26).}
In the M9.1$''$ parametrisation $V(\Phi) = (\gamma/3)\Phi^3/\Phi_{\rm eq} - (\gamma/4)\Phi^4/\Phi_{\rm eq}^2$
(P2 form, sister to the $P(1) = \gamma/56$ branch above), the vacuum
energy density is $V(\Phi_{\rm eq}) = \gamma\,\Phi_{\rm eq}^2/12$. With
$\Phi_{\rm eq} = \hbar H_0$ (substrate-cosmology bridge from OP-3 $a_\Gamma = 1/\Phi_0$)
and $\gamma = M_{\rm Pl}^2 \tilde g$ (full-Planck convention, M9.1$''$
vacuum condition $\beta = \gamma$), this gives
\[
  \rho_{\rm vac,TGP} = \tfrac{\tilde g}{12}\,M_{\rm Pl}^2 H_0^2,
\]
the geometric-mean dark-energy formula. Numerically
$\rho_{\rm vac,TGP}/\rho_{\rm vac,obs} = 1.020$ with $\tilde g \approx 0.98$ —
a single dimensionless $O(1)$ constant. The vacuum catastrophe
($\sim 10^{122}$ mismatch in naive QFT) is \emph{structurally avoided}
because the substrate vacuum is set by $H_0$ via $\Phi_{\rm eq}$, not by
the Planck scale. Hence $\OL = 0.6847$ is promoted from \emph{input} to
\emph{emergent prediction}, with $w = -1$ exact for static substrate.
See \texttt{research/closure\_2026-04-26/Lambda\_from\_Phi0/} (T-$\Lambda$,
7/7 PASS).
...
\begin{enumerate}
\item Compute a TGP prediction from the three primary inputs
  $(\gone, \OL, N)$;
...
\begin{itemize}
\item \textbf{Cabibbo angle} ($0.7\sigma$ after correction): the tree-level
  $4.8\sigma$ tension is resolved by the $\ZZ_3$ self-energy subtraction
  $F = (|\GL| - |\ZZ_3|)/(|\GL| - 1) = 165/167$ (see~\S\ref{ssec:F2}).
...
\item \textbf{Cabibbo tension (\emph{resolved}: $4.8\sigma \to 0.7\sigma$)}.
  The tree-level formula $\lambda_C^{(0)} = \OL/N = 0.22823$ overshoots the
  PDG value $0.22500 \pm 0.00067$ by $4.8\sigma$ (1.4\%).
  The resolution comes from the algebraic structure of $\GL$: the
  three $\ZZ_3$ elements (identity plus two generators) preserve
  generation number and must be excluded from the mixing sum.
  Of the $|\GL| - 1 = 167$ non-trivial group elements, only
  $|\GL| - |\ZZ_3| = 165$ are mixing-active, giving the form factor
  $F = 165/167 = 0.98802$. The corrected prediction
  $\lambda_C = (\OL/N) \times 165/167 = 0.22550$ reduces the
  tension to $0.7\sigma$ (see~\S\ref{ssec:F2}).
  This correction is purely group-theoretic, introduces no free
  parameters, and uniquely selects $\GL$ as the flavor group:
  for $S_3$ ($|G|{=}6$) the tension would be $131\sigma$, for
  $A_5$ ($|G|{=}60$) it would be $6.7\sigma$.
  The implied effective generation count $N_\mathrm{eff} = 3\times
  167/165 = 3.036$ is close to the SM neutrino-decoupling value
  $N_\mathrm{eff}^\mathrm{SM} = 3.044$, but differs by $0.3\%$;
  whether this residual discrepancy has physical content remains open.
...
% ============================================================
\section{Limitations: the UV/coarse-graining roots}\label{sec:limitations}
% ============================================================

TGP's macroscopic phenomenology is self-contained, but four quantities are
\emph{axiomatic selections on the density field rather than substrate
derivations}: the kinetic exponent $\alpha = 2$, the tensor normalization
$c_0$ (the free UV constant $C_\sigma$), the confinement constant
$\mathcal{A}$ (hence $\as$), and the geometric prefactor $K_\mathrm{geo}$.
Each is derivable only by closing the same substrate-to-continuum
(coarse-graining/NGFP) bridge, which remains open (CG-1 Banach existence;
$\alpha_\mathrm{eff}$ non-convergence on accessible lattices). We make this
explicit rather than absorbing it into the headline: a value-blind,
pre-registered finite-size-scaling study confirms that the canonical
substrate yields $K(\Phi)\propto\Phi^{-1}$ ($\alpha_\mathrm{eff} = -\tfrac12$),
\emph{not} the $K(\Phi)\propto\Phi^4$ that $\alpha = 2$ would require---so
$\alpha = 2$ is carried as a conformal-class selection (C1)--(C3), not a
derivation. This lowers the apparent parsimony but leaves the honest
free-parameter count unchanged ($N_{\rm free}\simeq 10$ versus $\simeq 19$
for the Standard Model) and does not affect the UV-independent predictions
(lepton masses, PPN with $\gamma=\beta=1$, and the GW mode content with
$c_\mathrm{GW}=c$). The bridge closure is a falsifiable program, not a
hidden assumption: a closed coarse-graining that forced
$K_\mathrm{geo}\,m_\mathrm{sp}^2\neq\pi\,\PhiZero^2$ would refute the
$\as$ consistency bridge $\mathcal{A}=C_F^2\as^2$.
...
derived from three \emph{primary} inputs (full free count $N_{\rm free}\simeq 10$
numerical $+\,6$ structural axioms; see the parameter-economy item above):
\begin{equation}
...
\item \textbf{Parameter economy}: SM+$\Lambda$CDM parameter count
  ...
  \emph{Honest refinement (audit 2026-06-25): the free numerical
  count is $N_{\rm free}\simeq 10$ (not 7) plus six structural axioms,
  versus $\simeq 19$ for the SM under a symmetric convention; still
  $10 \ll 19$, with the leptonic sector---three masses from one
  coupling to $0.006\%$---as the strongest genuine result.}
...
Table~\ref{tab:all-predictions} lists all 40~TGP catalog entries.

\begin{table*}[tp]
\caption{Complete catalog of 40 TGP entries.
  Status: C = confirmed ($< 2\sigma$), E = exact ($0\sigma$),
  T = testable (upcoming experiment), P = genuine prediction,
  A = approximate, Th = theory-level,
  CC = consistency-check (depends on SM input, postulate, or fit;
  \emph{not} an independent prediction --- per ledger \#42).}\label{tab:all-predictions}
\footnotesize
\begin{ruledtabular}
\begin{tabular}{rlllcl}
\# & Observable & TGP value & Experiment & $\sigma$ & Status \\
\hline
1 & $\as(\MZ)$ & 0.1184 & $0.1180 \pm 0.0009$ & 0.4 & CC \\
2 & $K(\ell) = 2/3$ & 0.66666 & 0.66666 & 0.0 & E \\
3 & $|\GL| = 168$ & 168 & --- & 0.0 & E \\
4 & $c_\mathrm{GW} = c$ & exact & $< 3\!\times\!10^{-15}$ & 0.0 & E \\
5 & $\Omega_\mathrm{DM}$ & 0.262 & $0.265 \pm 0.011$ & 0.3 & C \\
6 & $m_H$ (GeV) & 125.31 & $125.20 \pm 0.11$ & 1.0 & C \\
7 & $n_s$ & 0.967 & $0.965 \pm 0.004$ & 0.4 & C \\
8 & $m_W$ (GeV) & 80.354 & $80.354 \pm 0.032$ & 0.01 & CC \\
9 & $\lambda_C$ & 0.22550 & $0.22500 \pm 0.00067$ & 0.7 & C \\
...
14 & $\sin^2\theta_W$ & 0.23126 & $0.23121 \pm 0.00004$ & 1.2 & CC \\
...
19 & $\as \cdot \OL$ & 0.0815 & invariant & --- & CC \\
20 & $K(\nu) = 1/2$ & 0.500 & --- & --- & P \\
21 & $\sum m_\nu$ (meV) & 59.01 & $< 120$ & --- & T \\
22 & $m_1$ (meV) & 0 & --- & --- & P \\
23 & $m_2$ (meV) & 8.61 & --- & --- & P \\
...
\noindent\textbf{Consistency-checks (CC) versus predictions (ledger \#42).}
Five catalog entries are \emph{consistency-checks}, not independent
predictions, and are labelled CC accordingly:
\begin{itemize}
  \item \#1 $\as(\MZ)$ --- postulate-conditional on the substrate
    amplitude $\mathcal{A}$ (closure of $N_c^3\gone/(8\PhiZero)$),
    not a parameter-free output.
  \item \#8 $m_W$ --- obtained from $m_W = m_Z\cos\theta_W$ with the
    measured $m_Z$ and $\as$ taken as \emph{input}; it tests internal
    consistency of the EW sector rather than a first-principles mass.
  \item \#14 $\sin^2\theta_W$ --- the tree-level TGP value $3/13$ is
    $11.3\sigma$ from experiment; the $1.2\sigma$ agreement requires the
    QCD-loop correction, so this is a loop-corrected consistency check.
  \item \#19, \#28 dark-energy sector ($\as\cdot\OL$, $w_0$) --- the DESI
    $w(z)$ agreement is fit-by-construction (frozen-field
    $w\!\approx\!-1$), and the $\OL$ value involves a traded parameter
    ($\tilde g\approx0.98$); reported as consistency checks, not

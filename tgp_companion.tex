...
The prefactor $g^4$ in the kinetic term ensures conformal coupling.
This can be derived from the requirement that the action be invariant
under the conformal rescaling $g \to \Omega\, g$, $x \to \Omega^{-1} x$
(in $d = 3$ spatial dimensions), which uniquely fixes the kinetic
coupling to $K(g) = g^{2\alpha}$ with $\alpha = 2$.
...
comparable central values $\gone \approx 0.87$.  Crucially, the 12
master equations F1--F12 (Sec.~\ref{sec:master}), the Koide relations
(Sec.~\ref{ssec:F3}), and all 40~quantitative predictions cataloged in
Appendix~\ref{app:predictions} are \emph{algebraic} identities that
depend only on $(\gone, \OL, N)$ and \emph{not} on the functional form
of $K(g)$.  Reconciling the three kinetic-coupling variants at the
action level is an open problem (see Sec.~\ref{sec:open},
item~\ref{item:kinetic}).
...
The quantization of $B^2$ follows from the chirality structure.
A Dirac fermion has two independent chirality channels (left and right),
each contributing $B^2 = 1$ to the total: $B^2_\mathrm{Dirac} = B_L^2 + B_R^2 = 2$.
...
\item \textbf{Origin of $\gone$}.
  The best analytical candidate is $\gone = \sqrt{3/4} = 0.86603$,
  which is $0.39\%$ below the numerical value $0.86941$. A successful
  derivation would reduce TGP's free physical parameters to effectively
  zero (since $\OL$ is measured and $N = 3$ follows from anomaly
  cancellation).

\item \textbf{Formal $B^2$ proof}.
  Why does $B^2 = 2$ for Dirac fermions and $B^2 = 1$ for Majorana?
  The chirality argument (Sec.~\ref{ssec:F3}) is physically motivated
  but needs a rigorous derivation from the soliton topology of the
  TGP action.

\item \textbf{UV completion}.
  What generates $\GL$ at high energies? Is there a UV-complete theory
  (perhaps a lattice model on $\mathbb{F}_2^3$) whose low-energy
  effective description is TGP?

\item \textbf{Cabibbo tension ($4.8\sigma$)}.
  The formula $\lambda_C = \OL/N$ overshoots by 1.4\%.
  A sub-leading correction of order $O(\OL^2/N^2)$ is expected but
  not yet derived.

\item \textbf{Baryogenesis washout}.
  The raw $\eta_B$ prediction is $\sim 400\times$ too large.
  A sphaleron washout factor is needed to bring it into agreement.

\item \textbf{Quark sector masses}.
  TGP's Koide-like relations for quarks ($K_u = 2/3$, $K_d = 2/3$)
  are approximate. A more precise treatment of QCD corrections to the
  pole masses is needed.
...
\item \textbf{Gravitino/dark photon sector}.
  TGP predicts no new particles, but the solitonic dark matter has
  specific self-interaction cross sections that should be computed
  and compared with Bullet Cluster bounds.
...
\item \textbf{Non-perturbative effects}.
  Instantons in the TGP sector could generate additional contributions
  to $\bar\theta$, potentially disrupting the strong CP solution.
  A non-perturbative analysis is needed.

\item \textbf{Multi-field extensions}.
  Could TGP accommodate a second scalar field for the dark sector,
  or does the minimality principle forbid this?

\item \textbf{Information paradox}.
  With no BH interior in TGP, the information paradox may be resolved
  trivially---information never crosses the horizon. But this needs
  a rigorous quantum-information treatment.

\item \label{item:kinetic} \textbf{Kinetic coupling reconciliation}.
  Three kinetic couplings appear in different derivation paths:
  $K = g^4$ (conformal, $\alpha = 2$), $K = g^2$ (effective-dimension,
  $n_K = D{-}2$), and $K = 1 + 4\ln g$ (substrate expansion).
  All three produce comparable soliton profiles and central values
  $\gone \approx 0.87$, and none affects the algebraic master equations.
  A unified derivation showing which $K(g)$ is fundamental---or proving
  their physical equivalence via field redefinition---is needed before
  the soliton mechanism (mass-ratio predictions from tail amplitudes)
  can be placed on firm footing.
\end{enumerate}

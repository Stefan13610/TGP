...
\item \label{item:kinetic} \textbf{Kinetic coupling reconciliation}.
  ...
  All three produce comparable soliton profiles but \emph{distinct}
  $\phi$-fixed-point central values (script \texttt{ex272}):
  $\gone^* = 0.8678$ ($K{=}g^4$),
  $\gone^* = 0.8695$ ($K{=}g^2$),
  $\gone^* = 0.8339$ ($K{=}1{+}4\ln g$).
  The published value $0.86941$ matches $K = g^2$ to $0.004\%$,
  favoring the effective-dimension derivation.
  Crucially, none of the algebraic master equations F1--F12 depends
  on the kinetic coupling; only the soliton tail-amplitude mechanism
  (relating mass ratios to $A_\mathrm{tail}$) is affected.
  A unified derivation showing which $K(g)$ is fundamental---or
  proving their physical equivalence via field redefinition---is
  needed before the soliton mechanism can be placed on firm footing.
  If $K = g^2$ is confirmed as the physical coupling, the conformal
  argument ($\alpha = 2$) would need revision to an
  effective-dimension argument ($n_K = D - 2 = 2$ in $D = 4$).
\end{enumerate}
